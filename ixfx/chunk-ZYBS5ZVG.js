import {
  ...
  isLine,
  isQuadraticBezier
} from "./chunk-TXGLTN6N.js";
import {
  stack
} from "./chunk-5JFBGWGL.js";
import {
  lerp,
  startsEnds
} from "./chunk-MXKQX7ER.js";
import {
  resolveEl
...
var rgbFromArray = (v) => {
  if (v.length !== 3)
    throw new Error(`expected array of three`);
...
var guard2 = (rgb2) => {
  if (`a` in rgb2) {
    number(rgb2.a, `percentage`, `rgb[3]`);
  }
  if (rgb2.r < 0 || rgb2.r > 255)
    throw new Error(`rgba.r expected to be 0-255`);
  if (rgb2.g < 0 || rgb2.g > 255)
    throw new Error(`rgba.g expected to be 0-255`);
  if (rgb2.b < 0 || rgb2.b > 255)
...
var guard3 = (hsl2) => {
  number(hsl2.h, ``, `hsl.h`);
  if (hsl2.s > 360)
    throw new Error(`hsl.s expected to be 0-360`);
  number(hsl2.s, `percentage`, `hsl.s`);
  number(hsl2.l, `percentage`, `hsl.l`);
  if (`a` in hsl2) {
    number(hsl2.a, `percentage`, `hsl.a`);
  }
};
var toRgb = (hsl2) => {
  let { h, s, l } = hsl2;
  let t1, t2, t3, rgb2, i = 0;
  if (s === 0) {
    let val = l * 255;
  ...
  for (; i < 3; ) {
    t3 = h + 1 / 3 * -(i - 1);
    t3 < 0 ? t3++ : t3 > 1 && t3--;
    let val = 6 * t3 < 1 ? t1 + (t2 - t1) * 6 * t3 : 2 * t3 < 1 ? t2 : 3 * t3 < 2 ? t1 + (t2 - t1) * (2 / 3 - t3) * 6 : t1;
    rgb2[i++] = val * 255;
  }
...
var fromRgb = (rgb2) => {
  ...
  g /= 255;
  b /= 255;
  var min = Math.min(r2, g, b), max2 = Math.max(r2, g, b), delta = max2 - min, h, s, l;
  h = 0;
  if (max2 === min) {
    h = 0;
  } else if (r2 === max2) {
...
var lerp3 = (amount, a, b) => {
  ...
  let f = false;
  let dist = hb - ha;
  const distForward = b.h - a.h;
  const distBackward = 360 - b.h + a.h;
  let incrBy = 0;
  if (Math.abs(distBackward) < Math.abs(distForward)) {
    hb = -(ha + (360 - hb));
  }
  console.log(`a: ${a.h} b: ${b.h} distF: ${distForward} distB: ${distBackward}`);
  let h = lerp(amount, ha, hb) - incrBy;
  if (h > 360)
    h = h % 360;
  const s = lerp(amount, a.s, b.s);
  const l = lerp(amount, a.l, b.l);
  const hsl2 = Object.freeze({ h, s, l });
  guard3(hsl2);
  return hsl2;
};
var toCss = (hsl2) => {
  if (`a` in hsl2) {
    return `hsl(${hsl2.h}, ${hsl2.s * 100}%, ${hsl2.l * 100}%, ${hsl2.a * 100}%)`;
  } else {
    return `hsl(${hsl2.h}, ${hsl2.s * 100}%, ${hsl2.l * 100}%)`;
  }
};
var hsl = {
  name: "hsl",
...
var parseNeutral = (c) => {
  ...
  if (r === void 0)
    throw new Error(`Cannot parse as HSL (${c})`);
};
