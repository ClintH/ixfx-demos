...
var draw = (buffer, drawing) => {
  ...
  if (drawing.showYAxis) {
    series.forEach((s) => {
      ...
      drawVerticalAxis(s, { height: plotSize.height + margin + margin, width: plotSize.width }, drawing);
      ctx.translate(yLabelWidth + 3, 0);
    });
  }
  if (drawing.showXAxis) {
    drawHorizontalAxis(plotSize, drawing);
  }
  ctx.translate(margin, margin);
  const plotDrawing = {
    ...drawing,
  ...
  series.forEach((s) => {
    ...
    if (data2 === void 0)
      return;
    drawSeriesData(s, data2, plotSize, plotDrawing);
  });
  ctx.resetTransform();
};
...
var drawHorizontalAxis = (plotSize, drawing) => {
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(yLabelWidth, height);
  ctx.lineTo(width, height);
  ctx.stroke();
};
var drawSeriesData = (series, values, plotSize, drawing) => {
