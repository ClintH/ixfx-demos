import {
  ...
  Svg_exports,
  opacity
} from "./chunk-3HPWVP33.js";
import {
  mapCircular,
  stack
} from "./chunk-RIZQWJQ7.js";
import {
  resolveEl
} from "./chunk-HBZOTNT6.js";
import {
  getCorners,
  guard,
  isCubicBezier,
  isLine,
  isQuadraticBezier
} from "./chunk-7CELPBFO.js";
import {
  minMaxAvg,
  randomElement
} from "./chunk-V2CC3OS2.js";
import {
  array,
  number
...
var draw = (buffer, drawing) => {
  ...
  const showYAxis = drawing.showYAxis;
  const margin = 10;
  ...
  let plotSize = drawing.plotSize ?? plotSizeFromBounds(canvasSize, drawing);
  ...
  if (plotDrawing.dataXScale === void 0)
    plotDrawing.dataXScale = plotSize.width / cap, ctx.translate(0, margin);
  series.forEach((s) => {
...
var drawSeriesAxis = (series, drawing) => {
  const { ctx, canvasSize, palette } = drawing;
  ...
  ctx.fillText(series.min.toFixed(2), 0, canvasSize.height - drawing.textHeight - y);
  ctx.fillText(series.max.toFixed(2), 0, halfHeight);
  ctx.fillText(mid.toFixed(2), 0, canvasSize.height / 2 - halfHeight);
};
var drawSeries = (series, values, drawing) => {
  const { ctx, canvasSize, dataXScale = 1, lineWidth = 2, fixedRange } = drawing;
  ...
  for (let i = 0; i < values.length; i += incrementBy) {
    let y = (1 - (values[i] - series.min) / series.range) * canvasSize.height + 5;
    if (style === `dots`) {
      ctx.beginPath();
...
var plot = (parentElOrQuery, opts) => {
  ...
  if (parentEl.nodeName === `CANVAS`) {
    ...
    if (plotSize) {
      canvasEl.width = plotSize.width + 100;
      canvasEl.height = plotSize.height + 100;
    }
    canvasSize = { width: canvasEl.width, height: canvasEl.height };
  }
  ...
  number(capacity, `aboveZero`, `capacity`);
  if (ctx === null)
