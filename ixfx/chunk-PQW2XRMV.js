...
var require_ReadonlyNonEmptyArray = __commonJS({
  "node_modules/fp-ts/lib/ReadonlyNonEmptyArray.js"(exports) {
    ...
    var snoc = function(init3, end) {
      return (0, function_1.pipe)(init3, concat3([end]));
    };
    exports.snoc = snoc;
...
var clampIndex = (v, arrayOrLength) => {
  ...
  if (!Number.isInteger(length6))
    throw new Error(`length parameter must be an integer (${length6}, ${typeof length6})`);
  v = Math.round(v);
  if (v < 0)
    return 0;
  if (v >= length6)
...
var afterMatch = (source, match, opts = {}) => {
  const startPos = opts.startPos ?? void 0;
  const fromEnd = opts.fromEnd ?? false;
  const m = fromEnd ? source.lastIndexOf(match, startPos) : source.indexOf(match, startPos);
  if (m < 0)
...
async function max2(it, gt = (a, b) => a > b) {
  let max6;
  for await (const v of it) {
    if (!max6) {
      max6 = v;
...
var hasKeyValue = (map5, key, value, comparer) => {
  if (!map5.has(key))
    return false;
  const values = Array.from(map5.values());
  return values.some((v) => comparer(v, value));
};
var deleteByValue = (map5, value, comparer = isEqualDefault) => {
  for (const e of Object.entries(map5)) {
    if (comparer(e[1], value)) {
      map5.delete(e[0]);
    }
  }
};
var getOrGenerate = (map5, fn) => async (key, args) => {
...
var getOrGenerateSync = (map5, fn) => (key, args) => {
  let value = map5.get(key);
  if (value !== void 0)
    return value;
  value = fn(key, args);
...
var sortByValue = (map5, compareFn) => {
  const f = compareFn ?? defaultComparer;
  [...map5.entries()].sort((a, b) => f(a[1], b[1]));
};
var sortByValueProperty = (map5, prop, compareFn) => {
  const cfn = typeof compareFn === `undefined` ? defaultComparer : compareFn;
  return [...map5.entries()].sort((aE, bE) => {
    const a = aE[1];
    const b = bE[1];
    return cfn(a[prop], b[prop]);
  });
};
var hasAnyValue = (map5, value, comparer) => {
  const entries = Array.from(map5.entries());
  return entries.some((kv) => comparer(kv[1], value));
};
function* filter2(map5, predicate) {
  for (const v of map5.values()) {
    if (predicate(v))
      yield v;
  }
}
var toArray2 = (map5) => Array.from(map5.values());
...
var fromObject = (data) => {
  ...
  if (Array.isArray(data)) {
    data.forEach((d) => addObject(map5, d));
  } else {
    addObject(map5, data);
  }
  return map5;
};
var addObject = (map5, data) => {
  const entries = Object.entries(data);
  for (const [key, value] of entries) {
    map5.set(key, value);
  }
};
var find2 = (map5, predicate) => Array.from(map5.values()).find((vv) => predicate(vv));
...
`;
  ...
  deleteKeyValueFromMap(map5, key, value) {
    const preCount = this.type.count(map5);
    const filtered = this.type.without(map5, value);
  ...
  deleteByValue(value) {
    let something = false;
    Array.from(__privateGet(this, _map).keys()).filter((key) => {
      const a = __privateGet(this, _map).get(key);
      if (!a)
  ...
  *entries() {
    for (const e of __privateGet(this, _map).entries()) {
...
var addArray = (map5, data) => {
  const x = new Map(map5.entries());
  data.forEach((d) => {
    if (d[0] === void 0)
...
var add = (map5, ...data) => {
  ...
  return isObj ? addObjects(map5, data) : addArray(map5, data);
};
var set = (map5, key, value) => {
  const x = new Map(map5.entries());
  x.set(key, value);
  return x;
};
...
var _children, _parent;
var _TreeNodeMutable = class {
  /**
   * Constructor
  ...
  constructor(value, label) {
    ...
    __publicField(this, "hasChild", (possibleChild) => hasChild(this, possibleChild));
    __publicField(this, "hasAnyChild", (possibleChild) => hasAnyChild(this, possibleChild));
    __publicField(this, "hasParent", (possibleParent) => hasParent(this, possibleParent));
    __publicField(this, "hasAnyParent", (possibleParent) => hasAnyParent(this, possibleParent));
    __publicField(this, "getByPath", (path2, opts = {}) => {
  ...
  add(child) {
    ...
    __privateSet(this, _children, [...__privateGet(this, _children), child]);
    if (__privateGet(child, _parent)) {
      __privateGet(child, _parent).remove(child);
    }
  ...
  remove(child) {
    ...
    const w = without(__privateGet(this, _children), child);
    if (w.length === __privateGet(this, _children).length)
      throw new Error(`child not found in descendants`);
    __privateSet(this, _children, w);
  }
  /**
   ...
   */
  setDescendants(d) {
    for (const d2 of __privateGet(this, _children)) {
      __privateSet(d2, _parent, void 0);
    }
    for (const dd of d) {
      couldAddChild(this, dd);
      __privateSet(dd, _parent, this);
    }
    __privateSet(this, _children, d);
  }
  /**
  ...
  prettyPrint(indent = 0) {
    ...
    if (__privateGet(this, _children).length) {
      return t4 + "\n" + __privateGet(this, _children).map((d) => d.prettyPrint(indent + 1)).join("\n");
    } else {
      return t4;
    }
  }
  ...
  *parents() {
    let n = this;
    while (__privateGet(n, _parent) !== void 0) {
      yield __privateGet(n, _parent);
      n = __privateGet(n, _parent);
    }
  }
  /**
   ...
   */
  *children() {
    for (const c of __privateGet(this, _children)) {
      yield c;
    }
  }
  /**
   ...
   */
  get isEmpty() {
    return __privateGet(this, _children).length === 0;
  }
};
var TreeNodeMutable = _TreeNodeMutable;
...
function* depthFirst(root) {
  if (!root)
    return;
  const stack2 = stackMutable();
  stack2.push(getEntry(root, "root"));
  let entry = stack2.pop();
  while (entry) {
    yield entry;
    if (entry) {
      stack2.push(...directChildren(entry[1], entry[0]));
    }
    if (stack2.isEmpty)
      break;
    entry = stack2.pop();
...
function* breadthFirst(root) {
  if (!root)
    return;
  const queue2 = queueMutable();
  queue2.enqueue(getEntry(root, "root"));
  let entry = queue2.dequeue();
  while (entry) {
    yield entry;
    if (entry) {
      queue2.enqueue(...directChildren(entry[1], entry[0]));
    }
    if (queue2.isEmpty)
      break;
    entry = queue2.dequeue();
  }
}
...
function length2(aOrLine, pointB) {
  if (isPolyLine(aOrLine)) {
    const sum5 = aOrLine.reduce((acc, v) => length2(v) + acc, 0);
    return sum5;
  }
...
function hasElapsedMs(totalMs) {
  const t4 = relativeTimer(totalMs, msElapsedTimer());
  return () => t4.isDone;
}
function completionMs(totalMs) {
  const t4 = relativeTimer(totalMs, msElapsedTimer());
  return () => t4.elapsed;
}
var frequencyTimerSource = (frequency) => () => frequencyTimer(frequency, msElapsedTimer());
var relativeTimer = (total3, timer, clampValue = true) => {
  let done = false;
  let modAmt = 1;
  return {
    mod(amt) {
...
var relativeTimerMs = (total3, clampValue = true) => relativeTimer(total3, msElapsedTimer(), clampValue);
var relativeTimerTicks = (total3, clampValue = true) => relativeTimer(total3, ticksElapsedTimer(), clampValue);
...
var timeout = (callback, timeoutMs) => {
  ...
  let timer = 0;
  let startedAt = 0;
  const start3 = async (altTimeoutMs = timeoutMs, args) => {
...
var continuously = (callback, intervalMs, opts = {}) => {
  ...
  let currentTimer = 0;
  const schedule = iMs === 0 ? raf : (cb) => window.setTimeout(cb, iMs);
  const deschedule = iMs === 0 ? (_) => {
...
// src/flow/BackOff.ts
var retryWithBackOff = async (cb, opts) => {
  ...
  while (attempts <= count2) {
    ...
    const ok = await cb();
    if (ok)
      return { success: true, attempts, elapsed: Date.now() - startedAt };
    log2({ msg: `retryWithBackOff attempts: ${attempts} t: ${elapsedMs(t4)}` });
    try {
      await sleep(t4, 0, signal);
    } catch (ex) {
...
var _TaskQueue = class {
  constructor(opts = {}) {
    //eslint-disable-next-line @typescript-eslint/no-explicit-any
    __publicField(this, "_timer", 0);
    __publicField(this, "_queue");
    __publicField(this, "_startDelayMs");
    __publicField(this, "_intervalMs");
    this._startDelayMs = opts.startDelayMs ?? 500;
    this._intervalMs = opts.intervalMs ?? 100;
    this._queue = Queue_exports.queueMutable();
  }
  /**
...
__publicField(TaskQueue, "instance", new _TaskQueue());

// src/flow/index.ts
var forEach3 = (iterator2, fn) => {
...
try {
  if (typeof window !== `undefined`) {
    window.ixfx = { ...window.ixfx, Flow: { StateMachine: StateMachine_exports, Timer: Timer_exports, forEach: forEach3, forEachAsync, repeat } };
  }
} catch {
}
...
var min4 = (...data) => min(data);
var max4 = (...data) => max(data);
...
var { abs: abs3, min: min5, max: max5, cos: cos2, sin: sin2, acos: acos2, sqrt: sqrt2 } = Math;
var pi2 = Math.PI;
var Bezier = class {
  constructor(coords) {
  ...
  lineIntersects(line3) {
    const mx = min5(line3.p1.x, line3.p2.x), my = min5(line3.p1.y, line3.p2.y), MX = max5(line3.p1.x, line3.p2.x), MY = max5(line3.p1.y, line3.p2.y);
...
var randomPoint = (within, opts = {}) => {
  const offset2 = isPositioned2(within) ? within : { x: 0, y: 0 };
...
function* asRectangles(grid2) {
  for (const c of cells(grid2)) {
    yield rectangleForCell(c, grid2);
  }
}
...
var array2dUpdater = (grid2, array3) => {
  const fn = (v, position) => {
    ...
    if (pos === void 0)
      throw new Error(`Position does not exist. Pos: ${JSON.stringify(position)} Grid: ${JSON.stringify(grid2)}`);
...
var _items, _children2, _parent2, _subdivide, subdivide_fn;
var _QuadTreeNode = class {
  /**
   * Constructor
   * @param boundary 
   * @param level 
  ...
  getLengthChildren() {
    return __privateGet(this, _children2).length;
  }
  *parents() {
    let n = this;
    while (__privateGet(n, _parent2) !== void 0) {
      yield __privateGet(n, _parent2);
      n = __privateGet(n, _parent2);
    }
  }
  *children() {
    for (const c of __privateGet(this, _children2)) {
      yield c;
    }
  }
  /**
   ...
   */
  direction(d) {
    return __privateGet(this, _children2)[d];
  }
  /**
  ...
  add(p) {
    if (!Shape_exports.isIntersecting(this.boundary, p))
      return false;
    if (__privateGet(this, _children2).length) {
      __privateGet(this, _children2).forEach((d) => d.add(p));
      return true;
    }
    __privateGet(this, _items).push(p);
    if (__privateGet(this, _items).length > this.opts.maxItems && this.level < this.opts.maxLevels) {
      if (!__privateGet(this, _children2).length) {
        __privateMethod(this, _subdivide, subdivide_fn).call(this);
      }
      __privateGet(this, _items).forEach((item) => {
        __privateGet(this, _children2).forEach((d) => d.add(item));
      });
...
var QuadTreeNode = _QuadTreeNode;
_items = new WeakMap();
_children2 = new WeakMap();
_parent2 = new WeakMap();
_subdivide = new WeakSet();
subdivide_fn = function() {
  const w = this.boundary.width / 2;
  const h = this.boundary.height / 2;
  const x = this.boundary.x;
  const y = this.boundary.y;
  const coords = Point_exports.fromNumbers(
    x + w,
    y,
    x,
    y,
  ...
  __privateSet(this, _children2, rects.map((r) => new _QuadTreeNode(this, r, this.level + 1, this.opts)));
};
...
var centroid2 = (t4) => {
  guard7(t4);
  const total3 = reduce3([t4.a, t4.b, t4.c], (p, acc) => ({
    x: p.x + acc.x,
    y: p.y + acc.y
  }));
  const div = {
...
var fromRadius = (origin, radius, opts = {}) => {
  ...
  const angles2 = [initialAngleRadian, initialAngleRadian + piPi5 * 1 / 3, initialAngleRadian + piPi5 * 2 / 3];
...
function map4(project2, thisArg) {
...
var scaleWithFixedRange = (buffer, range3, drawing) => calcScale(buffer, drawing, drawing.seriesColours).map((s) => ({ ...s, range: range3[1] - range3[0], min: range3[0], max: range3[1] }));
...
var drawYSeriesScale = (series, plotSize, drawing) => {
  ...
  ctx.textBaseline = `top`;
  ctx.fillText(min6.toFixed(digitsPrecision), 0, calcYForValue(min6, series, height4) - halfHeight);
  ctx.fillText(mid.toFixed(digitsPrecision), 0, calcYForValue(mid, series, height4) - halfHeight);
  ctx.fillText(max6.toFixed(digitsPrecision), 0, calcYForValue(max6, series, height4) - margin);
...
function* circleRings(circle3, opts = {}) {
  ...
  for (let r = ringR; r <= 1; r += ringR) {
    ...
    for (const theta of linearSpace(0, piPi7, n + 1)) {
      yield Object.freeze({
...
var pingPong = function* (interval2, lower, upper, start3, rounding) {
  ...
  if (start3 > upper || start3 < lower)
    throw new Error(`Start (${start3 / rounding}) must be within lower (${lower / rounding}) and upper (${upper / rounding})`);
  let v = start3;
  yield v / rounding;
  let firstLoop = true;
  while (true) {
...
var numericRange = function* (interval2, start3 = 0, end, repeating = false, rounding) {
  ...
  if (end === void 0) {
    ...
    if (negativeInterval && start3 < end)
      throw new Error(`Interval of ${interval2} will never go from ${start3} to ${end}`);
    if (!negativeInterval && start3 > end)
      throw new Error(`Interval of ${interval2} will never go from ${start3} to ${end}`);
  }
...
var rx = (elOrQuery, event, opts) => {
  ...
  if (opts === void 0)
    return setup(ev);
  if (opts.pluck) {
    return setup(ev.pipe(map4((x) => x[opts.pluck])));
  } else if (opts.transform) {
    return setup(ev.pipe(map4((x) => opts.transform(x))));
  }
  return setup(ev);
};
...
var jitter = (opts = {}) => {
  ...
  if (typeof opts.absolute !== "undefined") {
    number(opts.absolute, clamped ? `percentage` : `bipolar`, `opts.absolute`);
    const absRand = floatFn({ min: -opts.absolute, max: opts.absolute, source: opts.source });
    r = (v) => v + absRand();
  } else if (typeof opts.relative !== "undefined") {
    number(opts.relative, clamped ? `percentage` : `bipolar`, `opts.relative`);
    r = (v) => v + float({ min: -opts.relative * v, max: opts.relative * v, source: opts.source });
  } else {
...
var FrequencyMutable = class extends SimpleEventEmitter {
  ...
  debugString() {
    let t4 = ``;
    for (const [key, count2] of __privateGet(this, _store2).entries()) {
      t4 += `${key}: ${count2}, `;
    }
    if (t4.endsWith(`, `))
...
var align = (similarityFn, lastData, newData, opts = {}) => {
  ...
  for (let i = 0; i < newData.length; i++) {
    ...
    const scoredLastValues = Array.from(lastMap.values()).map((last2) => ({ id: last2.id, score: last2 === null ? -1 : similarityFn(last2, newD), last: last2 }));
...
var randomPluck2 = (array3, mutate = false, rand = defaultRandom) => {
  if (array3 === void 0)
...
tslib/tslib.es6.js:
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=chunk-PQW2XRMV.js.map