...
var draw = (buffer, drawing) => {
  ...
  series.forEach((s) => {
    ...
    if (data2 === void 0)
      return;
    const leadingEdgeIndex = buffer.typeName === `circular` ? data2.pointer - 1 : data2.length - 1;
    drawSeriesData(s, data2, plotSize, plotDrawing, leadingEdgeIndex);
  });
  ctx.resetTransform();
};
var drawVerticalAxis = (series, plotSize, drawing) => {
...
var drawHorizontalAxis = (plotSize, drawing) => {
  const { width, height } = plotSize;
  const { ctx, palette, yLabelWidth } = drawing;
...
var drawSeriesData = (series, values, plotSize, drawing, leadingEdgeIndex) => {
  ...
  let x = 0;
  let leadingEdge;
  const drawIndex = drawing.drawIndex ?? -1;
  const colourTransform = (c) => {
    if (translucentPlot) {
      return Colour_exports.opacity(c, 0.5);
    }
    return c;
  };
  if (style === `dots`) {
