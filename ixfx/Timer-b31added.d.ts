...
declare type Timeout = HasCompletion & {
    start(altTimeoutMs?: number, args?: readonly unknown[]): Promise<void>;
    cancel(): void;
    get isDone(): boolean;
};
/**
 ...
 * @returns
 */
declare const debounce: (callback: () => void | Promise<any>, timeoutMs: number) => (...args: unknown[]) => Promise<void>;
/***
 ...
 * ```
 *
 * Waiting for execution. `.start()` returns a Promise that can be awaited:
 * ```js
 * const t = timeout(fn, 1000);
 * await t.start();
 * console.log(`done`); // Only runs after timeout has happened.
 * ```
 *
 * @param callback
 * @param timeoutMs
...
export { Continuously as C, HasCompletion as H, ModTimer as M, TimerSource as T, Timer as a, Timeout as b, TimeoutSyncCallback as c, debounce as d, TimeoutAsyncCallback as e, timeout as f, ContinuouslySyncCallback as g, ContinuouslyAsyncCallback as h, interval as i, continuously as j, delay as k, frequencyTimerSource as l, frequencyTimer as m, msElapsedTimer as n, ticksElapsedTimer as o, relativeTimer as r, sleep as s, throttle as t };
