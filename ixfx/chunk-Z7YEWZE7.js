...
var toSvg2 = (a, origin) => {
  if (isCircle(a)) {
    if (origin !== void 0) {
      return toSvgFull2(a.radius, origin);
    }
    if (isPositioned2(a)) {
      return toSvgFull2(a.radius, a);
    } else
      throw new Error(`origin parameter needed for non-positioned circle`);
  } else {
    if (origin !== void 0) {
      return toSvgFull2(a, origin);
    } else
      throw new Error(`origin parameter needed`);
  }
};
var toSvgFull2 = (radius, origin) => {
  const { x, y } = origin;
  return `
    M ${x}, ${y}
    m -${radius}, 0
...
//# sourceMappingURL=chunk-Z7YEWZE7.js.map