...
var AdsrBase = class extends SimpleEventEmitter {
  ...
  computeRaw() {
    if (__privateGet(this, _timer) === void 0)
      return [void 0, 0];
    const changedState = this.switchState();
    ...
    switch (state) {
      ...
      case `sustain`:
        relative = 1;
        this.releasedAt = this.releasedAt;
        break;
      case `release`:
  ...
  release() {
    ...
    const [stage, amt] = this.computeRaw();
    console.log(`release! ${stage} ... ${amt}`);
  }
};
_sm = new WeakMap();
  ...
  compute() {
    ...
    switch (stage) {
      case `attack`:
        ...
        break;
      case `decay`:
        console.log(`Released at ${this.releasedAt}`);
...
//# sourceMappingURL=chunk-DABI54IO.js.map