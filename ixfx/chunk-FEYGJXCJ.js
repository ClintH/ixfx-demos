import {
  ...
  isLine,
  isQuadraticBezier
} from "./chunk-LZKS3E7K.js";
import {
  stack
} from "./chunk-UUUGK466.js";
import {
  lerp,
  startsEnds
...
var wrappedRange = (a, b, min, max2, fn) => {
  const distF = Math.abs(b - a);
  const distFwrap = Math.abs(max2 - a + b);
  const distBWrap = Math.abs(a + (360 - b));
  console.log(`distF: ${distF} distFwrap ${distFwrap} distBwrap ${distBWrap}`);
...
var lerp3 = (amount, a, b) => {
  number(amount, `percentage`, `amount`);
  const h = wrappedRange(a.h, b.h, 0, 360, (max2) => lerp(amount, 0, max2));
  console.log(`hue: ${h} amt: ${amount}`);
...
var lerp_ = (amount, a, b) => {
  ...
  console.log(`amt: ${amount} a: ${a.h} (${ha}) b: ${b.h} (${hb}) distF: ${distForward} distB: ${distBackward}`);
  let h = lerp(amount, ha, hb) - incrBy;
  console.log(`  hue pre: ${h}`);
  if (h > 360)
    h = h % 360;
  else if (h < 0) {
    h = 360 + h + ha;
