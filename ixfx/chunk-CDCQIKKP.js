...
var grid = (parent, center, spacing, width, height, opts = {}) => {
  if (!opts.strokeStyle)
    opts = { ...opts, strokeStyle: Palette_exports.getCssVariable(`bg-dim`, `whitesmoke`) };
  if (!opts.strokeWidth)
    opts = { ...opts, strokeWidth: 1 };
  let y = 0;
  while (y < height) {
    let horiz = fromNumbers(0, y, width, y);
    lineEl(horiz, parent, opts);
    y += spacing;
  }
  let x = 0;
  while (x < width) {
    let vert = fromNumbers(x, 0, x, height);
    lineEl(vert, parent, opts);
    x += spacing;
  }
};
