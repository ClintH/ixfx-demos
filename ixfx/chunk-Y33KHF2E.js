...
var defaultAxis = (name) => ({
  colour: Colour_exports.getCssVariable(`fg`),
  endWith: `none`,
  lineWidth: 1,
  namePosition: "none",
  name,
...
var drawYSeriesScale = (series, plotSize, drawing) => {
  const { ctx, palette, y, margin } = drawing;
  ...
  ctx.textBaseline = `top`;
  ctx.fillText(min.toFixed(2), 0, calcYForValue(min, series, height));
  ctx.fillText(mid.toFixed(2), 0, calcYForValue(mid, series, height));
  ctx.fillText(max.toFixed(2), 0, calcYForValue(max, series, height));
...
var drawSeriesData = (series, values, plotSize, drawing, leadingEdgeIndex) => {
  const { ctx, dataXScale = 1, lineWidth = 2, translucentPlot = false, margin } = drawing;
  ...
  for (let i = 0; i < values.length; i += incrementBy) {
    let y = calcYForValue(values[i], series, height);
    if (style === `dots`) {
      ctx.beginPath();
