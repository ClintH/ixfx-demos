import {
  Easing_exports
} from "./chunk-6WBLOZSQ.js";
import {
  frequencyTimer,
  msElapsedTimer
} from "./chunk-ZIGO2W3X.js";
import {
  StateMachine
...
// src/modulation/index.ts
var jitter = (value, jitter2, type = `abs`) => {
  number(value, `percentage`, `value`);
  number(jitter2, `percentage`, `jitter`);
  if (type === `rel`) {
    jitter2 = value * jitter2;
    const j = jitter2 * 2 * Math.random();
    return clamp(value - jitter2 + j);
  } else if (type === `abs`) {
    const j = jitter2 * 2 * Math.random();
    return clamp(value - jitter2 + j);
  } else {
...
//# sourceMappingURL=chunk-MG555ARJ.js.map