 ...
 * @returns
 */
declare const debounce: (callback: () => void | Promise<any>, timeoutMs: number) => DebouncedFunction;
/**
 * Debounced function
 * @private
 */
declare type DebouncedFunction = (...args: unknown[]) => void;
/***
 ...
 * ```
 */
declare const throttle: (callback: (elapsedMs: number, ...args: readonly unknown[]) => void | Promise<any>, intervalMinMs: number) => (...args: unknown[]) => Promise<void>;
/**
 ...
 * ```
 *
 * By default, if `fn` throws an exception, the cached value is cleared, and it will be invoked again on next attempt.
 * If `errorsReset` is false and an exception happens, it will wait for at least `intervalMs` before trying again.
 * @param fn Async function to call. Must return a value.
 * @param intervalMs Maximum age of cached result
 * @param errorsReset If true (default), an exception from `fn` resets timer & cached value.
 * @returns
 */
declare const updateOutdated: <V>(fn: (elapsedMs?: number | undefined) => Promise<V>, intervalMs: number, errorsReset?: boolean) => () => Promise<V>;

export { Continuously as C, DebouncedFunction as D, HasCompletion as H, ModTimer as M, TimerSource as T, Timer as a, Timeout as b, TimeoutSyncCallback as c, debounce as d, TimeoutAsyncCallback as e, timeout as f, ContinuouslySyncCallback as g, ContinuouslyAsyncCallback as h, interval as i, continuously as j, delay as k, frequencyTimerSource as l, frequencyTimer as m, msElapsedTimer as n, ticksElapsedTimer as o, relativeTimer as r, sleep as s, throttle as t, updateOutdated as u };
