...
var draw = (buffer, drawing) => {
  const { fixedRange, ctx, yLabelWidth: yLabelWidth2, canvasSize } = drawing;
  ...
  if (showYAxis) {
    series.forEach((s) => {
      ...
      drawSeriesAxis(s, plotSize.height + margin + margin, drawing);
      ctx.translate(yLabelWidth2 + 3, 0);
    });
  }
  ctx.translate(margin, margin);
  const plotDrawing = {
  ...
  series.forEach((s) => {
    ...
    if (data2 === void 0)
      return;
    drawSeries(s, data2, plotSize, plotDrawing);
  });
  ctx.resetTransform();
};
var drawSeriesAxis = (series, height, drawing) => {
  const { ctx, palette } = drawing;
...
var drawSeries = (series, values, plotSize, drawing) => {
...
var plotSizeFromBounds = (bounds, opts) => {
  ...
  const yLabelWidth2 = opts.yLabelWidth;
  return {
    width: width - 4 * margin - (showYAxis ? yLabelWidth2 : 0),
...
var canvasSizeFromPlot = (plot2, opts) => {
  const showYAxis = opts.showYAxis;
  const margin = opts.margin;
  const width = plot2.width;
  const height = plot2.height;
  const yLabelWidth2 = opts.yLabelWidth;
  return {
    width: width + margin + margin + margin + margin + (showYAxis ? yLabelWidth2 : 0),
