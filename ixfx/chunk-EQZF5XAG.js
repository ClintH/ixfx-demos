...
var movingAverageLight = (scaling = 3) => {
  ...
  const ma = {
    ...
    add(v) {
      ...
      count++;
      average4 = average4 + (v - average4) / Math.min(count, scaling);
      return average4;
    },
    clear() {
      if (disposed)
...
function flatten2(flattener) {
  async function* flatten3(input) {
    input = resolveToGen2(input);
    for await (const value of input) {
      yield flattener(value);
    }
  }
  flatten3._name = `flatten`;
  return flatten3;
}
function duration2(elapsed) {
  const durationMs = intervalToMs(elapsed, 0);
  async function* duration3(input) {
    input = resolveToGen2(input);
...
function drop2(predicate) {
  async function* drop3(input) {
    input = resolveToGen2(input);
    for await (const value of input) {
      if (!predicate(value)) {
        yield value;
...
async function* mergeAsArray(...sources) {
  const sourcesInput = sources.map((source) => resolveToGen2(source));
