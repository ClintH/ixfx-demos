...
var lineThroughPoints = (ctx, points) => {
  ctx.moveTo(points[0].x, points[0].y);
  points.forEach((p, index) => {
    if (index + 2 >= points.length)
      return;
    const pNext = points[index + 1];
    var x_mid = (p.x + pNext.x) / 2;
    var y_mid = (p.y + pNext.y) / 2;
    var cp_x1 = (x_mid + p.x) / 2;
    var cp_x2 = (x_mid + pNext.x) / 2;
    ctx.quadraticCurveTo(cp_x1, pNext.y, x_mid, y_mid);
    ctx.quadraticCurveTo(cp_x2, pNext.y, pNext.x, pNext.y);
  });
};
