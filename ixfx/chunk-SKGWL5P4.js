...
var draw = (buffer, drawing) => {
  ...
  series.forEach((s) => {
    ...
    ctx.save();
    ctx.translate(0, drawing.margin);
...
var drawYSeriesScale = (series, plotSize, drawing) => {
  ...
  ctx.textBaseline = `top`;
  ctx.fillText(series.min.toFixed(2), 0, height - drawing.textHeight + margin);
  ctx.fillText(series.max.toFixed(2), 0, 0);
  ctx.fillText(mid.toFixed(2), 0, height / 2 - halfHeight);
  ctx.translate(y.textSize + drawing.margin, 0);
};
var drawYLine = (plotSize, drawing) => {
  ...
  ctx.lineTo(0, height + drawing.margin);
