  ...
  switch (policy) {
    ...
    case `newer`:
      if (toRemove >= queue2.length) {
        ...
        debug(opts, `trimQueue: toRemove: ${toRemove} from orig: ${queue2.slice(0, queue2.length - toRemove)} orig: ${JSON.stringify(queue2)}`);
...
var enqueue = (opts, queue2, ...toAdd) => {
  ...
  const potentialLength = queue2.length + toAdd.length;
  const overSize = opts.capacity && potentialLength > opts.capacity;
  console.log(`potentialLen: ${potentialLength} overSize: ${overSize}`);
...
//# sourceMappingURL=chunk-MPTQPX6Q.js.map