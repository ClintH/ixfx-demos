...
var normalise = (line, width, height) => Object.freeze({
  ...line,
  a: Point_exports.normalise(line.a, width, height),
  b: Point_exports.normalise(line.b, width, height)
});
...
var nearest = (line, point3) => {
  ...
  return Object.freeze({ x: a.x + atob.x * t2, y: a.y + atob.y * t2 });
};
...
var rotate2 = (pt, amountRadian, origin) => {
  if (origin === void 0)
    origin = { x: 0, y: 0 };
  guard(origin, `origin`);
  guard(pt, `pt`);
  number(amountRadian, ``, `amountRadian`);
  const p = Polar_exports.fromCartesian(pt, origin);
  const pp = Polar_exports.rotate(p, amountRadian);
  return Polar_exports.toCartesian(pp, origin);
};
function normalise2(a, b, c, d) {
  if (isPoint(a)) {
...
var isEmpty = (rect) => rect.width === 0 && rect.height === 0;
var isPlaceholder = (rect) => Number.isNaN(rect.width) && Number.isNaN(rect.height);
...
//# sourceMappingURL=chunk-HBMIRPMH.js.map