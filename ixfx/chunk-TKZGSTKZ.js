...
var getOrCreateDef = (parent, id, creator) => {
  const created = creator();
  if (created === void 0)
    throw new Error(`Could not create def ${id}`);
  return created;
};
var markerPrebuilt = (elem, id) => {
  if (elem === null)
    return `(elem null)`;
  ...
  let defEl = defsEl.querySelector(`#${id}`);
  if (defEl !== null)
    return `url(${id})`;
  console.log(`markerPrebuilt creating: ${id}`);
  if (id === `triangle`) {
    defEl = createMarker(id, { markerWidth: 6, markerHeight: 6, refX: 5, refY: 5, viewBox: `0 0 10 10` }, () => {
      console.log(`Creating triangle`);
      const tri = createEl(`path`);
      tri.setAttribute(`d`, `M 0 0 L 10 5 L 0 10 z`);
      applyOpts(tri, { strokeStyle: `transparent` });
      return tri;
    });
  } else
    throw new Error(`Do not know how to make ${id}`);
...
var applyPathOpts = (elem, opts) => {
  if (opts.markerEnd) {
    elem.setAttribute(`marker-end`, markerPrebuilt(elem, opts.markerEnd));
  }
  if (opts.markerStart)
...
var textEl = (pos, text, parent, opts, queryOrExisting) => {
...
var svg = (parent, opts) => {
  ...
  const o = {
    text: (pos, text, opts2, queryOrExisting) => textEl(pos, text, parent, opts2, queryOrExisting),
    line: (line, opts2, queryOrExisting) => lineEl(line, parent, opts2, queryOrExisting),
    circle: (circle, opts2, queryOrExisting) => circleEl(circle, parent, opts2, queryOrExisting),
...
//# sourceMappingURL=chunk-TKZGSTKZ.js.map