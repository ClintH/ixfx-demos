...
var getOrCreateDef = (parent, id, creator) => {
  ...
  const defEl = defsEl.querySelector(`#${id}`);
  if (defEl !== null)
    return defEl;
  ...
  created.id = id;
  defsEl.appendChild(created);
  return created;
};
var markerPrebuilt = (elem, id) => {
  if (elem === null)
    return;
  const m = getOrCreateDef(elem, id, () => {
    console.log(`Marker not found: ${id}`);
    if (id === `triangle`) {
      return createMarker(id, { markerWidth: 6 }, () => {
...
//# sourceMappingURL=chunk-ZU5UNXHX.js.map