import {
  Drawing_exports
} from "./chunk-DWOXNUWE.js";
import {
  mapCircular
} from "./chunk-AAKYT6K5.js";
import {
  minMaxAvg,
  randomElement
...
var pathEl = (svgOrArray, parent, opts) => {
...
var circleEl = (circle, parent, opts, idOrExisting) => {
  const p = createOrResolve(parent, `circle`, idOrExisting);
  circleUpdate(p, circle, opts);
  return p;
};
var createOrResolve = (parent, type, idOrExisting) => {
  let existing = null;
  if (idOrExisting !== void 0) {
    if (typeof idOrExisting === `string`)
      existing = parent.querySelector(`#${idOrExisting}`);
    else
      existing = idOrExisting;
  }
  if (existing === null) {
    const p = document.createElementNS(`http://www.w3.org/2000/svg`, type);
    parent.appendChild(p);
    if (idOrExisting && typeof idOrExisting === `string`)
      p.id = idOrExisting;
    return p;
  }
  return existing;
};
var lineEl = (line, parent, opts, idOrExisting) => {
  const lineEl2 = createOrResolve(parent, `line`, idOrExisting);
...
var textPathUpdate = (el, text, opts) => {
  ...
  if (opts?.textLength)
    el.setAttributeNS(null, `textLength`, numOrPerctange(opts.textLength));
  if (text) {
    el.textContent = text;
  }
};
var textPathEl = (pathRef, text, parent, opts, idOrExisting) => {
  const textEl2 = createOrResolve(parent, `text`, idOrExisting + `-text`);
  textElUpdate(textEl2, void 0, void 0, opts);
  const p = createOrResolve(textEl2, `textPath`, idOrExisting);
...
var textEl = (pos, text, parent, opts, idOrExisting) => {
  const p = createOrResolve(parent, `text`, idOrExisting);
...
var markerPrebuilt = (elem, name) => {
  if (elem === null)
    return;
  debugger;
  const m = elem.querySelector(`#name`);
  if (m === null) {
    console.log(`Marker not found: ${name}`);
  } else {
    console.log(`Marker found ${name}`);
  }
};
var applyPathOpts = (elem, opts) => {
  if (opts.markerEnd) {
    markerPrebuilt(elem.parentElement, opts.markerEnd);
    elem.setAttribute(`marker-end`, opts.markerEnd);
  }
  if (opts.markerStart)
    elem.setAttribute(`marker-end`, opts.markerStart);
  if (opts.markerMid)
    elem.setAttribute(`marker-end`, opts.markerMid);
};
...
var svg = (parent, opts) => {
  if (opts)
    applyOpts(parent, opts);
  const o = {
    text: (pos, text, opts2, idOrExisting) => textEl(pos, text, parent, opts2, idOrExisting),
    line: (line, opts2, idOrExisting) => lineEl(line, parent, opts2, idOrExisting),
    circle: (circle, opts2, idOrExisting) => circleEl(circle, parent, opts2, idOrExisting),
    path: (svgStr, opts2) => pathEl(svgStr, parent, opts2),
...
//# sourceMappingURL=chunk-GB4WRBXL.js.map