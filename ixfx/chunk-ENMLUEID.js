...
var StringWriteBuffer = class {
  ...
  async onWrite() {
    console.log(`StringWriteBuffer.onWrite`);
    ...
    console.log(`StringWriteBuffer.onWrite pre dequeue`);
    const s = this.queue.dequeue();
    console.log(`StringWriteBuffer.onWrite s: ${s}`);
    if (s === void 0)
      return false;
    await this.onData(s);
    return true;
  ...
  add(str) {
    console.log(`StringWriteBuffer add: ${str} chunk: ${this.chunkSize}`);
    if (this.chunkSize > 0) {
      this.queue.enqueue(...splitByLength(str, this.chunkSize));
    } else {
      this.queue.enqueue(str);
    }
    console.log(`StringWriteBuffer add length: ${this.queue.length} isEmpty: ${this.queue.isEmpty}`);
    this.writer.start();
  }
};
...
var Device = class extends JsonDevice {
  ...
  onDisconnect() {
    ...
    this.states.state = `disconnected`;
  }
  onPreConnect() {
  ...
  async onConnectAttempt() {
    ...
    if (this.config.filters)
      opts = { filters: [...this.config.filters] };
    this.port = await navigator.serial.requestPort(opts);
    this.port.addEventListener(`connect`, (_) => {
      this.log(`port connect`);
    });
    this.port.addEventListener(`disconnect`, (_) => {
      this.log(`port disconnect`);
    });
    this.log(`onConnectAttempt`);
    const tx = this.port.writable;
    const txText = new TextEncoderStream();
    if (tx !== null) {
      this.log(`Setting up`);
      txText.readable.pipeTo(tx);
...
//# sourceMappingURL=chunk-ENMLUEID.js.map