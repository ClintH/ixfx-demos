...
var compareData = (a, b, options = {}) => {
  ...
  if (b === void 0)
    return [{ path: options.pathPrefix ?? ``, previous: a, value: void 0 }];
  ...
  if (isPrimitive(a) && isPrimitive(b)) {
    if (a !== b)
      changes.push({ path: options.pathPrefix ?? ``, value: b, previous: a });
    return changes;
  }
  ...
  for (const [key, valueA] of entriesA) {
    ...
    if (typeof valueA === `object`) {
      ...
      if (key in b) {
        ...
        changes.push({ path: sub, previous: valueA, value: void 0 });
      }
    }
  }
  if (includeMissingFromA) {
    ...
    for (const [key, valueB] of entriesB) {
      if (entriesAKeys.has(key))
        continue;
      changes.push({ path: pathPrefix + key, previous: void 0, value: valueB });
    }
  }
  return changes;
};
...
var updateByPathImpl = (o, split, value, allowShapeChange) => {
  if (split.length === 0) {
    if (allowShapeChange)
      return value;
  ...
  if (isInt && Array.isArray(o)) {
    ...
    if (index >= o.length && !allowShapeChange)
      throw new Error(`Array index ${index} is outside of the existing length of ${o.length}. Use allowShapeChange=true to permit this.`);
    const copy = [...o];
...
var getField = (object, path) => {
  if (typeof path !== `string`)
    throw new Error(`Parameter 'path' ought to be a string. Got: '${typeof path}'`);
  if (path.length === 0)
...
var getPathsAndData = (o, maxDepth = Number.MAX_SAFE_INTEGER, prefix = ``) => {
  if (o === null)
    return [];
  if (o === void 0)
    return [];
...
var getPathsAndDataImpl = (o, prefix, result, maxDepth) => {
  if (maxDepth <= 0)
    return result;
  if (typeof o === `object`) {
...
//# sourceMappingURL=chunk-UG5VC4V5.js.map