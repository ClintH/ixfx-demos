...
var require_ReadonlyNonEmptyArray = __commonJS({
  "node_modules/fp-ts/lib/ReadonlyNonEmptyArray.js"(exports) {
    ...
    var union = function(E2) {
      ...
      return function(second) {
        return function(first2) {
          return uniqE((0, function_1.pipe)(first2, concat2(second)));
        };
      };
    };
    exports.union = union;
    ...
    var range2 = function(start3, end) {
    ...
    var snoc = function(init6, end) {
      return (0, function_1.pipe)(init6, concat2([end]));
    };
    exports.snoc = snoc;
...
var require_string = __commonJS({
  "node_modules/fp-ts/lib/string.js"(exports) {
    ...
    var slice2 = function(start3, end) {
      return function(s) {
  ...
  "node_modules/json5/dist/index.js"(exports, module) {
    ...
    (function(global, factory) {
      ...
      var _toAbsoluteIndex = function(index, length6) {
        ...
        return index < 0 ? max7(index + length6, 0) : min7(index, length6);
      };
...
var firstEntryByIterableValue = (map3, value, isEqual8 = isEqualDefault) => {
  for (const entry of map3.entries()) {
    if (isEqual8(entry[1], value))
      return entry;
  }
};
...
var find = (map3, predicate) => Array.from(map3.values()).find(predicate);
...
// src/data/Wrap.ts
var wrapInteger = (v, min7 = 0, max7 = 360) => {
  throwIntegerTest(v, void 0, `v`);
  throwIntegerTest(min7, void 0, `min`);
  throwIntegerTest(max7, void 0, `max`);
  if (v === min7)
    return min7;
  if (v === max7)
    return min7;
  ...
  v = v % max7;
  if (v < 0)
    v = max7 - Math.abs(v) + min7;
  return v + min7;
};
var wrap = (v, min7 = 0, max7 = 1) => {
  throwNumberTest(v, ``, `min`);
  throwNumberTest(min7, ``, `min`);
  throwNumberTest(max7, ``, `max`);
  if (v === min7)
    return min7;
  if (v === max7)
    return min7;
...
var wrapRange = (min7, max7, fn, a, b) => {
  let r = 0;
  const distF = Math.abs(b - a);
  const distFwrap = Math.abs(max7 - a + b);
  ...
  return wrapInteger(r, min7, max7);
};
...
var interpolateAngle = (amount, aRadians, bRadians) => {
  const t4 = wrap(bRadians - aRadians, 0, piPi);
  return interpolate(amount, aRadians, aRadians + (t4 > Math.PI ? t4 - piPi : t4));
};
...
function mutable2(opts = {}, ...startingItems) {
...
var mutable3 = (opts = {}, ...startingItems) => new StackMutable({ ...opts }, [...startingItems]);
...
var clampMagnitude = (v, max7 = 1, min7 = 0) => {
  let mag = v.distance;
  if (mag > max7)
    mag = max7;
  if (mag < min7)
...
var immutable2 = (opts = {}, ...startingItems) => {
  opts = { ...opts };
...
var immutable3 = (dataOrMap) => {
  ...
  if (Array.isArray(dataOrMap))
    return immutable3(add(/* @__PURE__ */ new Map(), ...dataOrMap));
...
`;
  ...
  deleteKeyValueFromMap(map3, key, value) {
    const preCount = this.type.count(map3);
    const filtered = this.type.without(map3, value);
...
var stream = (minDefault, maxDefault) => {
  let min7 = minDefault ?? Number.MAX_SAFE_INTEGER;
  let max7 = maxDefault ?? Number.MIN_SAFE_INTEGER;
...
var array = (values, minForced, maxForced) => {
  ...
  const min7 = minForced ?? mma.min;
  const max7 = maxForced ?? mma.max;
  return values.map((v) => clamp(scale(v, min7, max7)));
};
...
var retry = async (callback, opts) => {
  ...
  while (attempts < count2) {
    ...
    log2({
      msg: `retry attempts: ${attempts} t: ${toString3(t4)}`
    });
    if (attempts >= count2) {
      break;
    }
...
var interpolate3 = (circle3, t4) => point(circle3, t4 * piPi2);
...
var toPath2 = (circle3) => {
  ...
  return Object.freeze({
     ...
     * @returns {Point} X,y
     */
    interpolate: (t4) => interpolate3(circle3, t4),
    bbox: () => bbox2(circle3),
    length: () => length3(circle3),
...
function fromNumbers2(xOrWidth, yOrHeight, width, height4) {
...
var distanceFromCenter = (rect2, pt) => Point_exports.distance(center3(rect2), pt);
...
var normaliseByRect2 = (rect2, normaliseByOrWidth, height4) => {
  let width;
  if (height4 === void 0) {
...
var isEmpty6 = (t4) => isEmpty5(t4.a) && isEmpty5(t4.b) && isEmpty5(t4.c);
var isPlaceholder4 = (t4) => isPlaceholder3(t4.a) && isPlaceholder3(t4.b) && isPlaceholder3(t4.c);
...
var isEqual5 = (a, b) => isEqual4(a.a, b.a) && isEqual4(a.b, b.b) && isEqual4(a.c, b.c);
var corners2 = (t4) => {
  guard6(t4);
...
var fromFlatArray2 = (coords) => {
  ...
  return fromPoints2(fromNumbers3(...coords));
};
...
var utils = {
  ...
  getminmax: function(curve, d, list) {
    ...
    return { min: min7, mid: (min7 + max7) / 2, max: max7, size: max7 - min7 };
  },
...
function hslConvert(o) {
  ...
  if (o instanceof Hsl)
    return o;
  o = o.rgb();
  var r = o.r / 255, g = o.g / 255, b = o.b / 255, min7 = Math.min(r, g, b), max7 = Math.max(r, g, b), h = NaN, s = max7 - min7, l = (max7 + min7) / 2;
  if (s) {
    if (r === max7)
      h = (g - b) / s + (g < b) * 6;
    else if (g === max7)
...
var interpolate4 = (amount, from2, to2, optsOrSpace) => {
...
var Box = class {
  ...
  *getChildren() {
  ...
  measureSelf(opts, parent) {
    ...
    size = desired ? clamp2(desired, context2) : context2;
...
var updateByPath = (o, path2, value, createIfNecessary = false) => {
  if (path2 === void 0)
    throw new Error(`Parameter 'path' is undefined`);
  if (typeof path2 !== `string`)
    throw new Error(`Parameter 'path' should be a string. Got: ${typeof path2}`);
  if (o === void 0)
...
var updateByPathImpl = (o, split, value, createIfNecessary) => {
  if (split.length === 0)
    return value;
  const start3 = split.shift();
  if (!start3)
    return value;
  const arrayStart = start3.indexOf(`[`);
  const arrayEnd = start3.indexOf(`]`, arrayStart);
  if (arrayStart > 0 && arrayEnd > arrayStart) {
    const field2 = start3.slice(0, arrayStart);
    const index = start3.slice(arrayStart, arrayEnd + 1);
    split.unshift(index);
    const copy = { ...o };
    copy[field2] = updateByPathImpl(copy[field2], split, value, createIfNecessary);
    return copy;
  }
  if (start3.startsWith(`[`) && start3.endsWith(`]`) && Array.isArray(o)) {
    const index = Number.parseInt(start3.slice(1, -1));
    const copy = [...o];
    copy[index] = updateByPathImpl(copy[index], split, value, createIfNecessary);
    return copy;
  } else if (start3 in o) {
    const copy = { ...o };
    copy[start3] = updateByPathImpl(copy[start3], split, value, createIfNecessary);
    return copy;
  } else {
    throw new Error(`Path ${start3} not found in data`);
  }
};
var getField = (o, path2) => {
  if (typeof path2 !== `string`)
    throw new Error(`Path ought to be a string`);
  if (path2.length === 0)
  ...
  if (o === null)
    throw new Error(`Parameter 'o' is null`);
  const split = path2.split(`.`);
  const v = getFieldImpl(o, split);
  return v;
};
var getFieldImpl = (o, split) => {
  if (split.length === 0)
    throw new Error(`Path run out`);
  const start3 = split.shift();
  if (!start3)
    throw new Error(`Unexpected empty split`);
  const arrayStart = start3.indexOf(`[`);
  const arrayEnd = start3.indexOf(`]`, arrayStart);
  if (arrayStart > 0 && arrayEnd > arrayStart) {
    const field2 = start3.slice(0, arrayStart);
    const index = start3.slice(arrayStart, arrayEnd + 1);
    split.unshift(index);
    return getFieldImpl(o[field2], split);
  }
  if (arrayStart === 0 && arrayEnd === start3.length - 1 && Array.isArray(o)) {
    const index = Number.parseInt(start3.slice(1, -1));
    if (split.length === 0) {
      return o[index];
    } else {
    ...
    if (split.length === 0) {
      ...
      return getFieldImpl(o[start3], split);
    }
  } else {
    throw new Error(`Path ${start3} not found in data`);
  }
...
var getPaths = (o, onlyLeaves = false) => {
  ...
  const probe = (o2, initialPaths, prefix) => {
    ...
    const x = keys.map((key) => {
      const value = o2[key];
      const keyName = Array.isArray(o2) ? untilMatch(prefixToUse, `.`, { fromEnd: true }) + `[${key}]` : prefixToUse + key;
...
var getPathsAndDataImpl = (o, prefix, result) => {
  const isArray2 = Array.isArray(o);
  if (typeof o === `object`) {
    for (const entries of Object.entries(o)) {
      let sub = prefix + (!isArray2 && prefix.length > 0 ? `.` : ``);
      if (isArray2)
        sub += `[`;
      sub += entries[0];
      if (isArray2)
...
var toArray3 = (grid2, initialValue) => {
  const ret = [];
...
var array2dUpdater = (grid2, array2) => {
  const fn = (v, position) => {
    ...
    array2[pos.y][pos.x] = v;
  };
  return fn;
};
function* visitArray(array2, cols, iteratorFn, opts) {
...
var immutable4 = (startingValueOrBipolar = 0) => {
  ...
  return {
    ...
    value: v,
    towardZero: (amt) => {
      return immutable4(towardZero(v, amt));
    },
    add: (amt) => {
      return immutable4(clamp3(v + amt));
    },
    multiply: (amt) => {
      return immutable4(clamp3(v * amt));
    },
    ...
    interpolate: (amt, b) => {
      return immutable4(clamp3(interpolate(amt, v, b)));
    },
    asScalar: () => {
      return toScalar(v);
    }
...
var calcScale = (buffer, drawingOpts, seriesColours) => {
  ...
  for (const s of buffer.keys()) {
    ...
    if (series.length === 0)
      break;
    let { min: min7, max: max7 } = minMaxAvg(series);
    let range2 = max7 - min7;
    let colour;
...
var scaleWithFixedRange = (buffer, range2, drawing) => calcScale(buffer, drawing, drawing.seriesColours).map((s) => ({
  ...s,
...
var drawYSeriesScale = (series, plotSize, drawing) => {
  ...
  const min7 = y.labelRange ? y.labelRange[0] : series.min;
  const max7 = y.labelRange ? y.labelRange[1] : series.max;
  const range2 = y.labelRange ? max7 - min7 : series.range;
  const mid = min7 + range2 / 2;
  const halfHeight = drawing.textHeight / 2;
  ctx.textBaseline = `top`;
  ctx.fillText(
    min7.toFixed(digitsPrecision),
    0,
    calcYForValue(min7, series, height4) - halfHeight
  );
  ctx.fillText(
  ...
  ctx.fillText(
    max7.toFixed(digitsPrecision),
    0,
    calcYForValue(max7, series, height4) - margin
  );
...
var drawYLine = (plotSize, series, drawing) => {
  ...
  const min7 = y.labelRange ? y.labelRange[0] : series.min;
  const max7 = y.labelRange ? y.labelRange[1] : series.max;
  const minPos = calcYForValue(min7, series, height4);
  const maxPos = calcYForValue(max7, series, height4);
...
var PointTracker = class extends ObjectTracker {
  ...
  difference() {
    const initial = this.initial;
    return this.values.length >= 2 && initial !== void 0 ? subtract(this.last, initial) : Placeholder3;
  }
  /**
...
var DispatchList = class {
  ...
  remove(id) {
    const length6 = this.#handlers.length;
    this.#handlers = this.#handlers.filter((handler) => handler.id !== id);
    return this.#handlers.length !== length6;
  }
  notify(value) {
...
var clone = (graph3) => {
  const g = {
    vertices: immutable3([...graph3.vertices.entries()])
  };
  return g;
};
...
function resolveAfter(interval2, callbackOrValue, options = {}) {
  const intervalMs = intervalToMs(interval2, 0);
  const event2 = initEvent();
...
function object(initialValue, options = {}) {
  ...
  const updateField = (path2, valueForField) => {
    ...
    if (eq(existing, valueForField, path2))
      return;
    const o = updateByPath(value, path2, valueForField);
    value = o;
...
function lazy() {
  ...
  const w = {
    ...
    flatten: (flattener) => {
      chained.push(flatten2(flattener));
      return w;
    },
    drop: (predicate) => {
    ...
    filter: (predicate) => {
      chained.push(filter3(predicate));
      return w;
    },
    min: () => {
      chained.push(min4());
...
function flatten2(flattener) {
  async function* flatten4(input) {
    input = resolveToGen(input);
    for await (const value of input) {
      yield flattener(value);
    }
  }
  flatten4._name = `flatten`;
  return flatten4;
}
...
function min4() {
  async function* min7(input) {
    input = resolveToGen(input);
    let min8 = Number.MAX_SAFE_INTEGER;
    for await (const value of input) {
      if (typeof value !== `number`)
        break;
      min8 = Math.min(value, min8);
      yield min8;
    }
  }
  min7._name = `min`;
...
function max4() {
  async function* max7(input) {
    input = resolveToGen(input);
    let max8 = Number.MIN_SAFE_INTEGER;
    for await (const value of input) {
      if (typeof value !== `number`)
        break;
      max8 = Math.max(value, max8);
      yield max8;
    }
  }
  max7._name = `max`;
...
var isEmpty5 = (p) => p.x === 0 && p.y === 0;
var isPlaceholder3 = (p) => Number.isNaN(p.x) && Number.isNaN(p.y);
...
var interpolate5 = (amount, a, b, allowOverflow = false) => interpolate2(amount, a, b, allowOverflow);
var from = (xOrArray, y) => {
...
var apply3 = (pt, fn) => Object.freeze({
  ...pt,
  x: fn(pt.x, `x`),
  y: fn(pt.y, `y`)
});
...
var convexHull = (...pts) => {
  ...
  if (upper.length === 1 && lower.length === 1 && isEqual4(lower[0], upper[0])) {
    return upper;
  }
  return [...upper, ...lower];
};
...
var clampMagnitude2 = (pt, max7 = 1, min7 = 0) => {
  const length6 = distance2(pt);
  let ratio = 1;
  if (length6 > max7) {
    ratio = max7 / length6;
  } else if (length6 < min7) {
    ratio = min7 / length6;
  }
  return ratio === 1 ? pt : multiply4(pt, ratio, ratio);
};
function clamp4(a, b, c, d) {
  if (isPoint(a)) {
    if (b === void 0)
      b = 0;
    if (c === void 0)
...
var toString5 = (paths2) => paths2.map((p) => p.toString()).join(`, `);
...
var clampMagnitude3 = (v, max7 = 1, min7 = 0) => {
  if (isPolar(v)) {
    return Polar_exports.clampMagnitude(v, max7, min7);
  } else if (isCartesian(v)) {
    return clampMagnitude2(v, max7, min7);
  }
...
function convolveCell(c, kernel, source, access, reduce3) {
  ...
  return reduce3(valuesAtKernelPos);
}
...
function* convolve(kernel, source, access, visitor2, reduce3, origin) {
  if (!origin) {
  ...
  for (const c of visitor2) {
    const v = convolveCell(c, asArray2, source, access, reduce3);
    yield [c, v];
  }
}
...
function* circleVogelSpiral(circle3, opts = {}) {
  ...
  let count2 = 0;
  let angle2 = 0;
  while (count2 < maxPoints && radius < max7) {
...
function* sine(timerOrFreq) {
  ...
  while (true) {
    yield (Math.sin(timerOrFreq.elapsed * piPi9) + 1) / 2;
  }
}
function* sineBipolar(timerOrFreq) {
  ...
  while (true) {
    yield Math.sin(timerOrFreq.elapsed * piPi9);
  }
}
...
var applyToValues = (object2, apply5) => {
  const o = { ...object2 };
...
var quantiseEvery = (v, every2, middleRoundsUp = true) => {
  ...
  throwFromResult(integerTest(every2, ``, `every`));
  let div = v / every2;
...
var toString3 = (millisOrFunction, rounding = 2) => {
  let interval2 = {} = 0;
  if (typeof millisOrFunction === `function`) {
    const intervalResult = millisOrFunction();
    return toString3(intervalResult);
...
var valuesEqual = (array2, equality) => {
  if (!Array.isArray(array2))
...
var filterBetween = (array2, predicate, startIndex, endIndex) => {
  guardArray(array2);
...
var randomElement = (array2, rand = defaultRandom) => {
  guardArray(array2, `array`);
  return array2[Math.floor(rand() * array2.length)];
};
var randomPluck = (array2, mutate = false, rand = defaultRandom) => {
  if (array2 === void 0)
  ...
  const index = randomIndex(array2, rand);
  if (mutate) {
    return {
      value: array2[index],
...
var sample = (array2, amount) => {
  let subsampleSteps = 1;
  if (amount <= 1) {
    const numberOfItems = array2.length * amount;
    subsampleSteps = Math.round(array2.length / numberOfItems);
  } else {
    subsampleSteps = amount;
  }
  throwIntegerTest(subsampleSteps, `positive`, `amount`);
  if (subsampleSteps > array2.length - 1) {
...
var integerSource = (maxOrOptions) => {
  ...
  const options = typeof maxOrOptions === `number` ? { max: maxOrOptions } : maxOrOptions;
  let max7 = Math.floor(options.max);
  let min7 = Math.floor(options.min ?? 0);
  if (!options.min && max7 < 0) {
    max7 = 1;
    min7 = options.max;
  }
  ...
  throwFromResult(numberTest(min7, ``, `min`));
  throwFromResult(numberTest(max7, ``, `max`));
  if (max7 === min7) {
...
var floatSource = (maxOrOptions = 1) => {
  const options = typeof maxOrOptions === `number` ? { max: maxOrOptions } : maxOrOptions;
  let max7 = options.max;
  let min7 = options.min ?? 0;
  const source = options.source ?? defaultRandom;
  throwFromResult(numberTest(min7, ``, `min`));
  throwFromResult(numberTest(max7, ``, `max`));
  if (!options.min && max7 < 0) {
  ...
  if (min7 > max7) {
    throw new Error(`Min is greater than max. Min: ${min7} max: ${max7}`);
  }
  return () => source() * (max7 - min7) + min7;
};
...
var minutesMsSource = (maxMinutesOrOptions) => {
  ...
  const min7 = (options.min ?? 0) * 60 * 1e3;
  const max7 = options.max * 60 * 1e3;
  return integerSource({ ...options, max: max7, min: min7 });
};
...
var secondsMsSource = (maxSecondsOrOptions) => {
  ...
  const min7 = (options.min ?? 0) * 1e3;
  const max7 = options.max * 1e3;
  return () => integer({ ...options, max: max7, min: min7 });
};
...
function* integerUniqueGen(maxOrOptions) {
  ...
  throwFromResult(integerTest(min7, ``, `min`));
  throwFromResult(integerTest(max7, ``, `max`));
  if (min7 > max7) {
    throw new Error(`Min value is greater than max. Min: ${min7} Max: ${max7}`);
  }
  const origRange = [...range(min7, max7 - min7)];
...
var splitByLength = (source, length6) => {
  ...
  const chunks3 = Math.ceil(source.length / length6);
  const returnValue = [];
  let start3 = 0;
  for (let c = 0; c < chunks3; c++) {
