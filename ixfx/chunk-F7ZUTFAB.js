...
var multiply = (l, p) => ({
  a: Point_exports.multiply(l.a, p),
  b: Point_exports.multiply(l.b, p)
});
var withinRange = (l, p, maxRange) => {
...
function interpolate(amount, a, b) {
...
var toString = (a, b) => Point_exports.toString(a) + `-` + Point_exports.toString(b);
...
var rotate = (line, angleRad, origin) => {
  if (angleRad === void 0 || angleRad === 0)
    return line;
  if (origin === void 0)
    origin = 0.5;
  if (typeof origin === `number`) {
    percent(origin, `origin`);
    origin = interpolate(origin, line.a, line.b);
  }
  const pA = Polar_exports.fromCartesian(line.a, origin);
  const pB = Polar_exports.fromCartesian(line.b, origin);
  return {
    a: Polar_exports.toCartesian(Polar_exports.rotate(pA, angleRad)),
    b: Polar_exports.toCartesian(Polar_exports.rotate(pB, angleRad))
  };
};
...
var equals2 = (a, b) => a.x === b.x && a.y === b.y;
...
var length3 = (circle) => piPi2 * circle.radius;
var bbox4 = (circle) => {
  if (isPositioned2(circle)) {
...
var intersections = (a, b) => {
  ...
  return [
    ...
    Point_exports.subtract(centroid, intersection)
  ];
};
var isEquals2 = (a, b) => {
  if (a.radius !== b.radius)
...
var fromCartesian = (point3, origin) => {
  ...
  let a = Math.atan2(point3.y, point3.x);
  if (a < 0)
    a = Math.abs(a);
  else
    a = Math.PI + (Math.PI - a);
...
var rotate2 = (c, amountRadian) => ({
  ...c,
  angleRadian: c.angleRadian + amountRadian
});
var rotateDegrees = (c, amountDeg) => ({
  ...c,
...
// src/geometry/index.ts
var degreeToRadian = (angleInDegrees) => angleInDegrees * (Math.PI / 180);
...
//# sourceMappingURL=chunk-F7ZUTFAB.js.map