  ...
  "node_modules/json5/dist/index.js"(exports, module) {
    (function(global, factory) {
      ...
      var _toAbsoluteIndex = function(index, length6) {
        ...
        return index < 0 ? max4(index + length6, 0) : min4(index, length6);
      };
...
__export(Random_exports, {
  ...
  generateIntegerUnique: () => generateIntegerUnique,
  hue: () => randomHue,
  integer: () => integer2,
  minutesMs: () => minutesMs,
  secondsMs: () => secondsMs,
...
var splitByLength = (source, length6) => {
  ...
  const chunks2 = Math.ceil(source.length / length6);
  const ret = [];
  let start3 = 0;
...
var repeatReduce = (countOrPredicate, fn, initial, reduce2) => {
  if (typeof countOrPredicate === `number`) {
    number(countOrPredicate, `positive`, `countOrPredicate`);
    while (countOrPredicate-- > 0) {
      const v = fn();
...
var scaleWithFixedRange = (buffer, range2, drawing) => calcScale(buffer, drawing, drawing.seriesColours).map((s) => ({ ...s, range: range2[1] - range2[0], min: range2[0], max: range2[1] }));
...
var drawYSeriesScale = (series, plotSize, drawing) => {
  ...
  const range2 = y.labelRange ? max4 - min4 : series.range;
  const mid = min4 + range2 / 2;
...
var weighted = (easingNameOrOpts = "quadIn") => {
...
var weightedSkewed = (easingNameOrOpts = `quadIn`) => () => weighted(easingNameOrOpts);
var weightedInteger = (maxOrOpts) => {
...
var integer2 = (maxOrOpts) => {
  let reverse = false;
  if (typeof maxOrOpts === `undefined`)
    throw new Error(`maxOrOpts is undefined`);
  const opts = typeof maxOrOpts === "number" ? { max: maxOrOpts } : maxOrOpts;
  let max4 = Math.floor(opts.max);
  let min4 = Math.floor(opts.min ?? 0);
  if (!opts.min && max4 < 0) {
    max4 = 1;
    min4 = opts.max;
  }
  ...
  if (min4 > max4)
    throw new Error(`Min value is greater than max (min: ${min4} max: ${max4})`);
  number(min4, "", "min");
  number(max4, "", "max");
  if (max4 === min4)
    throw new Error(`Max and min values cannot be the same (${max4})`);
  const amt = Math.abs(max4 - min4);
  const r = Math.floor(randomSource() * amt) + min4;
  return r;
};
var float = (maxOrOpts = 1) => {
  const opts = typeof maxOrOpts === "number" ? { max: maxOrOpts } : maxOrOpts;
  let max4 = opts.max;
  let min4 = opts.min ?? 0;
  const source = opts.source ?? defaultRandom;
  ...
  if (min4 > max4)
    throw new Error(`Min is greater than max. Min: ${min4} max: ${max4}`);
  return source() * (max4 - min4) + min4;
};
...
var minutesMs = (maxMinutesOrOpts) => {
  const opts = typeof maxMinutesOrOpts === "number" ? { max: maxMinutesOrOpts } : maxMinutesOrOpts;
  const min4 = (opts.min ?? 0) * 60 * 1e3;
  const max4 = opts.max * 60 * 1e3;
  return integer2({ ...opts, max: max4, min: min4 });
};
var secondsMs = (maxSecondsOrOpts) => {
  const opts = typeof maxSecondsOrOpts === "number" ? { max: maxSecondsOrOpts } : maxSecondsOrOpts;
  const min4 = (opts.min ?? 0) * 1e3;
  const max4 = opts.max * 1e3;
  return integer2({ ...opts, max: max4, min: min4 });
};
function* generateIntegerUnique(maxOrOpts) {
  const opts = typeof maxOrOpts === "number" ? { max: maxOrOpts } : maxOrOpts;
  let min4 = opts.min ?? 0;
  let max4 = opts.max;
  const source = opts.source ?? defaultRandom;
  ...
  if (min4 > max4)
    throw new Error(`Min value is greater than max. Min: ${min4} Max: ${max4}`);
  const origRange = [...range(min4, max4 - min4)];
...
tslib/tslib.es6.js:
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=chunk-RBLY62Q2.js.map