import {
  Set_exports,
  setMutable
} from "./chunk-Q4PS5WQA.js";
import {
  StateMachine,
  StateMachine_exports
} from "./chunk-XDHTGB6U.js";
import {
...
var clampIndex = (v, arrayOrLength) => {
  ...
  if (!Number.isInteger(length5))
    throw new Error(`length parameter must be an integer (${length5}, ${typeof length5})`);
  v = Math.round(v);
  if (v < 0)
    return 0;
  if (v >= length5)
...
function* bfs(root) {
  ...
  while (!queue2.isEmpty) {
    ...
    yield node;
    if (node) {
      queue2.enqueue(...node.descendants);
    }
  }
}
function* dfs(root) {
...
var _subdivide, subdivide_fn;
var _QuadTreeNode = class extends TreeNode {
  /**
   * Constructor
   * @param boundary 
   * @param level 
   * @param opts 
   */
  constructor(boundary, level, opts) {
    super(void 0);
  ...
  add(p) {
    ...
    if (this.descendants.length) {
      this.descendants.forEach((d) => d.add(p));
      return true;
    }
    this.items.push(p);
    if (this.items.length > this.opts.maxItems && this.level < this.opts.maxLevels) {
      ...
      this.items.forEach((item) => {
        this.descendants.forEach((d) => d.add(item));
      });
      this.items = [];
    }
    return true;
...
subdivide_fn = function() {
  ...
  rects.forEach((r, index) => {
    this.descendants[index] = new _QuadTreeNode(r, this.level + 1, this.opts);
  });
};
...
var minutesMs = (maxMinutes, minMinutes = 0) => integer2(maxMinutes * 60 * 1e3, minMinutes * 60 * 1e3);
var secondsMs = (maxSeconds, minSeconds = 0) => integer2(maxSeconds * 1e3, minSeconds * 1e3);
...
tslib/tslib.es6.js:
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=chunk-YJG2IBIZ.js.map