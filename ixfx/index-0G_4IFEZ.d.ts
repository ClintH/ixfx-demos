...
import { N as NumberFunction, P as PointTrackerResults, b as PointTrack, c as RankArrayOptions, R as RankFunction, a as RankOptions } from './Types-0RfFkd3L.js';
...
import { S as SortSyles } from './KeyValue-V1moTY2N.js';
import { K as KeyValue, a as PrimitiveOrObject } from './PrimitiveTypes-HWqXs_XP.js';
...
import { R as Reactive, P as Passed, U as Unsubscriber, a as Process, b as ProcessFactory, c as Processors, d as Processors1, e as Processors2, f as Processors3, g as Processors4, h as Processors5, p as processChain } from './Types-8cDVJ8Ha.js';
import { C as Change } from './Immutable-ZCanoHO-.js';
import { a as IMapImmutable } from './Map-Pf0nY9XK.js';
 ...
 * @returns Bipolar value on -1..1 scale
 */
declare const scaleClamped$1: (inputValue: number, inMin: number, inMax: number) => number;
/**
...
declare namespace Bipolar {
  export { type Bipolar_BipolarWrapper as BipolarWrapper, clamp$1 as clamp, Bipolar_fromScalar as fromScalar, Bipolar_immutable as immutable, Bipolar_random as random, Bipolar_randomSource as randomSource, scale$1 as scale, scaleClamped$1 as scaleClamped, Bipolar_toScalar as toScalar, Bipolar_towardZero as towardZero };
}
...
type Resolvable<V> = Promise<V> | Reactive<V> | Generator<V> | AsyncGenerator<V> | IterableIterator<V> | AsyncIterableIterator<V> | ((args: any) => V);
type PullRecord<T extends Record<string, PrimitiveOrObject | (() => any) | Reactive<any>>> = {
    [K in keyof T]: T[K] extends number ? number | undefined : T[K] extends string ? string | undefined : T[K] extends boolean ? boolean | undefined : T[K] extends bigint ? bigint | undefined : T[K] extends () => Promise<any> ? Awaited<ReturnType<T[K]>> : T[K] extends () => any ? ReturnType<T[K]> : T[K] extends Reactive<infer V> ? V | undefined : T[K] extends Generator<infer V> ? V | undefined : T[K] extends AsyncGenerator<infer V> ? V | undefined : T[K] extends IterableIterator<infer V> ? V | undefined : T[K] extends AsyncIterableIterator<infer V> ? V | undefined : T[K] extends Array<infer V> ? V | undefined : T[K] extends object ? T[K] : never;
};
type PullRecordWithInitial<T extends Record<string, PrimitiveOrObject | (() => any) | Reactive<any>>> = {
    [K in keyof T]: T[K] extends number ? number : T[K] extends string ? string : T[K] extends boolean ? boolean : T[K] extends bigint ? bigint : T[K] extends () => Promise<any> ? Awaited<ReturnType<T[K]>> : T[K] extends () => any ? ReturnType<T[K]> : T[K] extends Reactive<infer V> ? V : T[K] extends Generator<infer V> ? V : T[K] extends AsyncGenerator<infer V> ? V : T[K] extends IterableIterator<infer V> ? V : T[K] extends AsyncIterableIterator<infer V> ? V : T[K] extends Array<infer V> ? V : T[K] extends object ? T[K] : never;
};
type ResolveValue<V> = V extends () => Promise<any> ? Awaited<ReturnType<V>> : V extends () => any ? ReturnType<V> : V extends Iterator<object> ? object : V extends Iterator<number> ? number : V extends Iterator<string> ? string : V extends Iterator<boolean> ? boolean : V extends AsyncIterator<object> ? object : V extends AsyncIterator<number> ? number : V extends AsyncIterator<string> ? string : V extends AsyncIterator<boolean> ? boolean : V extends ValueType ? V : never;
type ResolvedObject<V> = Readonly<{
 ...
 * @returns
 */
declare function fieldResolve<V extends object>(object: V): Promise<ResolvedObject<V>>;
/**
 ...
 * @returns
 */
declare function fieldResolver<V extends object>(object: V): () => Promise<Readonly<{ [key in keyof V]: ResolveValue<V[key]>; }>>;
 ...
 * @returns
 * */
declare function reactiveUpdate<S extends Record<string, PrimitiveOrObject>>(schema: S, updaters?: Updated<S>): {
    pull: () => Promise<S>;
    on(handler: (value: Passed<S>) => void): Unsubscriber;
    onValue(handler: (value: S) => void): Unsubscriber;
    dispose(reason: string): void;
    isDisposed(): boolean;
    set(value: S): void;
    onField(fieldName: string, handler: (value: any, fieldName: string) => void): () => void;
    onDiff(handler: (changes: Passed<Change<any>[]>) => void): () => void;
    update(changedPart: Record<string, any>): S;
    updateField(field: string, value: any): void;
    last(): S;
};
declare function pull<T extends Record<string, PrimitiveOrObject | Resolvable<any>>>(value: T): {
...
declare const index_ProcessFactory: typeof ProcessFactory;
declare const index_Processors: typeof Processors;
declare const index_Processors1: typeof Processors1;
declare const index_Processors2: typeof Processors2;
declare const index_Processors3: typeof Processors3;
declare const index_Processors4: typeof Processors4;
declare const index_Processors5: typeof Processors5;
type index_PullRecord<T extends Record<string, PrimitiveOrObject | (() => any) | Reactive<any>>> = PullRecord<T>;
type index_PullRecordWithInitial<T extends Record<string, PrimitiveOrObject | (() => any) | Reactive<any>>> = PullRecordWithInitial<T>;
...
type index_Resolvable<V> = Resolvable<V>;
type index_ResolveValue<V> = ResolveValue<V>;
type index_ResolvedObject<V> = ResolvedObject<V>;
...
declare const index_fieldResolve: typeof fieldResolve;
declare const index_fieldResolver: typeof fieldResolver;
...
declare const index_processChain: typeof processChain;
declare const index_proportion: typeof proportion;
declare const index_pull: typeof pull;
declare const index_reactiveUpdate: typeof reactiveUpdate;
...
declare namespace index {
  export { index_Bipolar as Bipolar, index_Correlate as Correlate, type index_FrequencyEventMap as FrequencyEventMap, index_FrequencyMutable as FrequencyMutable, index$1 as Graphs, type index_InterpolateOptions as InterpolateOptions, index_IntervalTracker as IntervalTracker, type index_MovingAverageTimedOptions as MovingAverageTimedOptions, index_Normalise as Normalise, index_NumberFunction as NumberFunction, index_NumberTracker as NumberTracker, index_NumberTrackerResults as NumberTrackerResults, index_PointTrack as PointTrack, index_PointTracker as PointTracker, index_PointTrackerResults as PointTrackerResults, Pool$1 as Pool, index_PrimitiveTracker as PrimitiveTracker, index_Process as Process, index_ProcessFactory as ProcessFactory, index_Processors as Processors, index_Processors1 as Processors1, index_Processors2 as Processors2, index_Processors3 as Processors3, index_Processors4 as Processors4, index_Processors5 as Processors5, type index_PullRecord as PullRecord, type index_PullRecordWithInitial as PullRecordWithInitial, index_RankArrayOptions as RankArrayOptions, index_RankFunction as RankFunction, index_RankOptions as RankOptions, type index_Resolvable as Resolvable, type index_ResolveValue as ResolveValue, type index_ResolvedObject as ResolvedObject, index_Table as Table, index_Timestamped as Timestamped, index_TimestampedObject as TimestampedObject, index_TimestampedPrimitive as TimestampedPrimitive, type index_TrackUnique as TrackUnique, index_TrackedPointMap as TrackedPointMap, index_TrackedValueMap as TrackedValueMap, index_TrackedValueOpts as TrackedValueOpts, index_TrackerBase as TrackerBase, index_clamp as clamp, index_clampIndex as clampIndex, index_fieldResolve as fieldResolve, index_fieldResolver as fieldResolver, index_flip as flip, index_frequencyMutable as frequencyMutable, index_interpolate as interpolate, index_interpolateAngle as interpolateAngle, index_interpolatorInterval as interpolatorInterval, index_interpolatorStepped as interpolatorStepped, index_intervalTracker as intervalTracker, index_movingAverage as movingAverage, index_movingAverageLight as movingAverageLight, index_movingAverageTimed as movingAverageTimed, index_noiseFilter as noiseFilter, index_numberTracker as numberTracker, index_piPi as piPi, index_pointTracker as pointTracker, index_pointsTracker as pointsTracker, index_processChain as processChain, index_proportion as proportion, index_pull as pull, index_reactiveUpdate as reactiveUpdate, index_scale as scale, index_scaleClamped as scaleClamped, index_scalePercent as scalePercent, index_scalePercentages as scalePercentages, index_scaler as scaler, index_scalerPercent as scalerPercent, index_softmax as softmax, index_trackUnique as trackUnique, index_trackUniqueInstances as trackUniqueInstances, index_wrap as wrap, index_wrapInteger as wrapInteger, index_wrapRange as wrapRange };
}

export { fieldResolve as A, Bipolar as B, Correlate as C, fieldResolver as D, reactiveUpdate as E, type FrequencyEventMap as F, pull as G, scale as H, type InterpolateOptions as I, scaler as J, scaleClamped as K, scalePercentages as L, type MovingAverageTimedOptions as M, Normalise as N, scalePercent as O, Pool$1 as P, scalerPercent as Q, type Resolvable as R, softmax as S, TrackedPointMap as T, Table as U, type TrackUnique as V, trackUnique as W, trackUniqueInstances as X, wrapInteger as Y, wrap as Z, wrapRange as _, index$1 as a, clampIndex as b, clamp as c, FrequencyMutable as d, frequencyMutable as e, flip as f, interpolate as g, interpolatorStepped as h, index as i, interpolatorInterval as j, interpolateAngle as k, movingAverageTimed as l, movingAverageLight as m, movingAverage as n, noiseFilter as o, piPi as p, IntervalTracker as q, intervalTracker as r, PointTracker as s, pointsTracker as t, pointTracker as u, proportion as v, type PullRecord as w, type PullRecordWithInitial as x, type ResolveValue as y, type ResolvedObject as z };
