  ...
  seriesNames.forEach((s) => {
    ...
    if (colour == void 0) {
      if (drawingOpts.defaultSeriesVariable)
        colour = Colour_exports.getCssVariable(`yellow`, drawingOpts.defaultSeriesColour);
      else
    ...
    if (series.drawingStyle === `line`) {
      ...
      for (let i = 0; i < d.length; i++) {
        ...
        if (y > this.visual.height)
          console.log(y + " h: " + this.visual.height);
        x += pxPerPt;
      }
      ctx.strokeStyle = series.colour;
      ctx.stroke();
...
var Plot = class extends CanvasBox {
  ...
  createSeries(name, type = `array`, initialData) {
...
//# sourceMappingURL=chunk-654LPYUF.js.map