  ...
  "node_modules/fp-ts/lib/ReadonlyNonEmptyArray.js"(exports) {
    ...
    var appendW = function(end) {
      return function(init6) {
        return __spreadArray2(__spreadArray2([], init6, true), [end], false);
      };
    };
    ...
    exports.cons = cons;
    var snoc = function(init6, end) {
      return (0, function_1.pipe)(init6, concat3([end]));
    };
    exports.snoc = snoc;
...
var require_ReadonlyArray = __commonJS({
  "node_modules/fp-ts/lib/ReadonlyArray.js"(exports) {
    ...
    var isEmpty8 = function(as) {
      return as.length === 0;
    };
    exports.isEmpty = isEmpty8;
    ...
    function spanLeft(predicate) {
      return function(as) {
        var _a = (0, exports.splitAt)(spanLeftIndex(as, predicate))(as), init6 = _a[0], rest = _a[1];
  ...
  "node_modules/json5/dist/index.js"(exports, module) {
    ...
    (function(global, factory) {
      ...
      var _toAbsoluteIndex = function(index, length6) {
        ...
        return index < 0 ? max8(index + length6, 0) : min8(index, length6);
      };
...
// src/Guards.ts
var numberTest = (value, range3 = ``, parameterName = `?`) => {
  if (value === null)
...
var throwNumberTest = (value, range3 = ``, parameterName = `?`) => {
  throwFromResult(numberTest(value, range3, parameterName));
};
...
var integerTest = (value, range3 = ``, parameterName = `?`) => {
  const r = numberTest(value, range3, parameterName);
  if (!r[0])
    return r;
...
var throwIntegerTest = (value, range3 = ``, parameterName = `?`) => {
  throwFromResult(integerTest(value, range3, parameterName));
};
var integerParse = (value, range3 = ``, defaultValue = Number.NaN) => {
  ...
  try {
    ...
    const r = integerTest(parsed, range3, `parsed`);
...
var stream = (minDefault, maxDefault) => {
  ...
  let max8 = maxDefault ?? Number.MIN_SAFE_INTEGER;
  throwNumberTest(minDefault);
  throwNumberTest(maxDefault);
  return (v) => {
    throwNumberTest(v);
    min8 = Math.min(min8, v);
    max8 = Math.max(max8, v);
    return scale(v, min8, max8);
  };
};
var array = (values, minForced, maxForced) => {
  ...
  const max8 = maxForced ?? mma.max;
  return values.map((v) => clamp(scale(v, min8, max8)));
};
...
var MapOfSimpleMutable = class extends MapOfSimpleBase {
  ...
  deleteKeyValue(key, value) {
    ...
    if (existing === void 0)
      return false;
    const without2 = existing.filter((existingValue) => !this.valueEq(existingValue, value));
...
var QueueMutable = class {
   ...
   */
  remove(v, comparer) {
    const length6 = this.data.length;
    this.data = without(this.data, v, comparer ?? this.eq);
    return this.data.length !== length6;
  }
  get isEmpty() {
...
var peek2 = (opts, stack) => stack[stack.length - 1];
...
function min(it, gt = (a, b) => a > b) {
  ...
  for (const v of it) {
    ...
    min8 = gt(min8, v) ? v : min8;
  }
  return min8;
}
function* range(start3, length6) {
  for (let index = 0; index < length6; index++) {
    yield start3++;
  }
}
function reduce(it, f, start3) {
  for (const v of it)
    start3 = f(start3, v);
  return start3;
}
function* slice(it, start3 = 0, end = Number.POSITIVE_INFINITY) {
...
function* unique(iterable, keyFunction = toStringDefault2) {
  const buffer = [];
  let itera = [];
...
var clampMagnitude = (v, max8 = 1, min8 = 0) => {
  let mag = v.distance;
  if (mag > max8)
    mag = max8;
  if (mag < min8)
...
var TreeNodeMutable = class _TreeNodeMutable {
  ...
  addValueByPath(value, path2, pathOpts = {}) {
    ...
    const split2 = path2.split(sep);
    const label = split2.at(-1);
    let node = this;
...
var immutable2 = (keyString = toStringDefault2) => new SetStringImmutable(keyString);
...
var PriorityMutable = class extends QueueMutable {
  ...
  dequeueMax() {
    const m = max2(this.data, (v) => v.priority);
    if (m === void 0)
      return;
    this.remove(m);
    return m.item;
  ...
  dequeueMin() {
    const m = min2(this.data, (v) => v.priority);
    if (m === void 0)
      return;
    this.remove(m);
    return m.item;
...
var toArray = (map4) => Array.from(map4.values());
var fromIterable = (data, keyFn = toStringDefault2, allowOverwrites = false) => {
...
var immutable4 = (dataOrMap) => {
  ...
  if (Array.isArray(dataOrMap))
    return immutable4(add(/* @__PURE__ */ new Map(), ...dataOrMap));
...
var MapOfMutableImpl = class extends SimpleEventEmitter {
  ...
  constructor(type, opts = {}) {
    super();
    this.type = type;
    this.groupBy = opts.groupBy ?? toStringDefault2;
  }
  /**
...
`;
        ...
        const asArray3 = this.type.toArray(v);
        if (asArray3 !== void 0) {
...
var ofSetMutable = (opts) => {
  const hash = opts?.hash ?? toStringDefault2;
...
var guard3 = (line3, name = `line`) => {
  if (line3 === void 0)
    throw new Error(`${name} undefined`);
  if (line3.a === void 0)
    throw new Error(`${name}.a undefined. Expected {a:Point, b:Point}. Got: ${JSON.stringify(line3)}`);
  if (line3.b === void 0)
    throw new Error(`${name}.b undefined. Expected {a:Point, b:Point} Got: ${JSON.stringify(line3)}`);
};
...
var multiply2 = (line3, point3) => Object.freeze({
  ...line3,
  a: Point_exports.multiply(line3.a, point3),
  b: Point_exports.multiply(line3.b, point3)
});
var divide2 = (line3, point3) => Object.freeze({
  ...line3,
...
var subtract2 = (line3, point3) => Object.freeze({
  ...line3,
...
var withinRange = (line3, point3, maxRange) => {
  const calculatedDistance = distance(line3, point3);
...
var nearest = (line3, point3) => {
  const n = (line4) => {
    ...
    const length6 = atob.x * atob.x + atob.y * atob.y;
    let dot2 = atop.x * atob.x + atop.y * atob.y;
    const t4 = Math.min(1, Math.max(0, dot2 / length6));
...
var perpendicularPoint = (line3, distance4, amount = 0) => {
  const origin = interpolate(amount, line3);
  const dvn = directionVectorNormalised(line3);
  return {
    x: origin.x - dvn.y * distance4,
    y: origin.y + dvn.x * distance4
  };
};
var parallel = (line3, distance4) => {
  const dv = directionVector(line3);
  const dvn = directionVectorNormalised(line3);
  const a = {
    x: line3.a.x - dvn.y * distance4,
    y: line3.a.y + dvn.x * distance4
  };
  return {
    a,
    b: {
...
var extendFromA = (line3, distance4) => {
  const calculatedLength = length2(line3);
  return Object.freeze({
    ...line3,
    a: line3.a,
    b: Object.freeze({
      x: line3.b.x + (line3.b.x - line3.a.x) / calculatedLength * distance4,
      y: line3.b.y + (line3.b.y - line3.a.y) / calculatedLength * distance4
    })
  });
};
...
var distanceSingleLine = (line3, point3) => {
  ...
  guard(point3, `point`);
  if (length2(line3) === 0) {
    return length2(line3.a, point3);
  }
  const near = nearest(line3, point3);
  return length2(near, point3);
};
...
var joinPointsToLines = (...points) => {
  ...
  for (let index = 1; index < points.length; index++) {
    lines.push(fromPoints(start3, points[index]));
    start3 = points[index];
  }
  return lines;
};
...
var toPath = (line3) => {
  ...
  return Object.freeze({
    ...
    toPoints: () => [a, b],
    rotate: (amountRadian, origin) => toPath(rotate2(line3, amountRadian, origin)),
    nearest: (point3) => nearest(line3, point3),
    sum: (point3) => toPath(sum(line3, point3)),
    divide: (point3) => toPath(divide2(line3, point3)),
    multiply: (point3) => toPath(multiply2(line3, point3)),
    subtract: (point3) => toPath(subtract2(line3, point3)),
    midpoint: () => midpoint(a, b),
    distance: (point3) => distanceSingleLine(line3, point3),
    parallel: (distance4) => parallel(line3, distance4),
    perpendicularPoint: (distance4, amount) => perpendicularPoint(line3, distance4, amount),
    slope: () => slope(line3),
...
var rotate2 = (line3, amountRadian, origin) => {
...
var Placeholder2 = Object.freeze({ x: Number.NaN, y: Number.NaN });
...
var project = (origin, distance4, angle2) => {
  const x = Math.cos(angle2) * distance4 + origin.x;
  const y = Math.sin(angle2) * distance4 + origin.y;
  return { x, y };
};
function rotate3(pt, amountRadian, origin) {
  if (origin === void 0)
    origin = { x: 0, y: 0 };
  guard(origin, `origin`);
...
var clampMagnitude2 = (pt, max8 = 1, min8 = 0) => {
  const length6 = distance2(pt);
  let ratio = 1;
  if (length6 > max8) {
    ratio = max8 / length6;
  } else if (length6 < min8) {
    ratio = min8 / length6;
  }
  return ratio === 1 ? pt : multiply3(pt, ratio, ratio);
};
...
var relation = (a, b) => {
  const start3 = getPointParameter2(a, b);
  let totalX = 0;
  let totalY = 0;
  let count2 = 0;
...
var progressBetween = (currentPos, from2, to2) => {
  ...
  return Point_exports.isPoint3d(a) && Point_exports.isPoint3d(b) ? (a.x * b.x + a.y * b.y + a.z * b.z) / (b.x * b.x + b.y * b.y + b.z * b.z) : (a.x * b.x + a.y * b.y) / (b.x * b.x + b.y * b.y);
};
...
var length4 = (arc2) => piPi * arc2.radius * ((arc2.startRadian - arc2.endRadian) / piPi);
var bbox3 = (arc2) => {
  if (isPositioned(arc2)) {
...
var utils = {
  ...
  getminmax: function(curve, d, list) {
    ...
    return { min: min8, mid: (min8 + max8) / 2, max: max8, size: max8 - min8 };
  },
  ...
  roots: function(points, line3) {
    ...
    if (order === 2) {
      ...
      if (d2 !== 0) {
        ...
        return [(2 * b2 - c2) / (2 * b2 - 2 * c2)].filter(reduce5);
      }
      return [];
    }
    ...
    if (utils.approximately(d, 0)) {
      if (utils.approximately(a, 0)) {
        ...
        return [-c / b].filter(reduce5);
      }
      const q3 = sqrt(b * b - 4 * a * c), a2 = 2 * a;
      return [(q3 - b) / a2, (-b - q3) / a2].filter(reduce5);
    }
    a /= d;
    b /= d;
    c /= d;
    ...
    if (discriminant < 0) {
      ...
      return [x1, x2, x3].filter(reduce5);
    } else if (discriminant === 0) {
      u1 = q2 < 0 ? crt(-q2) : -crt(q2);
      x1 = 2 * u1 - a / 3;
      x2 = -u1 - a / 3;
      return [x1, x2].filter(reduce5);
    } else {
      const sd = sqrt(discriminant);
      u1 = crt(-q2 + sd);
      v1 = crt(q2 + sd);
      return [u1 - v1 - a / 3].filter(reduce5);
    }
  },
  droots: function(p) {
    if (p.length === 3) {
  ...
  _iterate(errorThreshold, circles) {
    ...
    do {
      ...
      do {
        ...
        let error2 = this._error(arc2, np1, t_s, t_e);
...
var normaliseByRect3 = (rect2, normaliseByOrWidth, height4) => {
  ...
  return isPositioned2(rect2) ? Object.freeze({
    x: rect2.x / width,
    y: rect2.y / height4,
...
var randomPoint2 = (within, opts = {}) => {
  ...
  return isPositioned2(within) ? Point_exports.sum(pos, within) : Object.freeze(pos);
};
...
var guard6 = (circle3, paramName = `circle`) => {
  if (isPositioned3(circle3)) {
...
var guardPositioned2 = (circle3, paramName = `circle`) => {
  ...
  return guard6(circle3, paramName);
};
var center3 = (circle3) => {
  if (isPositioned3(circle3))
...
var length5 = (circle3) => circumference(circle3);
...
var randomPoint3 = (within, opts = {}) => {
  const offset2 = isPositioned3(within) ? within : { x: 0, y: 0 };
  const strategy = opts.strategy ?? `uniform`;
  const rand = opts.randomSource ?? defaultRandom;
  switch (strategy) {
    case `naive`:
      return Point_exports.sum(offset2, Polar_exports.toCartesian(rand() * within.radius, rand() * piPi2));
    case `uniform`:
      return Point_exports.sum(offset2, Polar_exports.toCartesian(Math.sqrt(rand()) * within.radius, rand() * piPi2));
    default:
      throw new Error(`Unknown strategy ${strategy}`);
  }
};
function multiplyScalar3(a, value) {
  if (isPositioned3(a)) {
...
var visitor = function* (logic, grid2, start3, opts = {}) {
  guardGrid(grid2, `grid`);
  guardCell(start3, `start`, grid2);
  const v = opts.visited ?? mutable3(cellKeyString);
...
var fromLinePolar = (line3) => {
  guard3(line3, `line`);
  const pt = subtract(line3.b, line3.a);
...
var clampMagnitude3 = (v, max8 = 1, min8 = 0) => {
  if (isPolar(v)) {
    return Polar_exports.clampMagnitude(v, max8, min8);
  } else if (isCartesian(v)) {
    return clampMagnitude2(v, max8, min8);
  }
...
var init = (paths2, opts = {}) => {
  ...
  const checkUnordered = (pt) => {
    const results = paths2.map((p, index) => {
      ...
      const distance4 = distance2(pt, nearest3);
      return { path: p, index, nearest: nearest3, distance: distance4 };
    });
...
var random3 = (circles, container, opts = {}) => {
  ...
  const willHit = (b, radius) => positionedCircles.some((v) => isIntersecting3(v, b, radius));
  while (sorted.length) {
...
function hslConvert(o) {
  ...
  if (o instanceof Hsl)
    return o;
  o = o.rgb();
  var r = o.r / 255, g = o.g / 255, b = o.b / 255, min8 = Math.min(r, g, b), max8 = Math.max(r, g, b), h = NaN, s = max8 - min8, l = (max8 + min8) / 2;
  if (s) {
    if (r === max8)
      h = (g - b) / s + (g < b) * 6;
    else if (g === max8)
...
var resolveColour = (c) => {
  if (typeof c === `string`) {
    ...
    if (css !== null)
      return css;
    if (c.startsWith(`hsl`) && c.indexOf("%") <= 0)
      throw new Error(`Could not resolve CSS colour ${c}. HSL values should be in the form: hsl(0, 50%, 50%)`);
    else
      throw new Error(`Could not resolve CSS colour ${c}`);
  } else {
    if (isHsl(c))
...
var interpolate6 = (amount, from2, to2, optsOrSpace) => {
...
var rotate4 = (t4, amountRadian, origin) => {
  ...
  return Object.freeze({
    ...t4,
    a: rotate3(t4.a, amountRadian, origin),
    b: rotate3(t4.b, amountRadian, origin),
    c: rotate3(t4.c, amountRadian, origin)
  });
};
...
var canvasHelper = (domQueryOrEl, opts) => {
  ...
  let scaler3 = Scaler_exports.scaler(`both`);
  const updateDimensions = (rect2) => {
    scaler3 = Scaler_exports.scaler(scaleBy, rect2);
...
var resolveEl = (domQueryOrEl) => {
  if (typeof domQueryOrEl === `string`) {
    ...
    if (d === null) {
      const error2 = !domQueryOrEl.startsWith(`#`) ? new Error(
...
// src/visual/Drawing.ts
var PIPI = Math.PI * 2;
var getContext = (canvasElCtxOrQuery) => {
...
var boxRectFromPx = (x, y, width, height4) => {
  return {
    x: boxUnitFromPx(x),
    y: boxUnitFromPx(y),
...
var Box = class {
  ...
  *getChildren() {
  ...
  measureSelf(opts, parent) {
    ...
    const context2 = parent ? parent.actual : opts.bounds;
    const desired = opts.resolveBox(this._desiredRect);
    size = desired ? clamp3(desired, context2) : context2;
   ...
   * @returns 
   */
  update(context2, force = false) {
    if (context2 === void 0)
      throw new Error(`context is undefined`);
    if (!this._needsMeasuring && !this._needsLayoutX && !force)
      return;
    const [measureState, layoutState] = this.updateBegin(context2);
    let attempts = 5;
...
var CanvasBox = class _CanvasBox extends Box {
   ...
   */
  notifyClick(p) {
    if (isPlaceholder3(this.canvasRegion))
      return;
   ...
   */
  updateBegin(context2) {
    if (context2 === void 0)
      throw new Error(`Context is undefined`);
    let bounds = this.getBounds();
    if (bounds === void 0) {
      this.debugLog(`No bounds for element or parent, using canvas bounds`);
      bounds = { x: 0, y: 0, width: context2.canvas.width, height: context2.canvas.height };
    }
    return [
      new CanvasMeasureState(bounds, context2),
      new CanvasLayoutState(bounds, context2)
    ];
  }
...
var ArrayDataSource = class {
  ...
  get range() {
    if (!this.dirty && this._range !== void 0)
      return this._range;
    this.dirty = false;
    const updatedRange = Arrays_exports.minMaxAvg(this.data);
...
var PlotArea = class extends CanvasBox {
  ...
  drawDataSet(series, d, ctx) {
    ...
    const v = Rect_exports.subtract(this.canvasRegion, padding * 2, padding * 3.5);
...
var Legend = class extends CanvasBox {
  ...
  measureSelf(opts, _parent) {
    ...
    for (const s of series) {
      ...
      const text2 = textRect(ctx, s.name, padding, widthSnapping);
      ...
      rowHeight = Math.min(sample2.height + padding + padding, text2.height + padding + padding);
      x += padding;
      if (x > availableWidth) {
        x = padding;
        y += rowHeight;
  ...
  drawSelf(ctx) {
    ...
    for (const s of series) {
      ...
      ctx.fillRect(x, r.y, sample2.width, sample2.height);
      x += sample2.width + padding;
      ctx.textBaseline = `middle`;
      ctx.fillStyle = this.plot.legendTextColour;
      ctx.fillText(s.name, x, r.y + sample2.height / 2);
      if (s.tooltip) {
        ctx.fillStyle = this.plot.legendTextColour;
        ctx.fillText(s.tooltip, r.x, r.y + sample2.height / 2);
      }
    }
  }
  onNotify(message, source) {
...
var Plot = class extends CanvasBox {
  ...
  createSeriesFromObject(o, prefix = ``) {
    ...
    return keys.flatMap((k) => create5(k));
  }
...
var immutable5 = (startingValueOrBipolar = 0) => {
  ...
  return {
    ...
    value: v,
    towardZero: (amt) => {
      return immutable5(towardZero(v, amt));
    },
    add: (amt) => {
      return immutable5(clamp4(v + amt));
    },
    multiply: (amt) => {
      return immutable5(clamp4(v * amt));
    },
    ...
    interpolate: (amt, b) => {
      return immutable5(clamp4(interpolate7(amt, v, b)));
    },
    asScalar: () => {
      return toScalar(v);
    }
...
var scale3 = (inputValue, inMin, inMax) => {
  return clamp4(scaler(inMin, inMax, -1, 1)(inputValue));
};
var clamp4 = (bipolarValue) => {
  if (typeof bipolarValue !== `number`)
...
var init2 = (elementQuery, opts = {}) => {
  ...
  const height4 = opts.height ?? getNumericAttribute(element, `height`, 200) * window.devicePixelRatio;
  element.width = width;
  ...
  const renderBackground = opts.renderBackground ?? ((ctx2, width2, height5) => {
    ctx2.fillStyle = bgColour;
    ctx2.fillRect(0, 0, width2, height5);
  });
  return (x, y) => {
    x = clamp4(x);
    y = clamp4(y);
    ...
    ctx.stroke();
    ctx.closePath();
    const yy = (height4 - pad - pad) / 2 * -y;
    const xx = (width - pad - pad) / 2 * x;
    ctx.save();
    ctx.translate(midX, midY);
    if (showDot) {
      Drawing_exports.circle(ctx, { radius: dotRadius, x: xx, y: yy }, { fillStyle: dotColour });
    }
    if (showWhiskers) {
...
__export(Plot_exports, {
  scalarCanvasPlot: () => scalarCanvasPlot,
  scalarPlotData: () => scalarPlotData
});

// src/data/pipes/index.ts
var pipes_exports = {};
__export(pipes_exports, {
  As: () => As_exports,
  Feed: () => Feed_exports,
  Fns: () => Functions_exports,
  From: () => From_exports,
  LogSink: () => LogSink,
  NullSink: () => NullSink,
  Ops: () => Ops_exports,
...
var interval = async function* (produce, optsOrFixedMs = {}) {
  ...
  const doDelay = async () => {
    ...
    if (sleepMs) {
      await sleep({ millis: sleepMs, signal: signal2 });
    }
    started = performance.now();
    if (signal2?.aborted)
      throw new Error(`Signal aborted ${signal2.reason}`);
  };
...
var close = (source) => [void 0, { type: `closed`, source }];
var error = (message, source, stack) => [void 0, { type: `error`, message, source, stack }];
var emptyInOut = [void 0, void 0];
function isSignal(v) {
  if (v[0] !== void 0)
    return false;
  if (v[1] === void 0)
...
function isError(v) {
  const [_value, signal2] = v;
  if (signal2 === void 0)
    return false;
  return signal2.type === `error`;
}
function isClosed(v) {
  const [value, signal2] = v;
  if (signal2 === void 0)
    return false;
  if (value !== void 0)
    return false;
  return signal2.type === `closed`;
}
function isEmpty7(v) {
  return v[0] === void 0 && v[1] === void 0;
}

// src/data/pipes/As.ts
var As_exports = {};
__export(As_exports, {
  asArray: () => asArray,
...
// src/data/pipes/Types.ts
function isFinitePipe(p) {
  return `isClosed` in p;
}

// src/data/pipes/As.ts
async function asPromise(p) {
  if (isFinitePipe(p) && p.isClosed())
    throw new Error(`Finite pipe has closed`);
  let resolveP;
  let _rejectP;
  const initPromise = () => {
    const p2 = new Promise((resolveParameter, rejectParameter) => {
...
async function* asAsyncIterable(p) {
...
var asArray = async (pipe2, options = {}) => {
  const returnArray = [];
  const maximumValues = options.maximumValues ?? Number.MAX_SAFE_INTEGER;
  for await (const v of asAsyncIterable(pipe2)) {
    if (returnArray.length >= maximumValues)
      break;
    returnArray.push(v);
  }
  return returnArray;
};
var basic = (p, initialFire = false) => {
  const t4 = {
    get value() {
      return p.last();
    },
...
__export(Ops_exports, {
  ...
  filterAB: () => filterAB,
  filterTap: () => filterTap,
  isOnePipeOp: () => isOnePipeOp,
  mergeValues: () => mergeValues,
  minMaxAvgTotal: () => minMaxAvgTotal,
  monitor: () => monitor,
  reduce: () => reduce3,
  split: () => split,
...
// src/data/pipes/Feed.ts
var Feed_exports = {};
__export(Feed_exports, {
  feedGenerator: () => feedGenerator,
  feedIterable: () => feedIterable,
  feedPipes: () => feedPipes,
  feedTrigger: () => feedTrigger
});
var Name_Iterable = `feedIterable`;
var Name_Generator = `feedGenerator`;
var Name_Pipes = `feedPipes`;
var feedIterable = (destination, iterator2, options = {}) => {
  ...
  const done2 = () => {
    if (keepOpen)
      return;
    destination.inlet(Signal_exports.close(Name_Iterable));
  };
  const read = () => {
    if (count2 >= maximumValues) {
      done2();
      return;
    }
    const n = iterator2.next();
    if (n.done) {
      done2();
      return;
    } else {
      destination.inlet([n.value, void 0]);
      count2++;
    }
    setTimeout(read, ms);
  };
...
var feedGenerator = async (destination, generator, options = {}) => {
  const keepOpen = options.keepOpen ?? false;
  ...
  if (!keepOpen) {
    destination.inlet(Signal_exports.close(Name_Generator));
  }
};
var feedTrigger = (p) => {
...
var feedPipes = (desination, sourcePipes, options = {}) => {
  ...
  const close2 = () => {
    ...
    desination.inlet(Signal_exports.close(Name_Pipes));
  };
  for (const source of sourcePipes) {
    source.setOutlet((v) => {
      if (Signal_exports.isClosed(v)) {
        switch (closePolicy) {
          case `all`: {
            closedCount++;
            source.setOutlet(NullSink);
...
// src/data/pipes/From.ts
var array2 = (values, options = {}) => {
  const pipe2 = bidi();
  feedIterable(pipe2, values.values(), options);
  return pipe2;
};
var callback = (computeValue) => {
  const pipe2 = bidi();
  const listener = (args) => {
    pipe2.inlet([computeValue(args), void 0]);
  };
  return [pipe2, listener];
};
var trigger = (computeValue, opts = {}) => {
  const pipe2 = bidi(opts);
  const listener = () => {
    if (typeof computeValue === `function`) {
      pipe2.inlet(computeValue());
    } else {
      pipe2.inlet([computeValue, void 0]);
    }
  };
  if (opts.primeOutlet)
    listener();
...
var event = (eventObject, eventName) => {
  let disposed = false;
  const pipe2 = bidi({
    signalHandler: (signal2) => {
      if (signal2.type === `dispose` && !disposed) {
...
var pipes = (...inputPipes) => {
  const b = bidi();
  feedPipes(b, inputPipes, { close: `all` });
  return b;
};

// src/data/pipes/Functions.ts
...
var insert = (sourcePipe, insertPipe) => {
  const existingOutlet = sourcePipe.getOutlet();
  sourcePipe.setOutlet(insertPipe);
  if (existingOutlet)
    insertPipe.setOutlet(existingOutlet);
};
var inlet = (value, ...pipes2) => {
  for (const pipe2 of pipes2) {
    pipe2.inlet(value);
  }
};
var signal = (pipe2, signal2) => {
  pipe2.inlet([void 0, signal2]);
};

// src/data/pipes/Ops.ts
function isOnePipeOp(v) {
  return `_type` in v && v._type === `One`;
}
var Names_Take = `take`;
var Names_Synchronise = `synchronise`;
var take = (options) => {
  let count2 = 0;
  const length6 = options.length;
  const keepOpen = options.keepOpen ?? false;
  const run2 = (upstream) => {
    const downstream = bidi();
    upstream.setOutlet((value) => {
      if (count2 >= length6) {
        upstream.setOutlet(NullSink);
        if (keepOpen)
          return;
        downstream.inlet(Signal_exports.close(Names_Take));
        return;
      }
      downstream.inlet(value);
      if (Signal_exports.isValue(value)) {
        count2++;
      }
    });
    return downstream;
...
var monitor = (monitorFunction, initialState) => {
  const run2 = (upstream) => {
    const downstream = bidi();
...
var transform = (transformer) => {
  const run2 = (upstream) => {
    ...
    upstream.setOutlet((value) => {
      if (Signal_exports.isValue(value)) {
        downstream.inlet([transformer(value[0]), void 0]);
      } else {
        downstream.inlet([void 0, value[1]]);
      }
    });
    return downstream;
  };
...
var chunk = (size, returnRemainders = true) => {
  const run2 = (upstream) => {
    const downstream = bidi();
    let buffer = [];
    const lengthValue = number(size);
    upstream.setOutlet((value) => {
      if (Signal_exports.isValue(value)) {
        buffer.push(value[0]);
        if (buffer.length >= (lengthValue.last() ?? 0)) {
          downstream.inlet([buffer, void 0]);
          buffer = [];
        }
      } else {
        if (Signal_exports.isClosed(value) && buffer.length > 0 && returnRemainders) {
          downstream.inlet([buffer, void 0]);
        }
        downstream.inlet([void 0, value[1]]);
      }
...
var window2 = (length6) => {
  const run2 = (upstream) => {
    const downstream = bidi();
    let buffer = [];
    const lengthValue = number(length6);
    upstream.setOutlet((value) => {
      if (Signal_exports.isValue(value)) {
        buffer.push(value[0]);
        if (buffer.length > (lengthValue.last() ?? 0)) {
...
var emit = (compute, options) => {
  ...
  const run2 = (upstream) => {
    ...
    upstream.setOutlet((value) => {
      switch (when) {
        case `any`: {
          downstream.inlet([compute(), void 0]);
          return;
        }
        case `signal`: {
          if (Signal_exports.isSignal(value)) {
            downstream.inlet([compute(), void 0]);
            return;
          }
          break;
        ...
        case `emit-signal`: {
          if (Signal_exports.isSignal(value) && value[1].type === `emit`) {
            downstream.inlet([compute(), void 0]);
            return;
          }
          break;
      ...
      if (passIfNotHandled) {
        downstream.inlet([value[0], value[1]]);
      }
    });
    return downstream;
  };
...
var filterAB = (filterPredicate) => {
  const run2 = (upstream) => {
    const pipeA = bidi();
    const pipeB = bidi();
    upstream.setOutlet((v) => {
      if (Signal_exports.isValue(v)) {
  ...
  run2._type = `OneToMany`;
  return run2;
};
var filterTap = (filterPredicate) => {
  const run2 = (upstream) => {
    const seenPipe = bidi();
    const regularPipe = bidi();
    upstream.setOutlet((v) => {
      if (Signal_exports.isValue(v) && filterPredicate(v[0])) {
        seenPipe.inlet(v);
      }
      regularPipe.inlet(v);
    });
...
var reduce3 = (reducer) => {
  const run2 = (upstream) => {
    ...
    upstream.setOutlet((values) => {
      if (Signal_exports.isValue(values)) {
        pipe2.inlet([reducer(values[0]), void 0]);
      } else {
        pipe2.inlet([void 0, values[1]]);
      }
    });
    return pipe2;
  };
...
var minMaxAvgTotal = () => {
  const run2 = (upstream) => {
    ...
    let max8 = Number.MIN_SAFE_INTEGER;
    let avg = Number.NaN;
    let total4 = 0;
    let count2 = 1;
    upstream.setOutlet((v) => {
      if (Signal_exports.isValue(v)) {
        ...
        count2++;
        avg = total4 / count2;
        downstream.inlet([{ min: min8, max: max8, avg, total: total4 }, void 0]);
      } else {
        downstream.inlet([void 0, v[1]]);
      }
    });
    return downstream;
  };
...
var split = (options = {}) => {
  ...
  const run2 = (upstream) => {
    const copies = [...repeat(numberOfCopies, () => bidi())];
    const split2 = (v) => {
      if (filterSignals && Signal_exports.isSignal(v))
        return;
      inlet(v, ...copies);
    };
    upstream.setOutlet(split2);
    return copies;
  };
  run2._type = `OneToMany`;
  return run2;
};
var synchronise = (opts) => {
  const overwritePolicy = opts.overwritePolicy ?? `latest`;
  const loop = opts.loop ?? true;
  const run2 = (upstreams) => {
    let results = [];
    const downstream = bidi();
    const resetValues = () => {
      results = [];
    ...
    const checkCompletion = () => {
      if (results.includes(void 0))
        return;
      downstream.inlet([results, void 0]);
      if (loop) {
        resetValues();
      } else {
        dispose();
    ...
    const dispose = () => {
      ...
      downstream.inlet(Signal_exports.close(Names_Synchronise));
    };
    resetValues();
    for (const [index, upstream] of upstreams.entries()) {
      upstream.setOutlet((inOut) => {
        if (!Signal_exports.isValue(inOut))
          return;
        const value = inOut[0];
    ...
    const trigger2 = () => {
      ...
      if (elapsed > rateMs) {
        ...
        if (latestValue !== void 0) {
          downstream.inlet([latestValue, void 0]);
        }
        clearTimeout(timer);
        return;
      }
      clearTimeout(timer);
      timer = setTimeout(trigger2, rateMs - elapsed);
    };
    upstream.setOutlet((value) => {
      if (Signal_exports.isValue(value)) {
        latestValue = value[0];
...
var mergeValues = (merger, opts) => {
  const run2 = (pipes2) => {
    ...
    const compute = () => {
      ...
      if (!allowUndefinedValues && undefinedExists)
        return;
      const mergedValue = merger(...values);
      pipe2.inlet([mergedValue, void 0]);
    };
    for (const [index] of pipes2.entries()) {
      values[index] = void 0;
    }
    for (const [index, pipe3] of pipes2.entries()) {
      pipe3.setOutlet((v) => {
        if (Signal_exports.isValue(v)) {
          values[index] = v[0];
          compute();
...
var LogSink = (value, signal2) => {
  if (signal2) {
    console.log(`Pipes.LogSink value: ${JSON.stringify(value)} signal: ${JSON.stringify(signal2)}`);
  } else {
...
var bidi = (opts = {}) => {
  ...
  let currentOutlet;
  let last2 = [opts.initialValue, void 0];
  let closed = false;
  const inlet2 = (value) => {
    if (isSignal(value)) {
      if (isClosed(value)) {
  ...
  const signal2 = (signal3) => {
    if (signalHandler) {
      const stop = signalHandler(signal3);
      if (stop)
        return;
    }
    if (currentOutlet) {
      currentOutlet([void 0, signal3]);
    } else {
      console.warn(`signal called on a pipe with no outlet`);
    }
  };
  const setOutlet = (callback2, setOpts = {}) => {
    if (closed)
      throw new Error(`Pipe is closed`);
    currentOutlet = typeof callback2 === `function` ? callback2 : callback2.inlet;
    if (last2[0] && (primeOutlet || setOpts.primeOutlet)) {
      currentOutlet(last2);
    }
  };
  return { signal: signal2, getOutlet: () => currentOutlet, inlet: inlet2, setOutlet, last: () => last2[0], isClosed: isClosed2 };
};
var compose = (...pipes2) => {
  let last2;
  let first2;
  ...
  const result = {
    getOutlet() {
      return first2?.getOutlet;
    },
    inlet(value) {
      if (first2 === void 0)
        throw new Error(`No first pipe`);
      if (`inlet` in first2) {
        first2.inlet(value);
      } else if (value[1] !== void 0) {
        first2.signal(value[1]);
      }
    },
    last() {
      if (last2 === void 0)
    ...
    setOutlet(callback2, setOpts) {
      if (last2 === void 0)
        throw new Error(`No pipes`);
      last2.setOutlet(callback2, setOpts);
    },
    signal(signal2) {
      if (first2 === void 0)
        throw new Error(`No first pipe`);
      first2.signal(signal2);
    }
  };
  return result;
};
var rxWindow = (opts = {}) => {
  ...
  return {
    size: basic(resize, true),
    dispose
  };
};

// src/visual/Plot.ts
var scalarCanvasPlot = (canvasElement, desiredRegion) => {
  const box = new SceneGraph_exports.CanvasBox(void 0, `scalarPlot`);
  const desiredBox = typeof desiredRegion.height === `number` ? SceneGraph_exports.boxRectFromRectPx(desiredRegion) : desiredRegion;
  box.desiredRegion = desiredBox;
  box.debugLayout = true;
  const data = scalarPlotData();
  const yAxis = new SceneGraph_exports.CanvasBox(box, `axis-y`);
  yAxis.desiredRegion = {
    ...desiredBox,
...
var calcScale = (buffer, drawingOpts, seriesColours) => {
  ...
  for (const s of buffer.keys()) {
    ...
    if (series.length === 0)
      break;
    let { min: min8, max: max8 } = minMaxAvg(series);
    let range3 = max8 - min8;
    let colour;
...
var drawYSeriesScale = (series, plotSize, drawing) => {
  ...
  const min8 = y.labelRange ? y.labelRange[0] : series.min;
  const max8 = y.labelRange ? y.labelRange[1] : series.max;
  const range3 = y.labelRange ? max8 - min8 : series.range;
  const mid = min8 + range3 / 2;
  const halfHeight = drawing.textHeight / 2;
  ctx.textBaseline = `top`;
  ctx.fillText(
    min8.toFixed(digitsPrecision),
    0,
    calcYForValue(min8, series, height4) - halfHeight
  );
  ctx.fillText(
  ...
  ctx.fillText(
    max8.toFixed(digitsPrecision),
    0,
    calcYForValue(max8, series, height4) - margin
  );
...
var drawYLine = (plotSize, series, drawing) => {
  ...
  const min8 = y.labelRange ? y.labelRange[0] : series.min;
  const max8 = y.labelRange ? y.labelRange[1] : series.max;
  const minPos = calcYForValue(min8, series, height4);
  const maxPos = calcYForValue(max8, series, height4);
...
function convolveCell(c, kernel, source, access, reduce5) {
  ...
  return reduce5(valuesAtKernelPos);
}
...
function* convolve(kernel, source, access, visitor2, reduce5, origin) {
  if (!origin) {
    const kernelRows = kernel.length;
    const kernelCols = kernel[0].length;
    origin = { x: Math.floor(kernelRows / 2), y: Math.floor(kernelCols / 2) };
  }
  const asArray3 = kernel2dToArray(kernel, origin);
  for (const c of visitor2) {
    const v = convolveCell(c, asArray3, source, access, reduce5);
    yield [c, v];
  }
}
...
function* circleVogelSpiral(circle3, opts = {}) {
  ...
  let count2 = 0;
  let angle2 = 0;
  while (count2 < maxPoints && radius < max8) {
    radius = spacing * count2 ** 0.5;
    angle2 = rotationOffset + count2 * 2 * pi4 / goldenSection;
    yield Object.freeze({
...
async function init4(machine, handlersOrOpts) {
...
var init3 = (stateMachine, initialState) => {
...
var retry = async (cb, opts) => {
  ...
  if (predelayMs > 0)
    await sleep({ millis: predelayMs, signal: signal2 });
  if (signal2?.aborted) {
    return {
      success: false,
      attempts,
  ...
  while (attempts < count2) {
    ...
    log2({
      msg: `retry attempts: ${attempts} t: ${toString6(t4)}`
    });
    if (attempts >= count2) {
      break;
    }
    try {
      await sleep({ millis: t4, signal: signal2 });
    } catch (ex) {
      return {
        success: false,
        attempts,
...
var TrackerBase2 = class {
  ...
  seen(...p) {
    ...
    const t4 = this.filterData(p);
    return this.computeResults(t4);
  }
};
...
// src/io/AudioAnalyser.ts
var basic2 = (onData, opts = {}) => new AudioAnalyser2((node, analyser) => {
...
var pingPongPercent = function(interval2 = 0.1, lower, upper, start3, rounding) {
  ...
  throwNumberTest(start3, `bipolar`, `offset`);
...
var pingPong = function* (interval2, lower, upper, start3, rounding) {
  ...
  const distance4 = upper - lower;
  if (Math.abs(interval2) >= distance4) {
    throw new Error(`Interval should be between -${distance4} and ${distance4}`);
  }
...
// src/flow/Delay.ts
var delay = async (callback2, optsOrMillis) => {
  ...
  const r = Promise.resolve(await callback2());
...
async function* delayAnimationLoop() {
  ...
  try {
    while (true) {
      timer = window.requestAnimationFrame(callback2);
      yield await p;
    }
  } finally {
    resolve();
...
async function* delayLoop(timeout2) {
  ...
  try {
    while (true) {
      timer = window.setTimeout(callback2, timeoutMs);
      yield await p;
    }
  } finally {
    resolve();
...
var numericPercent = function(interval2 = 0.01, repeating = false, start3 = 0, end = 1) {
  ...
  throwNumberTest(start3, `percentage`, `start`);
...
// src/dom/DomRx.ts
var rx = (elOrQuery, event2, opts) => {
  const el2 = resolveEl(elOrQuery);
  const ev = fromEvent(el2, event2);
  const value = {};
  const clear3 = () => {
...
var fromObject2 = (parentOrQuery, data, opts) => {
  ...
  const update = (d) => {
    ...
    updateElement(t4, d, { ...opts, idPrefix });
  };
...
var select = (domQueryOrEl, onChanged, opts = {}) => {
  ...
  return {
    ...
    select(index = 0, trigger2 = false) {
      el2.selectedIndex = index;
      if (trigger2 && onChanged) {
        change();
      }
    }
  };
...
var PointTracker = class extends ObjectTracker {
  ...
  difference() {
    const initial = this.initial;
    return this.values.length >= 2 && initial !== void 0 ? subtract(this.last, initial) : Placeholder2;
  }
  /**
...
var pointerVisualise = (elOrQuery, opts = {}) => {
  ...
  const trackPointer = async (event2) => {
    ...
    if (event2.type === `pointermove` && !tracker2.has(id)) {
      return;
    }
    const info = await tracker2.seen(event2.pointerId.toString(), { x: event2.clientX, y: event2.clientY });
...
var defaultErrorHandler2 = () => {
  ...
  window.onerror = (message, url, lineNo, colNo, error2) => {
    if (enabled) {
      if (error2) {
        console.log(error2);
        show(error2);
...
var inlineConsole = (opts = {}) => {
  ...
  window.onerror = (event2, source, lineno, _colno, error2) => {
    const abbreviatedSource = source === void 0 ? `` : afterMatch2(source, `/`, { fromEnd: true });
    const eventString = typeof event2 === `string` ? event2 : event2.toString();
...
var relativeTimer = (total4, opts = {}) => {
  ...
  return {
    ...
    get elapsed() {
      let v = timer.elapsed / (total4 * modulationAmount);
      if (clampValue)
        v = clamp(v);
      else if (wrapValue) {
        if (v >= 1)
...
var computePositionFromAngle = (distance4, angleRadians, origin) => Polar_exports.toCartesian(distance4, angleRadians, origin);
...
function* sine(timerOrFreq) {
  ...
  while (true) {
    yield (Math.sin(timerOrFreq.elapsed * piPi7) + 1) / 2;
  }
}
function* sineBipolar(timerOrFreq) {
  ...
  while (true) {
    yield Math.sin(timerOrFreq.elapsed * piPi7);
  }
}
...
// src/Iterable.ts
var isAsyncIterable2 = (v) => Symbol.asyncIterator in new Object(v);
...
function lazy() {
  ...
  const w = {
    asGenerator,
    fromFunction: (callback2) => {
      chained.push(fromFunction(callback2));
      return w;
    },
    take: (limit) => {
      chained.push(take2(limit));
      return w;
    },
    chunk: (size, returnRemainders = true) => {
      chained.push(chunk2(size, returnRemainders));
      return w;
    },
    filter: (predicate) => {
      chained.push(filter4(predicate));
      return w;
    },
    min: () => {
      chained.push(min4());
...
function fromFunction(callback2) {
  async function* fromFunction2() {
    while (true) {
      const v = await callback2();
      if (v === void 0)
        break;
      yield v;
    }
...
async function asCallback(valueToWrap, callback2, onDone) {
  const outputType = typeof valueToWrap === `function` ? valueToWrap() : valueToWrap;
...
async function asArray2(valueToWrap) {
  const outputType = typeof valueToWrap === `function` ? valueToWrap() : valueToWrap;
  return IterableAsync_exports.toArray(outputType);
}
async function addToArray(array3, valueToWrap) {
  const outputType = typeof valueToWrap === `function` ? valueToWrap() : valueToWrap;
  for await (const value of outputType) {
    array3.push(value);
  }
...
async function single(f, input) {
  const iterator2 = await f([input]).next();
  return iterator2.value;
}
function flatten2(flattener) {
...
function take2(limit) {
  async function* take3(input) {
    input = resolveToGen(input);
    let yielded = 0;
    for await (const value of input) {
      if (++yielded > limit)
...
function min4() {
  async function* min8(input) {
    input = resolveToGen(input);
    let min9 = Number.MAX_SAFE_INTEGER;
    for await (const value of input) {
      if (typeof value !== `number`)
        break;
      min9 = Math.min(value, min9);
      yield min9;
    }
  }
  min8._name = `min`;
...
function max4() {
  async function* max8(input) {
    input = resolveToGen(input);
    let max9 = Number.MIN_SAFE_INTEGER;
    for await (const value of input) {
      if (typeof value !== `number`)
        break;
      max9 = Math.max(value, max9);
      yield max9;
    }
  }
  max8._name = `max`;
...
function chunk2(size, returnRemainders = true) {
  throwIntegerTest(size, `aboveZero`, `size`);
  async function* chunk3(input) {
    input = resolveToGen(input);
    let buffer = [];
    for await (const value of input) {
      buffer.push(value);
...
var debugGraphToArray = (graph3) => {
  ...
  for (const v of vertices2) {
    ...
    r.push(...str.map((line3) => ` ${line3}`));
  }
  return r;
};
var distance3 = (graph3, edge) => {
  if (edge.weight !== void 0)
    return edge.weight;
  return 1;
};
function* edges3(graph3) {
...
function* adjacentVertices(graph3, context2) {
  if (context2 === void 0)
    return;
  const vertex = typeof context2 === `string` ? graph3.vertices.get(context2) : context2;
  if (vertex === void 0)
    throw new Error(`Vertex not found ${JSON.stringify(context2)}`);
  for (const edge of vertex.out) {
...
var hasNoOuts = (graph3, vertex) => {
  const context2 = typeof vertex === `string` ? graph3.vertices.get(vertex) : vertex;
  if (context2 === void 0)
    return false;
  return context2.out.length === 0;
};
var hasOnlyOuts = (graph3, vertex, ...outIdOrVertex) => {
  const context2 = resolveVertex(graph3, vertex);
  const outs = outIdOrVertex.map((o) => resolveVertex(graph3, o));
  if (outs.length !== context2.out.length) {
    return false;
  }
  for (const out of outs) {
    if (!hasOut(graph3, context2, out)) {
      return false;
    }
  }
  return true;
...
var hasOut = (graph3, vertex, outIdOrVertex) => {
  const context2 = resolveVertex(graph3, vertex);
  const outId = typeof outIdOrVertex === `string` ? outIdOrVertex : outIdOrVertex.id;
  return context2.out.some((edge) => edge.id === outId);
};
...
function disconnect(graph3, from2, to2) {
  const fromV = resolveVertex(graph3, from2);
  const toV = resolveVertex(graph3, to2);
  return hasOut(graph3, fromV, toV) ? updateGraphVertex(graph3, {
    ...fromV,
    out: fromV.out.filter((t4) => t4.id !== toV.id)
  }) : graph3;
}
function connectTo(graph3, from2, to2, weight2) {
  const fromResult = getOrCreate(graph3, from2);
...
function connect(graph3, options) {
  ...
  const bidi2 = options.bidi ?? false;
  const toList = Array.isArray(to2) ? to2 : [to2];
  for (const toSingle of toList) {
    const result = connectTo(graph3, from2, toSingle, weight2);
    graph3 = result.graph;
  }
  if (!bidi2)
    return graph3;
  for (const toSingle of toList) {
    const result = connectTo(graph3, toSingle, from2, weight2);
    graph3 = result.graph;
  }
  return graph3;
}
...
var pathDijkstra = (graph3, sourceOrId) => {
  ...
  while (!pq.isEmpty) {
    ...
    for (const neighbour of vertexU.out) {
      const alt = distances.get(u) + distance3(graph3, neighbour);
...
var clone = (graph3) => {
  const g = {
    vertices: immutable4([...graph3.vertices.entries()])
  };
  return g;
};
...
function topologicalSort(graph3) {
  ...
  for (const edge of edges3(graph3)) {
...
function connectTo2(graph3, a, b, weight2) {
...
function connect2(graph3, options) {
  ...
  for (const destination of destinations) {
    const result = connectTo2(graph3, a, destination, weight2);
    graph3 = result.graph;
  }
  return graph3;
}
...
function* adjacentVertices2(graph3, context2) {
  if (context2 === void 0)
    return;
  const vertex = typeof context2 === `string` ? graph3.vertices.get(context2) : context2;
  if (vertex === void 0)
  ...
  for (const edge of graph3.edges) {
    ...
    else if (edge.b === context2)
      yield resolveVertex2(graph3, edge.a);
  }
}
function* edgesForVertex(graph3, context2) {
  if (context2 === void 0)
    return;
  const vertex = typeof context2 === `string` ? graph3.vertices.get(context2) : context2;
  if (vertex === void 0)
...
// src/data/Wrap.ts
var wrapInteger = (v, min8 = 0, max8 = 360) => {
  throwIntegerTest(v, void 0, `v`);
  throwIntegerTest(min8, void 0, `min`);
  throwIntegerTest(max8, void 0, `max`);
  if (v === min8)
    return min8;
  if (v === max8)
    return min8;
  ...
  v = v % max8;
  if (v < 0)
    v = max8 - Math.abs(v) + min8;
  return v + min8;
};
var wrap = (v, min8 = 0, max8 = 1) => {
  throwNumberTest(v, ``, `min`);
  throwNumberTest(min8, ``, `min`);
  throwNumberTest(max8, ``, `max`);
  if (v === min8)
    return min8;
  if (v === max8)
    return min8;
...
var wrapRange = (min8, max8, fn, a, b) => {
  let r = 0;
  const distF = Math.abs(b - a);
  const distFwrap = Math.abs(max8 - a + b);
  ...
  return wrapInteger(r, min8, max8);
};
...
var interpolateAngle = (amount, aRadians, bRadians) => {
  const t4 = wrap(bRadians - aRadians, 0, piPi8);
  return interpolate7(amount, aRadians, aRadians + (t4 > Math.PI ? t4 - piPi8 : t4));
};
...
var create4 = function(nameOrFunction, duration2, timerSource) {
  ...
  if (fn === void 0) {
    const error2 = typeof nameOrFunction === `string` ? new Error(`Easing function not found: ${nameOrFunction}`) : new Error(`Easing function not found`);
    throw error2;
  }
...
var minMaxAvg = (data, opts = {}) => {
  ...
  const total4 = validNumbers2.reduce((acc, v) => acc + v, 0);
  return {
    total: total4,
    max: Math.max(...validNumbers2),
    min: Math.min(...validNumbers2),
    avg: total4 / validNumbers2.length
  };
};
...
// src/Numbers.ts
var average3 = (...numbers) => average(numbers);
...
var min6 = (...data) => min5(data);
var max6 = (...data) => max5(data);
var total3 = (...data) => total2(data);
...
var quantiseEvery = (v, every3, middleRoundsUp = true) => {
  ...
  throwFromResult(integerTest(every3, ``, `every`));
  let div = v / every3;
...
function* linearSpace(start3, end, steps, precision) {
  throwFromResult(numberTest(start3, ``, `start`));
...
var toString6 = (millisOrFunction, rounding = 2) => {
  let interval2 = {} = 0;
  if (typeof millisOrFunction === `function`) {
    const intervalResult = millisOrFunction();
    return toString6(intervalResult);
...
// src/flow/Timeout.ts
var timeout = (callback2, interval2) => {
  if (callback2 === void 0) {
  ...
  const start3 = async (altInterval = interval2, args) => {
    const p = new Promise((resolve, reject) => {
      ...
      timer = window.setTimeout(async () => {
        await callback2(performance.now() - startedAt, ...args);
        timer = 0;
        resolve(void 0);
      }, altTimeoutMs);
    });
...
var continuously = (callback2, interval2, opts = {}) => {
  ...
  const loop = async () => {
    if (!running)
      return;
    const valOrPromise = callback2(ticks++, performance.now() - startedAt);
    let val;
...
var debounce2 = (callback2, timeoutMs) => {
  const t4 = timeout(callback2, timeoutMs);
  return (...args) => t4.start(void 0, args);
};

// src/flow/Throttle.ts
var throttle2 = (callback2, intervalMinMs) => {
  let trigger2 = 0;
  return async (...args) => {
    const elapsed = performance.now() - trigger2;
    if (elapsed >= intervalMinMs) {
      const r = callback2(elapsed, ...args);
      if (typeof r === `object`)
        await r;
      trigger2 = performance.now();
    }
  };
};

// src/flow/Every.ts
var everyNth = (nth, callback2) => {
...
var repeatReduce = (countOrPredicate, fn, initial, reduce5) => {
  if (typeof countOrPredicate === `number`) {
    throwNumberTest(countOrPredicate, `positive`, `countOrPredicate`);
    while (countOrPredicate-- > 0) {
      const v = fn();
...
var sleep = (optsOrMillis) => {
  ...
  const signal2 = optsOrMillis.signal;
...
// src/IterableAsync.ts
async function* fromArray(array3) {
  for (const v of array3) {
    yield v;
    await sleep(1);
  }
}
async function* fromIterable3(iterable) {
...
async function find3(it, f) {
  for await (const v of it) {
    if (f(v))
      return v;
  }
}
async function* flatten3(it) {
  for await (const v of it) {
...
async function forEach3(it, f) {
  for await (const v of it)
    f(v);
}
async function* map3(it, f) {
  for await (const v of it) {
    yield f(v);
  }
}
async function max7(it, gt = (a, b) => a > b) {
  let max8;
  for await (const v of it) {
    if (!max8) {
      max8 = v;
...
async function min7(it, gt = (a, b) => a > b) {
  let min8;
  for await (const v of it) {
    if (!min8) {
      min8 = v;
...
async function* range2(start3, length6) {
  for (let index = 0; index < length6; index++) {
    yield start3++;
  }
}
async function reduce4(it, f, start3) {
  for await (const v of it)
    start3 = f(start3, v);
  return start3;
}
async function* slice2(it, start3 = 0, end = Number.POSITIVE_INFINITY) {
...
async function* takeWhile2(it, f) {
  for await (const v of it) {
    if (!f(v))
      return;
    yield v;
...
async function toArray5(it, count2 = Number.POSITIVE_INFINITY) {
  const result = [];
...
async function* unique3(it, f = (id) => id) {
  const buffer = [];
  for await (const v of it) {
    const fv = f(v);
    if (buffer.includes(fv))
...
// src/Util.ts
var isFunction2 = (object) => object instanceof Function;
...
var mapObject = (object, mapFunction) => {
...
var getFieldByPath = (o, path2 = ``) => {
  ...
  if (path2 in o) {
    ...
    return start3 in o ? getFieldByPath(o[start3], path2.slice(Math.max(0, start3.length + 1))) : void 0;
  }
};
var getFieldPaths = (o) => {
  if (o === null)
  ...
  const probe = (o2, initialPaths, prefix) => {
    ...
    return [...keys.map((key) => probe(o2[key], [], prefixToUse + key)), initialPaths].flat();
  };
  return probe(o, [], ``);
};
...
var flatten4 = (array3) => Array.prototype.concat.apply([], [...array3]);
var zip2 = (...arrays) => {
...
var containsDuplicateValues = (array3, keyFn = toStringDefault2) => {
  if (!Array.isArray(array3))
...
var weightedIntegerSource = (maxOrOptions) => {
  ...
  const source = options.source ?? defaultRandom;
  const max8 = options.max;
  const min8 = options.min ?? 0;
  const easingName = options.easing ?? `quadIn`;
  ...
  throwFromResult(numberTest(max8));
  const easingFunction = get(easingName);
  if (easingFunction === void 0) {
    throw new Error(`Easing '${easingName}' not found`);
  }
  throwFromResult(numberTest(min8));
  if (max8 <= min8)
    throw new Error(`Max should be greater than min`);
  const compute = () => {
    const r = clamp(easingFunction(source()));
    return Math.floor(r * (max8 - min8)) + min8;
  };
  return compute;
};
...
var integerSource = (maxOrOptions) => {
  ...
  const options = typeof maxOrOptions === `number` ? { max: maxOrOptions } : maxOrOptions;
  let max8 = Math.floor(options.max);
  let min8 = Math.floor(options.min ?? 0);
  if (!options.min && max8 < 0) {
    max8 = 1;
    min8 = options.max;
  }
  const randomSource = options.source ?? defaultRandom;
  if (min8 > max8) {
    throw new Error(`Min value is greater than max (min: ${min8} max: ${max8})`);
  }
  throwFromResult(numberTest(min8, ``, `min`));
  throwFromResult(numberTest(max8, ``, `max`));
  if (max8 === min8) {
    throw new Error(`Max and min values cannot be the same (${max8})`);
  }
  const amt = Math.abs(max8 - min8);
  return () => Math.floor(randomSource() * amt) + min8;
};
var integer = (maxOrOptions) => integerSource(maxOrOptions)();
var floatSource = (maxOrOptions = 1) => {
  const options = typeof maxOrOptions === `number` ? { max: maxOrOptions } : maxOrOptions;
  let max8 = options.max;
  ...
  if (min8 > max8) {
    throw new Error(`Min is greater than max. Min: ${min8} max: ${max8}`);
  }
  return () => source() * (max8 - min8) + min8;
};
...
var minutesMsSource = (maxMinutesOrOptions) => {
  ...
  const min8 = (options.min ?? 0) * 60 * 1e3;
  const max8 = options.max * 60 * 1e3;
  return integerSource({ ...options, max: max8, min: min8 });
};
...
var secondsMsSource = (maxSecondsOrOptions) => {
  ...
  const min8 = (options.min ?? 0) * 1e3;
  const max8 = options.max * 1e3;
  return () => integer({ ...options, max: max8, min: min8 });
};
...
function* integerUniqueGen(maxOrOptions) {
  ...
  throwFromResult(integerTest(min8, ``, `min`));
  throwFromResult(integerTest(max8, ``, `max`));
  if (min8 > max8) {
    throw new Error(`Min value is greater than max. Min: ${min8} Max: ${max8}`);
  }
  const origRange = [...range(min8, max8 - min8)];
...
var indexOfCharCode = (source, code, start3 = 0, end = source.length - 1) => {
  for (let index = start3; index <= end; index++) {
...
var splitByLength = (source, length6) => {
  throwFromResult(integerTest(length6, `aboveZero`, `length`));
  if (source === null)
  ...
  for (let c = 0; c < chunks4; c++) {
    returnValue.push(source.slice(start3, start3 + length6));
    start3 += length6;
  }
  return returnValue;
};
