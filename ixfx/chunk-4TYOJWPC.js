...
var markerPrebuilt = (elem, opts, context) => {
  if (elem === null)
    return `(elem null)`;
  ...
  if (defEl !== null) {
    console.log(`Already made ${opts.id}!`);
    return `url(${opts.id})`;
  }
  console.log(`markerPrebuilt creating: ${opts.id}. Opts: ${JSON.stringify(opts)}`);
  if (opts.id === `triangle`) {
...
//# sourceMappingURL=chunk-4TYOJWPC.js.map