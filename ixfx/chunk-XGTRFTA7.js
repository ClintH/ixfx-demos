...
var debounce = (callback, interval3) => {
  const t = timeout(callback, interval3);
  return (...args) => {
    t.start(void 0, args);
  };
};
...
try {
  if (typeof window !== `undefined`) {
    window.ixfx = {
      ...window.ixfx,
      Flow: { StateMachine: StateMachine_exports, Timer: Timer_exports, forEach, forEachAsync, repeatAwait, repeat }
    };
  }
} catch {
}
