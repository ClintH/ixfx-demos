...
var start = async (constraints = {}) => {
  ...
  try {
    console.log(`Camera.start startWithVideoEl`);
    const r = await startWithVideoEl(videoEl, constraints);
    stopVideo = r.dispose;
    console.log(`Camera.start startWithVideoEl done`);
    return { videoEl, dispose };
  } catch (ex) {
    console.log(`Camera.start startWithVideoEl throws`);
    console.error(ex);
    dispose();
    console.log(`Camera.start rethrowing`);
    throw ex;
  }
};
var startWithVideoEl = async (videoEl, constraints = {}) => {
  ...
  const done = waitFor(constraints.startTimeoutMs ?? startTimeoutMs, (reason) => {
    console.log(`Camera: getUserMedia failed: ${reason}`);
    throw new Error(reason);
  }, (success) => {
    console.log(`Camera: getUserMedia complete: ${success ? `successfully` : `failed`}`);
  });
  try {
    ...
    console.log(`Camera startWithVideoEl exception`);
    done(ex.toString());
    console.log(`Camera startWithVideoEl rethrowing`);
    throw ex;
  }
};
...
var FrameProcessor = class {
  constructor(opts = {}) {
    ...
    __publicField(this, "_cameraCapture");
  ...
  async useCamera(constraints) {
    ...
    console.log(`FrameProcessor.useCamera init`);
    await this.init();
    console.log(`FrameProcessor.useCamera done`);
  }
  async initCamera() {
    console.log(`FrameProcessor.initCamera`);
    const r = await start(this._cameraConstraints);
    console.log(`FrameProcessor.initCamera done`);
    if (r === void 0)
      throw new Error(`Could not start camera`);
    if (this._showPreview)
      r.videoEl.style.display = `block`;
    this._cameraCapture = manualCapture(r.videoEl, {
...
var grid = (parent, center3, spacing, width, height4, opts = {}) => {
  if (!opts.strokeStyle)
...
var makeHelper2 = (parent, parentOpts) => {
  ...
  const o = {
    ...
    grid: (center3, spacing, width, height4, opts) => grid(parent, center3, spacing, width, height4, opts),
...
var drawYSeriesScale = (series, plotSize, drawing) => {
  ...
  ctx.textBaseline = `top`;
  ctx.fillText(min3.toFixed(digitsPrecision), 0, calcYForValue(min3, series, height4) - halfHeight);
  ctx.fillText(mid.toFixed(digitsPrecision), 0, calcYForValue(mid, series, height4) - halfHeight);
  ctx.fillText(max3.toFixed(digitsPrecision), 0, calcYForValue(max3, series, height4) - margin);
...
var drawYLine = (plotSize, series, drawing) => {
  ...
  const min3 = y.labelRange ? y.labelRange[0] : series.min;
  const max3 = y.labelRange ? y.labelRange[1] : series.max;
  const minPos = calcYForValue(min3, series, height4);
  const maxPos = calcYForValue(max3, series, height4);
...
//# sourceMappingURL=chunk-7NFKSEUP.js.map