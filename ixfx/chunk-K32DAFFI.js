...
var bindTextContent = (source, elOrQuery, bindOpts = {}) => {
  return bindElement(source, elOrQuery, { ...bindOpts, elField: `textContent` });
};
var bindHtmlContent = (source, elOrQuery, bindOpts = {}) => {
...
var bind = (source, ...bindsUnresolvedElements) => {
  const binds = bindsUnresolvedElements.map((bind2) => {
    if (bind2.element)
      return bind2;
    if (bind2.query)
      return { ...bind2, element: resolveEl(bind2.query) };
    throw new Error(`Unable to resolve element`);
  });
  const resolveTransform = (bind2) => {
  ...
  const resolveBindUpdater = (bind2) => {
    if (bind2.elField !== void 0 || bind2.cssVariable === void 0 && bind2.attribName === void 0 && bind2.cssProperty === void 0) {
      ...
      return (v2) => {
        bind2.element[field2] = v2;
      };
    }
    if (bind2.attribName !== void 0) {
      const attrib = bind2.attribName;
      return (v2) => {
        bind2.element.setAttribute(attrib, v2);
      };
    }
    if (bind2.cssVariable !== void 0) {
      ...
      if (!css.startsWith(`--`))
        css = `--` + css;
      return (v2) => {
        console.log(`set css var ${css} = ${v2}`);
        bind2.element.style.setProperty(css, v2);
      };
    }
    if (bind2.cssProperty !== void 0) {
      return (v2) => {
        bind2.element.style[bind2.cssProperty] = v2;
      };
    }
    return (v2) => {
    };
    ...
    for (const bind2 of bindsResolved) {
      if (bind2.transformer) {
        ...
        if (typeof value === `object`)
          bind2.update(JSON.stringify(value));
        return v;
      }
      if (bind2.sourceField && bind2.update) {
        bind2.update(bind2.transformer(value[bind2.sourceField]));
      } else if (bind2.update) {
...
var cloneFromFields = () => transform((v2) => {
  const entries = [];
...
function toAdjacencyMatrix(graph2) {
  ...
  for (let i = 0; i < v2.length; i++) {
    table.setRow(i, v2.length, false);
    const ii = v2[i];
    for (const [j, jj] of v2.entries()) {
...
function transitiveReduction(graph2) {
  for (const u of vertices(graph2)) {
    for (const v2 of adjacentVertices(graph2, u)) {
...
function fromObject(initialValue, options = {}) {
  ...
  const set = (v2) => {
    if (value !== void 0) {
      const diff = compareData(value, v2, ``, options);
      if (diff.length === 0)
        return;
      diffEvent.set(diff);
    }
