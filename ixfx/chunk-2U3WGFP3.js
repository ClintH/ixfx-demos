...
function length(aOrLine, pointB) {
  if (isPolyLine(aOrLine)) {
    const sum4 = aOrLine.reduce((acc, v) => length(v) + acc, 0);
    return sum4;
  }
...
var centroid = (...points) => {
  ...
  const sum4 = points.reduce((prev, p) => {
    if (p === void 0)
      return prev;
    if (Array.isArray(p))
...
var clampMagnitude = (pt, m) => {
  ...
  if (length5 > m) {
    ...
    return multiply2(pt, ratio, ratio);
  }
  return pt;
};
...
var utils = {
  ...
  length: function(derivativeFn) {
    ...
    for (let i = 0, t4; i < len; i++) {
      ...
      sum4 += utils.Cvalues[i] * utils.arcfn(t4, derivativeFn);
    }
    return z * sum4;
  },
...
var arrow = (origin, from2, opts = {}) => {
  ...
  if (from2 === `tip`) {
    ...
    tailPoints = Rect_exports.corners(Rect_exports.fromTopLeft({ x: origin.x - midX, y: origin.y - midY }, tailLength + arrowSize, tailThickness));
  } else {
...
var PointTracker = class extends ObjectTracker {
  ...
  seen(...p) {
    ...
    const initialRel = {
      ...
      speed: this.values.length < 2 ? 0 : length(this.initial ?? Empty, newLast) / (newLast.at - (this.initial ? this.initial.at : 0))
    };
    const lastRel = {
...
var AdsrImpl = class extends AdsrBase {
  constructor(opts) {
    ...
    const max4 = 1;
    this.attackPath = toPath3(quadraticSimple({ x: 0, y: this.initialLevel }, { x: max4, y: this.peakLevel }, -this.attackBend));
    this.decayPath = toPath3(quadraticSimple({ x: 0, y: this.peakLevel }, { x: max4, y: this.sustainLevel }, -this.decayBend));
    this.releasePath = toPath3(quadraticSimple({ x: 0, y: this.sustainLevel }, { x: max4, y: this.releaseLevel }, -this.releaseBend));
  }
  onTrigger() {
...
var springForce = (pinnedAt, restingLength = 0.5, k = 2e-4, damping = 0.999) => (t4) => {
  ...
  const f = Point_exports.pipelineApply(dir, Point_exports.normalise, (p) => Point_exports.multiply(p, -k * stretch));
...
var springRaw = (opts = {}, from2 = 0, to = 1) => {
  ...
  if (soft === true || 1 <= damping / (2 * Math.sqrt(stiffness * mass))) {
...
var interpolateAngle = (amount, aRadians, bRadians) => {
  const t4 = wrap(bRadians - aRadians, 0, piPi6);
  return interpolate7(amount, aRadians, aRadians + (t4 > Math.PI ? t4 - piPi6 : t4));
};
...
//# sourceMappingURL=chunk-2U3WGFP3.js.map