...
var interpolate = (amount, from, to, space = `rgb`) => {
  const inter = getInterpolator(space, from, to);
  if (inter === void 0)
    throw new Error(`Could not handle colour/space`);
  return inter(amount);
};
var getInterpolator = (space, ...colours) => {
  let inter;
  switch (space) {
    case `lab`:
      if (colours.length !== 2)
        throw new Error(`LAB interpolation expects two colours`);
      inter = lab(colours[0], colours[1]);
      break;
    case `hsl`:
      if (colours.length !== 2)
        throw new Error(`HSL interpolation expects two colours`);
      inter = hsl_default(colours[0], colours[1]);
      break;
    case `hcl`:
      if (colours.length !== 2)
        throw new Error(`HCL interpolation expects two colours`);
      inter = hcl_default(colours[0], colours[1]);
      break;
    case `cubehelix`:
      console.log(space);
      if (colours.length !== 2)
        throw new Error(`Cubehelix interpolation expects two colours`);
      inter = cubehelix_default(colours[0], colours[1]);
      break;
    default:
      if (colours.length < 2)
        throw new Error(`RGB interpolation Expects at least two colours`);
      inter = rgbBasis(colours);
  }
  return inter;
};
var scale = (steps, space, ...colours) => {
  const inter = getInterpolator(space, ...colours);
  if (inter === void 0)
    throw new Error(`Could not handle colour/space`);
