...
var require_ReadonlyNonEmptyArray = __commonJS({
  "node_modules/fp-ts/lib/ReadonlyNonEmptyArray.js"(exports) {
    ...
    var range = function(start2, end) {
      return start2 <= end ? (0, exports.makeBy)(function(i) {
        return start2 + i;
      })(end - start2 + 1) : [start2];
    };
    exports.range = range;
...
var require_string = __commonJS({
  "node_modules/fp-ts/lib/string.js"(exports) {
    ...
    var slice = function(start2, end) {
      return function(s) {
        return s.slice(start2, end);
      };
    };
    exports.slice = slice;
...
var relation = (a, b) => {
  ...
  let lastPoint = start2;
  const update = (aa, bb) => {
    const p = getPointParam(aa, bb);
    totalX += p.x;
    totalY += p.y;
    count++;
    const distanceFromStart = distance2(p, start2);
...
var fullSizeElement = (domQueryOrEl, onResized) => {
  ...
  const update = () => {
    ...
    const bounds = { width, height: height4, center: { x: width / 2, y: height4 / 2 } };
    if (onResized !== void 0)
      onResized({ el, bounds });
  };
  r.subscribe(update);
...
var textAreaKeyboard = (el) => {
  el.addEventListener(`keydown`, (evt) => {
    ...
    if (evt.key === `Tab` && evt.shiftKey) {
      if (el.value.substring(start2 - 2, start2) === `  `) {
        el.value = val.substring(0, start2 - 2) + val.substring(end);
      }
      el.selectionStart = el.selectionEnd = start2 - 2;
      evt.preventDefault();
      return false;
    } else if (evt.key === `Tab`) {
      el.value = val.substring(0, start2) + `  ` + val.substring(end);
      el.selectionStart = el.selectionEnd = start2 + 2;
      evt.preventDefault();
      return false;
    }
...
var align = (fn, lastData, newData, opts = {}) => {
  ...
  for (let i = 0; i < newData.length; i++) {
    ...
    if (r.length === 0) {
      if (debug2)
        console.debug(`align() no valid last values id: ${newD.id}`);
      newThings.push(newD);
      continue;
    }
    r.sort(orderScore);
...
var functions = {
  arch,
  bell: gaussian(),
  sineIn: (x) => 1 - cos3(x * pi4 / 2),
  sineOut: (x) => sin3(x * pi4 / 2),
  quadIn: (x) => x * x,
  quadOut: (x) => 1 - (1 - x) * (1 - x),
  sineInOut: (x) => -(cos3(pi4 * x) - 1) / 2,
...
var mergeByKey = (keyFn, reconcile, ...arrays) => {
...
***************************************************************************** */
//# sourceMappingURL=chunk-I6DRZS6B.js.map