...
var parentSizeSvg = (domQueryOrEl, onResized, timeoutMs = 100) => {
  const el = resolveEl(domQueryOrEl);
  if (el.nodeName !== `svg`)
    throw new Error(`Expected HTML element with node name 'svg', not ${el.nodeName}`);
  const ro = resizeObservable(el, timeoutMs).subscribe((entries) => {
    console.log(`ro happening`);
    const e = entries.find((v) => v.target === el);
    if (e === void 0)
      return;
    const width = e.contentRect.width;
    const height = e.contentRect.height;
    console.log(`updating with ${width},${height}`);
...
***************************************************************************** */
//# sourceMappingURL=chunk-SRSRD5M4.js.map