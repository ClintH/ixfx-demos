...
// src/Guards.ts
var number = (value, range3 = ``, paramName = `?`) => {
...
__export(flow_exports, {
  ...
  iterableFromPoll: () => iterableFromPoll,
...
var sleep = (optsOrMillis) => {
  if (typeof optsOrMillis === "number")
    optsOrMillis = { millis: optsOrMillis };
...
__export(StateMachine_exports, {
  ...
  bidirectionalDescriptionFromList: () => bidirectionalDescriptionFromList,
  create: () => create,
...
var SimpleEventEmitter = class {
   ...
   */
  fireEvent(type, args) {
    const listeners = this.#listeners.values(type);
...
// src/flow/StateMachine.ts
var StateMachine = class _StateMachine extends SimpleEventEmitter {
  #state;
  #debug;
  #m;
  #isDone;
  ...
  constructor(initial, m, opts = { debug: false }) {
    super();
    const [isValid2, errorMsg] = _StateMachine.validate(initial, m);
    if (!isValid2)
  ...
  isValid(newState) {
    return _StateMachine.isValid(this.state, newState, this.#m);
  }
  /**
  ...
  set state(newState) {
    ...
    if (newState === priorState)
      return;
    const [isValid2, errorMsg] = _StateMachine.isValid(
      priorState,
      newState,
      this.#m
    );
...
var intervalToMs = (i, defaultNumber) => {
  if (typeof i === "undefined")
    return defaultNumber;
  if (typeof i === "number")
    return i;
  let ms = i.millis ?? 0;
  ms += (i.hrs ?? 0) * 60 * 60 * 1e3;
  ms += (i.mins ?? 0) * 60 * 1e3;
  ms += (i.secs ?? 0) * 1e3;
  return ms;
};
var interval = async function* (produce, opts = {}) {
  let cancelled = false;
  const signal = opts.signal;
  const when = opts.delay ?? "before";
  let sleepMs = intervalToMs(opts.fixedIntervalMs) ?? intervalToMs(opts.minIntervalMs, 0);
...
// src/flow/Delay.ts
var delay = async (callback, opts) => {
  await sleep(opts);
  return Promise.resolve(await callback());
};
async function* delayIterable(iter, opts) {
...
var addKeepingExisting = (set3, hashFunc, ...values) => {
  const s = set3 === void 0 ? /* @__PURE__ */ new Map() : new Map(set3);
  values.forEach((v) => {
...
function* filter(map4, predicate) {
...
var find = (map4, predicate) => Array.from(map4.values()).find(predicate);
...
var trimStack = (opts, stack2, toAdd) => {
  ...
  if (opts.debug) {
    console.log(
      `Stack.push: stackLen: ${stack2.length} potentialLen: ${potentialLength} toRemove: ${toRemove} policy: ${policy}`
    );
  }
  switch (policy) {
    case `additions`:
      if (opts.debug) {
        console.log(
          `Stack.push:DiscardAdditions: stackLen: ${stack2.length} slice: ${potentialLength - capacity} toAddLen: ${toAdd.length}`
        );
      }
    ...
    case `newer`:
      if (toRemove >= stack2.length) {
        ...
        return [
          ...stack2.slice(0, stack2.length - toRemove),
...
// src/collections/Stack.ts
var StackImpl = class _StackImpl {
  opts;
  /* eslint-disable-next-line functional/prefer-readonly-type */
  data;
  ...
  push(...toAdd) {
    return new _StackImpl(this.opts, push(this.opts, this.data, ...toAdd));
  }
  pop() {
    return new _StackImpl(this.opts, pop(this.opts, this.data));
  }
  forEach(fn) {
...
var Bezier = class _Bezier {
  ...
  lineIntersects(line3) {
    const mx = min(line3.p1.x, line3.p2.x), my = min(line3.p1.y, line3.p2.y), MX = max(line3.p1.x, line3.p2.x), MY = max(line3.p1.y, line3.p2.y);
...
// src/flow/Poll.ts
async function* iterableFromPoll(poll, optsOrMillis) {
  const sampleRateMs = typeof optsOrMillis === `number` ? optsOrMillis : optsOrMillis.sampleRateMs;
  const signal = typeof optsOrMillis === `number` ? void 0 : optsOrMillis.signal;
  while (true) {
    const v = poll();
    ...
    if (signal?.aborted)
      throw new Error(`Cancelling due to signal`);
  }
}
...
var min3 = (...data) => min2(data);
var max3 = (...data) => max2(data);
...
async function max4(it, gt = (a, b) => a > b) {
  let max6;
  for await (const v of it) {
    if (!max6) {
      max6 = v;
...
async function min4(it, gt = (a, b) => a > b) {
  let min6;
  for await (const v of it) {
    if (!min6) {
      min6 = v;
...
async function toArray3(it, count2 = Infinity) {
  const result = [];
...
var interpolate2 = (amount, from2, to, optsOrSpace) => {
...
var create3 = (fallbacks) => new PaletteImpl(fallbacks);
...
var setMutable = (keyString) => new MutableStringSet(keyString);
var MutableStringSet = class extends SimpleEventEmitter {
  // ✔ UNIT TESTED
...
// src/collections/set/StringSetImpl.ts
var StringSet = class _StringSet {
  ...
  add(...values) {
    ...
    return new _StringSet(this.keyString, s);
  }
  delete(v) {
    const s = new Map(this.store);
    const key = this.keyString(v);
...
var set = (keyString) => new StringSet(keyString);
...
var pointerVisualise = (elOrQuery, opts = {}) => {
  ...
  const trackPointer = async (ev) => {
    ...
    const el2 = SvgElements_exports.circle(
...
var create4 = (opts = {}) => new Pool(opts);
...
var time = function(nameOrFn, durationMs) {
  return create5(nameOrFn, durationMs, msElapsedTimer);
};
var tick = function(nameOrFn, durationTicks) {
  return create5(nameOrFn, durationTicks, ticksElapsedTimer);
};
var create5 = function(nameOrFn, duration, timerSource) {
  let fn;
...
var immutable = (dataOrMap) => {
  ...
  if (Array.isArray(dataOrMap))
    return immutable(add2(/* @__PURE__ */ new Map(), ...dataOrMap));
...
tslib/tslib.es6.js:
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=chunk-TNAMZGY5.js.map