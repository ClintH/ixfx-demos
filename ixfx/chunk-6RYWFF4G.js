...
var ellipse = (ctx, ellipsesToDraw, opts = {}) => {
  ...
  if (Array.isArray(ellipsesToDraw)) {
    for (const ellipse2 of ellipsesToDraw) {
      draw2(ellipse2);
    }
  } else
...
var triangle = (ctx, toDraw, opts = {}) => {
  applyOpts(ctx, opts);
  const draw2 = (t) => {
    connectedPoints(ctx, corners(t), { ...opts, loop: true });
    if (opts.debug) {
      pointLabels(ctx, corners(t), void 0, [`a`, `b`, `c`]);
    }
  };
  if (Array.isArray(toDraw)) {
    for (const t of toDraw) {
...
var rect = (ctx, toDraw, opts = {}) => {
  ...
  const draw2 = (d) => {
    ...
    if (opts.debug) {
      pointLabels(ctx, corners2(d), void 0, [`NW`, `NE`, `SE`, `SW`]);
    }
  };
  if (Array.isArray(toDraw)) {
    for (const t of toDraw) {
...
var Box = class {
  ...
  *getChildren() {
  ...
  layoutApply(l) {
    ...
    const different = this._layoutPosition === void 0 ? true : !points_exports.isEqual(l.actual, this._layoutPosition);
...
var CanvasBox = class _CanvasBox extends Box {
  ...
  notifyClick(p) {
    ...
    if (intersectsPoint(this.canvasRegion, p)) {
      const pp = points_exports.subtract(p, this.canvasRegion.x, this.canvasRegion.y);
      this.onClick(pp);
      for (const c of this.children)
        c.notifyClick(pp);
    }
...
var immutable = (startingValueOrBipolar = 0) => {
  ...
  return {
    ...
    interpolate: (amt, b) => {
      return immutable(clamp3(interpolate(amt, v, b)));
    },
    asScalar: () => {
      return toScalar(v);
    }
...
var scaleClamped = (inputValue, inMin, inMax) => {
...
//# sourceMappingURL=chunk-6RYWFF4G.js.map