...
var toD3Colour = (colour) => {
  ...
  throw new Error(`Cannot convert to d3 colour: ${JSON.stringify(c)}`);
};
var scale = (steps, space, colours) => {
  const colours_ = colours.map(toD3Colour);
  let inter;
  switch (space) {
    default:
      inter = rgbBasis(colours_);
  }
  if (inter === void 0)
...
var cubicBezier = (ctx, bezierToDraw, opts = {}) => {
  ...
  if (isDebug) {
    stack2 = stack2.push(optsOp({
      ...opts,
      strokeStyle: (void 0)(opts.strokeStyle ?? `silver`, 0.6),
      fillStyle: (void 0)(opts.fillStyle ?? `yellow`, 0.4)
    }));
    stack2.apply();
    ctx.moveTo(a.x, a.y);
