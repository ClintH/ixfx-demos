...
var sine = (timerSrc) => {
  ...
  return {
    get value() {
      return Math.abs(Math.sin(t.elapsed * Math.PI * 2)) * 2 - 1;
    }
  };
};
var sineBipolar = (timerSrc) => {
  const t = timerSrc();
  return {
    get value() {
      return Math.sin(t.elapsed * Math.PI * 2);
    }
  };
};
var triangle = (timerSrc) => {
  const t = timerSrc();
  return {
    get value() {
      let v = t.elapsed;
