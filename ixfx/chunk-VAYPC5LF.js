  ...
  const resolveBindUpdater = (bind2) => {
    ...
    if (bind2.cssVariable !== void 0) {
      ...
      if (!css.startsWith(`--`))
        css = `--` + css;
      return (v) => {
        bind2.element.style.setProperty(css, v);
      };
    }
    if (bind2.cssProperty !== void 0) {
      return (v) => {
        bind2.element.style[bind2.cssProperty] = v;
      };
    }
    return (_) => {
    };
...
var elements = (source, container) => {
  const containerEl = resolveEl(container);
  const elByField = /* @__PURE__ */ new Map();
  const create = () => {
    const el = document.createElement(`div`);
    el.textContent = d.value;
    containerEl.append(el);
    elByField.set(d.path, el);
  };
  source.onDiff((message) => {
    console.log(`diff`, message);
    if (message.value) {
      for (const d2 of message.value) {
        if (d2.previous === void 0) {
        } else if (d2.value === void 0) {
          const el = elByField.get(d2.path);
          if (el === void 0) {
            console.warn(`No element to delete? ${d2.path}`);
          } else {
            el.remove();
          }
        } else {
          const el = elByField.get(d2.path);
          if (el === void 0) {
            console.warn(`No element to update? ${d2.path}`);
          } else {
            el.textContent = d2.value;
          }
        }
      }
    }
