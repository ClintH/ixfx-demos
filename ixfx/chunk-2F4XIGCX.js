...
__export(ReactiveDom_exports, {
  ...
  bindCssVariable: () => bindCssVariable,
...
var bindAttribute = (elOrQuery, source, attribute, bindOpts = {}) => {
  return bind(elOrQuery, source, { ...bindOpts, attribName: attribute });
};
var bindCssVariable = (elOrQuery, source, cssVariable, bindOpts = {}) => {
  return bind(elOrQuery, source, { ...bindOpts, cssVariable });
};
var create = (source, options = {}) => {
  ...
  if (options.parentEl) {
    ...
    if (parentElementOrQuery === void 0)
      throw new Error(`Parent element could not be resolved`);
    parentElementOrQuery.append(el);
  }
  return b;
};
var bind = (elOrQuery, source, bindOpts = {}) => {
  const el = resolveEl(elOrQuery);
  const sourceField = bindOpts.sourceField;
  const transformer = bindOpts.transform ?? ((v) => {
    if (typeof v === `object`)
  ...
  if (bindOpts.elField !== void 0 || bindOpts.cssVariable === void 0 && bindOpts.attribName === void 0) {
    const field2 = bindOpts.elField ?? `textContent`;
    update = (v) => {
      el[field2] = v;
    };
  }
  if (bindOpts.attribName !== void 0) {
    const attrib = bindOpts.attribName;
    update = (v) => {
      el.setAttribute(attrib, v);
    };
  }
  if (bindOpts.cssVariable !== void 0) {
...
var to = (a, b, transform3, closeBonA = false) => {
  const unsub = a.on((message) => {
    if (messageHasValue(message)) {
      b.set(transform3(message.value));
...
function event(target, name, options = {}) {
  ...
  const initialValue2 = process ? process() : void 0;
  const rxObject = initialValue2 ? object(initialValue2, { deepEntries: true }) : object(void 0, { deepEntries: true });
  const lazy2 = options.lazy ?? false;
...
function object(initialValue2, options = {}) {
...
function asValue(valueToWrap, initialValue2) {
  let lastValue = initialValue2;
  let awaiting = false;
  const outputType = typeof valueToWrap === `function` ? valueToWrap() : valueToWrap;
  function asValue2() {
    if (!awaiting) {
...
__export(Pipe_exports, {
  ...
  isReadableValue: () => isReadableValue,
  merge: () => merge,
  number: () => number2,
  object: () => object2,
  split: () => split
...
// src/data/PipeDom.ts
var PipeDom_exports = {};
__export(PipeDom_exports, {
  bind: () => bind2,
  bindAttribute: () => bindAttribute2,
  bindCssVariable: () => bindCssVariable2,
  bindHtmlContent: () => bindHtmlContent2,
  bindTextContent: () => bindTextContent2,
  bindUpdate: () => bindUpdate2,
  create: () => create3,
  createUpdate: () => createUpdate2
});
var bindTextContent2 = (elOrQuery, source, bindOpts = {}) => {
  return bind2(elOrQuery, source, { ...bindOpts, elField: `textContent` });
};
var bindHtmlContent2 = (elOrQuery, source, bindOpts = {}) => {
  return bind2(elOrQuery, source, { ...bindOpts, elField: `innerHTML` });
};
var bindAttribute2 = (elOrQuery, source, attribute, bindOpts = {}) => {
  return bind2(elOrQuery, source, { ...bindOpts, attribName: attribute });
};
var bindCssVariable2 = (elOrQuery, source, cssVariable, bindOpts = {}) => {
  return bind2(elOrQuery, source, { ...bindOpts, cssVariable });
};
var create3 = (source, options = {}) => {
  const nodeType = options.tagName ?? `DIV`;
  const el = document.createElement(nodeType);
  const b = bind2(el, source, options);
  if (options.parentEl) {
    const parentElementOrQuery = resolveEl(options.parentEl);
    if (parentElementOrQuery === void 0)
      throw new Error(`Parent element could not be resolved`);
...
var bind2 = (elOrQuery, source, bindOpts = {}) => {
  const el = resolveEl(elOrQuery);
  const sourceField = bindOpts.sourceField;
  const transformer = bindOpts.transform ?? ((v) => {
    if (typeof v === `object`)
  ...
  const sub = source.on.value((v) => {
    if (sourceField)
      update(transformer(v[sourceField]));
    else
      update(transformer(v));
  });
...
var bindUpdate2 = (elOrQuery, source, updater) => {
  const el = resolveEl(elOrQuery);
  const update = (v) => {
    updater(v, el);
  };
...
var createUpdate2 = (source, updater, options = {}) => {
  const tag = options.tagName ?? `DIV`;
  const el = document.createElement(tag);
  if (options.parentEl) {
    const parent = resolveEl(options.parentEl);
  ...
  const b = bindUpdate2(el, source, updater);
  return b;
};

// src/data/Pipe.ts
var isReadableValue = (r) => {
  return `value` in r;
};
var number2 = (initialValue2) => {
  const r = new ReadWriteValue(initialValue2);
  return r;
};
var object2 = (initialValue2) => {
  const r = new ReadWriteObject(initialValue2);
  return r;
};
var event2 = (target, name) => {
  const callback = (args) => {
  ...
  const pipe = new ReadWriteStream({ closed });
  return pipe;
};
var merge = (...readables) => {
...
var initialValue = (value) => {
  const pipe = new ReadWriteObject(value);
  return pipe;
};
var ReadWriteStream = class {
  ...
  constructor(opts) {
    this.#opts = opts;
    const eventsReadable = initReadableEvents();
  ...
  push(message) {
    ...
    if (message.signal === `closed`) {
      if (this.#closed)
        return;
      if (this.#opts.closed)
        this.#opts.closed(`closed signal recv`);
      this.#closed = true;
    }
...
var initStreamEvents = () => {
  let _closed;
  const closed = (handler, options = {}) => {
    if (_closed === void 0)
      _closed = new DispatchList();
    const id = _closed.add(handler, options);
    return { id, source: menubar, off: () => {
      _closed?.remove(id);
    } };
  };
...
var initReadableEvents = () => {
  let _messages;
  let _values;
  const me = new Object();
  const value = (handler, options = {}) => {
    if (_values === void 0)
      _values = new DispatchList();
  ...
  const message = (handler, options = {}) => {
    if (_messages === void 0)
      _messages = new DispatchList();
...
var ReadWriteValue = class {
  _value;
  on;
  dispatch;
  #closed = false;
  constructor(initialValue2) {
    this._value = initialValue2;
    const events = initReadableEvents();
    this.on = {
      value: events.value,
  ...
  set value(value) {
    if (this.#closed)
      throw new Error(`ReadWriteValue closed`);
    this.push({ value });
  }
  get value() {
    return this._value;
  ...
  push(message) {
    if (this.#closed)
      throw new Error(`ReadWriteValue cannot receive messages when closed`);
    if (message.signal === `closed`) {
      this.#closed = true;
    }
    if (message.value !== void 0) {
...
var ReadWriteObject = class {
  ...
  push(message) {
    ...
    const r = compareData2(this._value, v);
    if (!r.hasChanged)
      return;
    this.setLastValue(v);
...
//# sourceMappingURL=chunk-2F4XIGCX.js.map