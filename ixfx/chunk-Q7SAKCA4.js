...
var floatSource = (maxOrOptions = 1) => {
  ...
  let max2 = options.max;
  let min2 = options.min ?? 0;
  const source = options.source ?? defaultRandom;
  throwFromResult(numberTest(min2, ``, `min`));
  throwFromResult(numberTest(max2, ``, `max`));
  if (!options.min && max2 < 0) {
    min2 = max2;
    max2 = 0;
  }
  if (min2 > max2) {
    throw new Error(`Min is greater than max. Min: ${min2} max: ${max2}`);
  }
  return () => source() * (max2 - min2) + min2;
};
...
var weightedIntegerSource = (maxOrOptions) => {
  ...
  throwFromResult(numberTest(max2));
  const easingFunction = get(easingName);
  if (easingFunction === void 0) {
    throw new Error(`Easing '${easingName}' not found`);
  }
  throwFromResult(numberTest(min2));
  if (max2 <= min2)
...
var integerSource = (maxOrOptions) => {
  ...
  const options = typeof maxOrOptions === `number` ? { max: maxOrOptions } : maxOrOptions;
  let max2 = Math.floor(options.max);
  let min2 = Math.floor(options.min ?? 0);
  if (!options.min && max2 < 0) {
    max2 = 1;
    min2 = options.max;
  }
  ...
  if (min2 > max2) {
    throw new Error(`Min value is greater than max (min: ${min2} max: ${max2})`);
  }
  throwFromResult(numberTest(min2, ``, `min`));
  throwFromResult(numberTest(max2, ``, `max`));
  if (max2 === min2) {
    throw new Error(`Max and min values cannot be the same (${max2})`);
  }
  const amt = Math.abs(max2 - min2);
  return () => Math.floor(randomSource() * amt) + min2;
};
...
var minutesMsSource = (maxMinutesOrOptions) => {
  ...
  const min2 = (options.min ?? 0) * 60 * 1e3;
  const max2 = options.max * 60 * 1e3;
  return integerSource({ ...options, max: max2, min: min2 });
};
...
var secondsMsSource = (maxSecondsOrOptions) => {
  ...
  const min2 = (options.min ?? 0) * 1e3;
  const max2 = options.max * 1e3;
  return () => integer({ ...options, max: max2, min: min2 });
};
...
function* integerUniqueGen(maxOrOptions) {
  ...
  throwFromResult(integerTest(min2, ``, `min`));
  throwFromResult(integerTest(max2, ``, `max`));
  if (min2 > max2) {
    throw new Error(`Min value is greater than max. Min: ${min2} Max: ${max2}`);
  }
  const origRange = [...range(min2, max2 - min2)];
...
//# sourceMappingURL=chunk-Q7SAKCA4.js.map