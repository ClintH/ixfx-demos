...
var textBlockCentered = (ctx, lines, opts) => {
  const { bounds } = opts;
  applyOpts(ctx, opts);
  ctx.save();
...
var scaleWithFixedRange = (buffer, range) => calcScale(buffer).map((s) => ({ ...s, range: range[1] - range[0], min: range[0], max: range[1] }));
var draw = (buffer, drawing) => {
  ...
  const series = drawing.y.scaleRange ? scaleWithFixedRange(buffer, drawing.y.scaleRange) : calcScale(buffer);
  console.log(drawing);
  ...
  ctx.translate(0, canvasSize.height / 2 - (plotSize.height + drawing.margin + drawing.margin) / 2);
  if (drawing.debug) {
  ...
  if (yAxis.showLabels || yAxis.showLine) {
    ...
    drawYLine(axisSize, drawing);
  }
  if (xAxis.showLabels || xAxis.showLine) {
    drawXAxis(axisSize, drawing);
  }
  ...
  series.forEach((s) => {
    ...
    if (data2 === void 0)
      return;
    let leadingEdgeIndex = buffer.typeName === `circular` ? data2.pointer - 1 : data2.length - 1;
    if (drawing.highlightIndex !== void 0)
      leadingEdgeIndex = drawing.highlightIndex;
    drawSeriesData(s, data2, plotSize, plotDrawing, leadingEdgeIndex);
  });
  ctx.resetTransform();
};
var drawYSeriesScale = (series, plotSize, drawing) => {
  const { ctx, palette, y } = drawing;
  const { width, height } = plotSize;
  if (drawing.debug) {
    ctx.strokeStyle = `green`;
    ctx.strokeRect(0, 0, y.textSize, height);
  }
  ...
  ctx.translate(y.textSize, 0);
};
var drawYLine = (plotSize, drawing) => {
  const { ctx, palette, y } = drawing;
  const { width, height } = plotSize;
  ctx.translate(drawing.margin, 0);
...
var drawXAxis = (plotSize, drawing) => {
  const { width, height } = plotSize;
  const { ctx, palette, x, y } = drawing;
  if (!x.showLine)
    return;
...
var drawSeriesData = (series, values, plotSize, drawing, leadingEdgeIndex) => {
  const { ctx, dataXScale = 1, lineWidth = 2, translucentPlot = false } = drawing;
  ...
  if (style === `dots`) {
    ctx.fillStyle = colourTransform(drawing.palette.getOrAdd(`series${series.name}`));
  } else if (style === `none`) {
  } else {
    ctx.beginPath();
    ctx.lineWidth = lineWidth;
    ctx.strokeStyle = colourTransform(drawing.palette.getOrAdd(`series${series.name}`));
  }
...
var calcSizing = (margin, y, x) => {
  ...
  if (x.showLabels || x.showLine)
    fromBottom += margin;
  console.log(x);
  console.log(`fromTop: ${fromTop} fromBottom: ${fromBottom} x.lineWidth: ${x.lineWidth} margin: ${margin} x.showLabels: ${x.showLabels}`);
  return {
    left: fromLeft,
    right: fromRight,
    top: fromTop,
...
var plot = (parentElOrQuery, opts) => {
  ...
  if (plotSize) {
    ...
    console.log(`Canvas size from plot: ${canvasSize2.width},${canvasSize2.height} plot: ${plotSize.width}, ${plotSize.height}`);
