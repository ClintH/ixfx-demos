...
var fromArray = (v) => {
  if (v.length === 3) {
    ...
    return { r: v[0] / 255, g: v[1] / 255, b: v[2] / 255, a: v[3] };
  } else {
...
var rgbaFromArray = (v) => {
  ...
  return { r: v[0] / 255, g: v[1] / 255, b: v[2] / 255, a: v[3] / 255 };
};
var guard2 = (rgb) => {
  if (hasAlpha(rgb)) {
    number(rgb.a, `percentage`, `rgb.a`);
  }
...
var lerp2 = (amount, from, to) => {
  ...
  if (hasAlpha(from) || hasAlpha(to)) {
    const fromA = hasAlpha(from) ? from.a : 1;
    const toA = hasAlpha(to) ? to.a : 1;
    const a = lerp(amount, fromA, toA);
    rgb = Object.freeze({ r, g, b, a });
  } else {
...
var toCss = (v) => {
  if (hasAlpha(v)) {
    return `rgba(${makeAbs(v.r)},${makeAbs(v.g)},${makeAbs(v.b)}, ${v.a})`;
  } else {
...
var lerp4 = (amount, from, to) => {
  ...
  if (hasAlpha(from) || hasAlpha(to)) {
    ...
    const alpha2 = lerp(amount, fromA, toA);
    lab = Object.freeze({ l, a, b, alpha: alpha2 });
  } else {
