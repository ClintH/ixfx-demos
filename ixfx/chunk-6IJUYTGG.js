...
var interpolate3 = (amount, a, b) => interpolate2(amount, a, b);
var from = (xOrArray, y) => {
...
var length2 = (arc) => piPi2 * arc.radius * ((arc.startRadian - arc.endRadian) / piPi2);
var bbox3 = (arc) => {
  if (isPositioned(arc)) {
...
__export(Bezier_exports, {
  ...
  normalisedCubic: () => normalisedCubic,
  quadratic: () => quadratic,
...
var normalisedCubic = (b, d, t2) => {
  const s = 1 - t2;
  const t22 = t2 * t2;
  const s2 = s * s;
  const t3 = t22 * t2;
  return 3 * b * s2 * t2 + 3 * d * s * t22 + t3;
};
...
var interpolate5 = (circle, t2) => point2(circle, t2 * piPi3);
var length3 = (circle) => piPi3 * circle.radius;
var bbox4 = (circle) => {
  if (isPositioned2(circle)) {
...
var toPath4 = (circle) => {
  ...
  return Object.freeze({
    ...circle,
    interpolate: (t2) => interpolate5(circle, t2),
    bbox: () => bbox4(circle),
...
var interpolate6 = (paths, t2, useWidth, dimensions) => {
...
var fromPaths = (...paths) => {
  ...
  return Object.freeze({
    ...
    interpolate: (t2, useWidth = false) => interpolate6(paths, t2, useWidth, dims),
    bbox: () => bbox5(paths),
...
var pi3 = Math.PI;
var sin3 = Math.sin;
var time = function(name, durationMs) {
  return create(name, durationMs, msElapsedTimer);
};
var tick = function(name, durationTicks) {
...
var functions = {
  arch,
  bell: (x) => Bezier_exports.normalisedCubic(1.5, -1.25, x),
