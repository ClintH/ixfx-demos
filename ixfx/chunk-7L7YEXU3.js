...
function wrap(source) {
  return {
    ...
    switcher: (cases, options = {}) => {
      const s = switcher(cases, source, options);
...
var bindElement = (source, elOrQuery, ...binds) => {
  console.log(`elOrQuery:`, elOrQuery);
  const el = resolveEl(elOrQuery);
  if (binds.length === 0) {
    binds.push({ elField: `textContent` });
  }
  binds = binds.map((bind2) => {
    if (bind2.element !== void 0)
      return bind2;
    console.log(`adding el`, el);
...
var bind = (source, ...bindsUnresolvedElements) => {
  ...
  const resolveBindUpdater = (bind2) => {
    ...
    if (bind2.textContent) {
      return (v) => {
        bind2.element.textContent = v;
      };
    }
    if (bind2.htmlContent) {
      return (v) => {
        bind2.element.innerHTML = v;
      };
    }
