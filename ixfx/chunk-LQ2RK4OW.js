...
var clampMagnitude = (v, max4 = 1, min4 = 0) => {
  let mag = v.distance;
  if (mag > max4)
    mag = max4;
  if (mag < min4)
...
// src/geometry/points/Distance.ts
...
var toPositioned = (circle, defaultPositionOrX, y) => {
  if (isCirclePositioned(circle))
    return circle;
  const pt = points_exports.getPointParameter(defaultPositionOrX, y);
  return Object.freeze({
    ...circle,
    ...pt
  });
...
// src/geometry/circle/index.ts
var piPi = Math.PI * 2;
var point = (circle, angleRadian2, origin) => {
  if (origin === void 0) {
...
var interpolate = (circle, t2) => point(circle, t2 * piPi);
var length = (circle) => circumference(circle);
...
var bbox = (circle) => {
  return isCirclePositioned(circle) ? fromCenter(circle, circle.radius * 2, circle.radius * 2) : { width: circle.radius * 2, height: circle.radius * 2 };
};
var isEqual = (a, b) => {
...
var randomPoint = (within, opts = {}) => {
  ...
  switch (strategy) {
    case `naive`: {
      return points_exports.sum(offset2, Polar_exports.toCartesian(rand() * within.radius, rand() * piPi));
    }
    case `uniform`: {
      return points_exports.sum(offset2, Polar_exports.toCartesian(Math.sqrt(rand()) * within.radius, rand() * piPi));
    }
    default: {
...
function multiplyScalar(a, value) {
  if (isCirclePositioned(a)) {
    const pt = points_exports.multiplyScalar(a, value);
    return Object.freeze({
      ...a,
      ...pt,
...
var nearest = (circle, b) => {
  ...
  if (Array.isArray(circle)) {
    ...
    const dists = pts.map((p) => points_exports.distance(p, b));
...
var toPath = (circle) => {
  ...
  return Object.freeze({
    ...circle,
    nearest: (point3) => nearest(circle, point3),
    /**
     ...
     * @returns {Point} X,y
     */
    interpolate: (t2) => interpolate(circle, t2),
    bbox: () => bbox(circle),
...
var normaliseByRect = (rect, normaliseByOrWidth, height4) => {
  let width;
  if (height4 === void 0) {
...
var quadraticSimple = (start, end, bend = 0) => {
  ...
  const middle = interpolate2(0.5, start, end);
  let target = middle;
  if (end.y < start.y) {
  ...
  const handle = interpolate2(Math.abs(bend), middle, target);
...
var constrainBounce = (bounds, dampen = 1) => {
  ...
  return (t2) => {
    const position = computePositionFromVelocity(
      t2.position ?? points_exports.Empty,
      t2.velocity ?? points_exports.Empty
    );
    let velocity = t2.velocity ?? points_exports.Empty;
    let { x, y } = position;
    if (x > maxX) {
      x = maxX;
      velocity = points_exports.invert(points_exports.multiply(velocity, dampen), `x`);
    } else if (x < minX) {
      x = minX;
      velocity = points_exports.invert(points_exports.multiply(velocity, dampen), `x`);
    }
    if (y > maxY) {
      y = maxY;
      velocity = points_exports.multiply(points_exports.invert(velocity, `y`), dampen);
    } else if (position.y < minY) {
      y = minY;
      velocity = points_exports.invert(points_exports.multiply(velocity, dampen), `y`);
    }
    return Object.freeze({
      ...t2,
      position: { x, y },
...
var attractionForce = (attractors, gravity, distanceRange = {}) => (attractee) => {
  let accel = attractee.acceleration ?? points_exports.Empty;
  for (const a of attractors) {
    if (a === attractee)
      continue;
    const f = computeAttractionForce(a, attractee, gravity, distanceRange);
    accel = points_exports.sum(accel, f);
  }
  return {
    ...attractee,
    acceleration: accel
...
var computeAttractionForce = (attractor, attractee, gravity, distanceRange = {}) => {
  ...
  const f = points_exports.normalise(
    points_exports.subtract(attractor.position, attractee.position)
  );
  const d = clamp(points_exports.distance(f), distributionRangeMin, distributionRangeMax);
  return points_exports.multiply(
    f,
...
var targetForce = (targetPos, opts = {}) => {
  const fn = (t2) => {
    ...
    return {
      ...t2,
      acceleration: points_exports.sum(t2.acceleration ?? points_exports.Empty, accel)
    };
  };
  return fn;
};
var apply = (t2, ...accelForces) => {
  if (t2 === void 0)
  ...
  for (const f of accelForces) {
    ...
    t2 = typeof f === `function` ? f(t2) : {
      ...t2,
      acceleration: points_exports.sum(t2.acceleration ?? points_exports.Empty, f)
    };
  }
  const velo = computeVelocity(
    t2.acceleration ?? points_exports.Empty,
    t2.velocity ?? points_exports.Empty
  );
  const pos = computePositionFromVelocity(t2.position ?? points_exports.Empty, velo);
  const ff = {
    ...t2,
    position: pos,
    velocity: velo,
    // Clear accel, because it has been integrated into velocity
    acceleration: points_exports.Empty
  };
  return ff;
};
...
var massApplyAccel = (vector, thing, mass = `ignored`) => {
  ...
  switch (mass) {
    case `dampen`: {
      op = (mass2) => points_exports.divide(vector, mass2, mass2);
      break;
    }
    case `multiply`: {
      op = (mass2) => points_exports.multiply(vector, mass2, mass2);
      break;
    }
    case `ignored`: {
      op = (_mass) => vector;
  ...
  return points_exports.sum(thing.acceleration ?? points_exports.Empty, op(thing.mass ?? 1));
};
var magnitudeForce = (force, mass = `ignored`) => (t2) => {
  if (t2.velocity === void 0)
    return t2;
  const mag = points_exports.distance(points_exports.normalise(t2.velocity));
  const magSq = force * mag * mag;
  const vv = points_exports.multiply(points_exports.invert(t2.velocity), magSq);
  return Object.freeze({
    ...t2,
...
var velocityForce = (force, mass) => {
  const pipeline2 = points_exports.pipeline(
    // Points.normalise,
    points_exports.invert,
    (v) => points_exports.multiply(v, force)
  );
  return (t2) => {
    if (t2.velocity === void 0)
      return t2;
...
var angleFromAccelerationForce = (scaling = 20) => (t2) => {
  const accel = t2.acceleration ?? points_exports.Empty;
  return Object.freeze({
    ...t2,
...
var angleFromVelocityForce = (interpolateAmt = 1) => (t2) => {
  const a = points_exports.angle(t2.velocity ?? points_exports.Empty);
  return Object.freeze({
    ...t2,
...
var springForce = (pinnedAt, restingLength = 0.5, k = 2e-4, damping = 0.999) => (t2) => {
  const direction = points_exports.subtract(t2.position ?? points_exports.Empty, pinnedAt);
  const mag = points_exports.distance(direction);
  const stretch = Math.abs(restingLength - mag);
  const f = points_exports.pipelineApply(
    direction,
    points_exports.normalise,
    (p) => points_exports.multiply(p, -k * stretch)
  );
  const accel = massApplyAccel(f, t2, `dampen`);
  const velo = computeVelocity(
    accel ?? points_exports.Empty,
    t2.velocity ?? points_exports.Empty
  );
  const veloDamped = points_exports.multiply(velo, damping, damping);
  return {
    ...t2,
    velocity: veloDamped,
    acceleration: points_exports.Empty
...
var pendulumForce = (pinnedAt, opts = {}) => (t2) => {
  if (!pinnedAt)
    pinnedAt = { x: 0, y: 0 };
  const length5 = opts.length ?? points_exports.distance(pinnedAt, t2.position ?? points_exports.Empty);
  const speed = opts.speed ?? 1e-3;
  const damping = opts.damping ?? 0.995;
  let angle2 = t2.angle;
  if (angle2 === void 0) {
    if (t2.position) {
      angle2 = points_exports.angle(pinnedAt, t2.position) - Math.PI / 2;
    } else {
      angle2 = 0;
    }
  }
...
var computeVelocity = (acceleration, velocity, velocityMax) => {
  const p = points_exports.sum(velocity, acceleration);
  return velocityMax === void 0 ? p : points_exports.clampMagnitude(p, velocityMax);
};
var computeAccelerationToTarget = (targetPos, currentPos, opts = {}) => {
  const diminishBy = opts.diminishBy ?? 1e-3;
  const direction = points_exports.subtract(targetPos, currentPos);
  if (opts.range && // If direction is less than range, return { x: 0, y: 0}
  points_exports.compare(points_exports.abs(direction), opts.range) === -2) {
    return points_exports.Empty;
  }
  return points_exports.multiply(direction, diminishBy);
};
var computePositionFromVelocity = (position, velocity) => points_exports.sum(position, velocity);
...
// src/geometry/points/DistanceToCenter.ts
...
// src/geometry/points/DistanceToExterior.ts
...
var clampMagnitude2 = (pt, max4 = 1, min4 = 0) => {
...
var isEmpty2 = (l) => isEmpty(l.a) && isEmpty(l.b);
...
var isEqual4 = (a, b) => isEqual2(a.a, b.a) && isEqual2(a.b, b.b);
var apply3 = (line, fn) => Object.freeze(
  {
    ...line,
    a: fn(line.a),
    b: fn(line.b)
...
var multiply4 = (line, point3) => Object.freeze({
  ...line,
  a: multiply3(line.a, point3),
  b: multiply3(line.b, point3)
});
var divide3 = (line, point3) => Object.freeze({
  ...line,
  a: divide2(line.a, point3),
  b: divide2(line.b, point3)
});
var sum3 = (line, point3) => Object.freeze({
  ...line,
  a: sum(line.a, point3),
  b: sum(line.b, point3)
});
var subtract3 = (line, point3) => Object.freeze({
  ...line,
  a: subtract(line.a, point3),
  b: subtract(line.b, point3)
});
var normaliseByRect3 = (line, width, height4) => Object.freeze({
  ...line,
  a: normaliseByRect2(line.a, width, height4),
  b: normaliseByRect2(line.b, width, height4)
});
var withinRange2 = (line, point3, maxRange) => {
...
var perpendicularPoint = (line, distance3, amount = 0) => {
  const origin = interpolate2(amount, line);
...
var scaleFromMidpoint = (line, factor) => {
  const a = interpolate2(factor / 2, line);
  const b = interpolate2(0.5 + factor / 2, line);
  return { a, b };
};
var pointAtX = (line, x) => {
...
var distanceSingleLine = (line, point3) => {
  ...
  guard(point3, `point`);
  if (length2(line) === 0) {
    return length2(line.a, point3);
  }
  const near = nearest2(line, point3);
...
var fromFlatArray = (array) => {
  ...
  return fromNumbers3(array[0], array[1], array[2], array[3]);
};
var fromPoints = (a, b) => {
  guard(a, `a`);
  guard(b, `b`);
...
var fromPointsToPath = (a, b) => toPath3(fromPoints(a, b));
var bbox3 = (line) => bbox2(line.a, line.b);
var toPath3 = (line) => {
  const { a, b } = line;
  return Object.freeze({
    ...line,
    ...
    toPoints: () => [a, b],
    rotate: (amountRadian, origin) => toPath3(rotate3(line, amountRadian, origin)),
    nearest: (point3) => nearest2(line, point3),
    sum: (point3) => toPath3(sum3(line, point3)),
    divide: (point3) => toPath3(divide3(line, point3)),
    multiply: (point3) => toPath3(multiply4(line, point3)),
    subtract: (point3) => toPath3(subtract3(line, point3)),
    midpoint: () => midpoint(a, b),
    distance: (point3) => distanceSingleLine(line, point3),
    parallel: (distance3) => parallel(line, distance3),
    perpendicularPoint: (distance3, amount) => perpendicularPoint(line, distance3, amount),
    slope: () => slope(line),
    withinRange: (point3, maxRange) => withinRange2(line, point3, maxRange),
    isEqual: (otherLine) => isEqual4(line, otherLine),
    apply: (fn) => toPath3(apply3(line, fn)),
    kind: `line`
  });
};
var rotate3 = (line, amountRadian, origin) => {
  ...
  if (typeof origin === `number`) {
    origin = interpolate2(origin, line.a, line.b);
  }
  return Object.freeze({
    ...line,
...
var starburst = (outerRadius, points = 5, innerRadius, origin = points_exports.Empty, opts) => {
...
var arrow = (origin, from2, opts = {}) => {
  ...
  const arrow2 = points_exports.rotate(
    [
      tailPoints[0],
      tailPoints[1],
      tri.a,
...
var bbox4 = (paths) => {
  ...
  return points_exports.bbox(...corners3);
};
...
var guardContinuous = (paths) => {
  ...
  for (let index = 1; index < paths.length; index++) {
    ...
    if (!points_exports.isEqual(start, lastPos))
...
var toLine = (arc) => Line_exports.fromPoints(
  point2(arc, arc.startRadian),
  point2(arc, arc.endRadian)
);
var point2 = (arc, angleRadian2, origin) => {
...
var interpolate5 = (amount, arc, origin) => {
  guard7(arc);
  return point2(arc, arc.startRadian + (arc.endRadian - arc.startRadian) * amount, origin);
};
var toPath4 = (arc) => {
  guard7(arc);
  return Object.freeze({
...
var length4 = (arc) => piPi3 * arc.radius * ((arc.startRadian - arc.endRadian) / piPi3);
var bbox5 = (arc) => {
  if (isPositioned3(arc)) {
    const middle = interpolate5(0.5, arc);
    const asLine = toLine(arc);
    return bbox2(middle, asLine.a, asLine.b);
  } else {
    return {
      width: arc.radius * 2,
      height: arc.radius * 2
...
var outerCircle = (t2) => {
  const [a, b, c] = edges2(t2).map((l) => length2(l));
...
var isEmpty3 = (t2) => isEmpty(t2.a) && isEmpty(t2.b) && isEmpty(t2.c);
...
var fromRadius = (origin, radius, opts = {}) => {
  ...
  const angles2 = [
    initialAngleRadian,
    initialAngleRadian + piPi6 * 1 / 3,
    initialAngleRadian + piPi6 * 2 / 3
  ];
...
export {
  ...
  multiply3 as multiply2,
  relation,
  points_exports
};
//# sourceMappingURL=chunk-LQ2RK4OW.js.map