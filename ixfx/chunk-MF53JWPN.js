...
var getName = (t2, defaultValue = ``) => {
  if (typeof t2 === `object` && `name` in t2) {
    if (t2.name !== void 0) return t2.name;
  }
  return defaultValue;
};
function* iterateBreadth(t2, pathPrefix) {
  if (typeof pathPrefix === `undefined`) {
    pathPrefix = getName(t2);
  }
...
var retryTask = async (task, opts = {}) => {
  ...
  for (const t2 of backoffGen) {
    ...
    log({
      msg: `retry attempts: ${attempts} t: ${toString(t2)}`
    });
    if (attempts >= limitAttempts) {
      break;
    }
    try {
      await sleep({ millis: t2, signal });
    } catch (error) {
      return {
        success: false,
        attempts,
...
//# sourceMappingURL=chunk-MF53JWPN.js.map