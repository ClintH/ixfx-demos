...
var joinPointsToLines = (...points2) => {
  ...
  for (let i = 1; i < points2.length; i++) {
    lines.push(fromPoints(start, points2[i]));
    start = points2[i];
  }
  return lines;
};
...
var findMinimum = (compareFn, ...points2) => {
  if (points2.length === 0)
...
var intersections = (a, b) => {
  ...
  return [
    Point_exports.sum(centroid2, intersection),
    Point_exports.subtract(centroid2, intersection)
  ];
};
var isEquals2 = (a, b) => {
  if (a.radius !== b.radius)
...
var neighbours = (grid, cell, bounds = `undefined`, directions) => {
  ...
  const points2 = dirs.map((c) => offset(grid, cell, getVectorFromCardinal(c), bounds));
  return zipKeyValue(dirs, points2);
};
...
var starburst = (outerRadius, points2 = 5, innerRadius, origin = { x: 0, y: 0 }, opts) => {
  integer(points2, `positive`, `points`);
...
var fromRadius = (origin, radius, opts = {}) => {
  ...
  const points2 = angles2.map((a) => Polar_exports.toCartesian(radius, a, origin));
  return fromPoints2(points2);
};
var toFlatArray2 = (t2) => {
  guard6(t2);
  return [
...
var fromPoints2 = (points2) => {
  if (!Array.isArray(points2))
    throw new Error(`points expected as array`);
  if (points2.length !== 3)
    throw new Error(`points array expected with 3 elements. Got ${points2.length}`);
  const t2 = {
    a: points2[0],
    b: points2[1],
    c: points2[2]
...
//# sourceMappingURL=chunk-UDDWVRTZ.js.map