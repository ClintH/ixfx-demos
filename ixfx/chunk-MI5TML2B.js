...
var omitChars = (source, removeStart, removeLength) => {
  return source.substring(0, removeStart) + source.substring(removeStart + removeLength);
};
var splitByLength = (source, length) => {
  const chunks = Math.ceil(source.length / length);
  const ret = [];
  for (let i = 0, o = 0; i < chunks; ++i, o += size) {
  }
};
