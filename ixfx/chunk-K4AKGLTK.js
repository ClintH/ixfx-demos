...
var fromArray = (v) => {
  if (v.length === 3) {
    ...
    return { r: v[0] / 255, g: v[1] / 255, b: v[2] / 255, alpha: v[3] };
  } else {
...
var guard2 = (rgb) => {
  if (hasAlpha(rgb)) {
    number(rgb.alpha, `percentage`, `rgb.alpha`);
  }
  number(rgb.r, `percentage`, `rgb.r`);
  number(rgb.g, `percentage`, `rgb.g`);
  number(rgb.b, `percentage`, `rgb.b`);
};
var toRgb2 = (colour) => {
  if (isRgb(colour))
    return colour;
  if (isHsl(colour))
    ...
    throw new Error(`Cannot convert colour ${colour}`);
};
var lerp2 = (amount, from, to) => {
  number(amount, `percentage`, `amount`);
  from = toRgb2(from);
  to = toRgb2(to);
  guard2(from);
  guard2(to);
  const r = lerp(amount, from.r, to.r);
  const g = lerp(amount, from.g, to.g);
  const b = lerp(amount, from.b, to.b);
  let rgb;
  if (hasAlpha(from) || hasAlpha(to)) {
    const fromA = hasAlpha(from) ? from.alpha : 1;
    const toA = hasAlpha(to) ? to.alpha : 1;
    const alpha = lerp(amount, fromA, toA);
    rgb = Object.freeze({ r, g, b, alpha });
  } else {
...
var makeAbs = (v) => Math.floor(v * 255);
var toCss = (v) => {
  if (hasAlpha(v)) {
    return `rgba(${makeAbs(v.r)},${makeAbs(v.g)},${makeAbs(v.b)}, ${v.alpha})`;
  } else {
    return `rgb(${makeAbs(v.r)},${makeAbs(v.g)},${makeAbs(v.b)})`;
  }
};
var isRgb = (p) => {
  if (p.r === void 0)
...
// src/visual/colour/Hsl.ts
var guard3 = (hsl) => {
  number(hsl.h, `percentage`, `hsl.h`);
  number(hsl.s, `percentage`, `hsl.s`);
  number(hsl.l, `percentage`, `hsl.l`);
  if (`alpha` in hsl) {
    number(hsl.alpha, `percentage`, `hsl.alpha`);
  }
};
var fromArray2 = (a) => {
  let h, s, l;
  ...
  if (a.length == 4) {
    ...
    throw new Error(`Expected array length of 3 or 4`);
  }
  guard3(hsl);
  return hsl;
};
...
var toRgb = (hsl) => {
  ...
  if (hasAlpha(hsl))
    return { r, g, b, alpha: hsl.alpha };
  return { r, g, b };
};
var fromRgba = (rgba) => {
  guard3;
  const hsl = fromRgb(rgba);
  const hsla = { ...hsl, alpha: rgba.alpha };
  return hsla;
};
var fromRgb = (rgb) => {
  guard2(rgb);
...
var lerp3 = (amount, a, b) => {
  number(amount, `percentage`, `amount`);
  const h = wrapRange(0, 1, (max2) => lerp(amount, 0, max2), a.h, b.h);
...
var makeAbsH = (v) => Math.floor(v * 360);
var makeStr = (v) => Math.floor(v * 100) + `%`;
var toCss2 = (hsl) => {
  guard3(hsl);
  if (hasAlpha(hsl)) {
    return `hsl(${makeAbsH(hsl.h)}, ${makeStr(hsl.s)}, ${makeStr(hsl.l)}, ${makeStr(hsl.alpha)})`;
  } else {
    return `hsl(${makeAbsH(hsl.h)}, ${makeStr(hsl.s)}, ${makeStr(hsl.l)})`;
  }
};
var isHsla = (p) => {
  if (p.h === void 0)
...
var toRgb3 = (_xyz, white = max) => {
  white = white || whitepoint[2].E;
  let x = _xyz.x / white[0], y = _xyz.y / white[1], z = _xyz.z / white[2], r, g, b;
...
var fromRgb2 = (rgb, white = max) => {
  ...
  white = white || whitepoint[2].E;
  return { x: x * white[0], y: y * white[1], z: z * white[1] };
};
var toCss3 = (v) => {
...
var toLab = (c) => {
  if (isLab(c))
    return c;
  if (isHsl(c)) {
    return fromXyz(fromRgb2(toRgb(c)));
  } else if (Rgb_exports.isRgb(c)) {
    return fromXyz(fromRgb2(c));
  }
  throw new Error(`Unknown colour space: ${JSON.stringify(c)}`);
};
var lerp4 = (amount, from, to) => {
  number(amount, `percentage`, `amount`);
  from = toLab(from);
  to = toLab(to);
  const l = lerp(amount, from.l, to.l);
  const a = lerp(amount, from.a, to.a);
  const b = lerp(amount, from.b, to.b);
  let lab;
  if (hasAlpha(from) || hasAlpha(to)) {
    const fromA = hasAlpha(from) ? from.alpha : 1;
    const toA = hasAlpha(to) ? to.alpha : 1;
    const alpha = lerp(amount, fromA, toA);
    lab = Object.freeze({ l, a, b, alpha });
  } else {
    lab = Object.freeze({ l, a, b });
  }
  guard4(lab);
  return lab;
};
var toCss4 = (v) => {
  const r = toRgb3(toXyz(v));
  return Rgb_exports.toCss(r);
};
var guard4 = (lab) => {
  number(lab.l, `percentage`, `lab.l`);
  number(lab.a, ``, `lab.a`);
  number(lab.b, ``, `lab.b`);
};
...
var getNamed = (name, space = `hsl`) => {
  ...
  if (c === void 0)
    return c;
  if (space === `hsl`) {
    return fromArray2(c.hslValues);
  } else if (space === `rgb`) {
    fromArray(c.rgbValues);
  }
};
...
var opacity = (c, reduceBy) => {
  ...
  alpha = alpha * reduceBy;
  number(alpha, `percentage`, `alpha`);
  return toCss2({ ...parsed, alpha });
};
var toCss5 = (c) => {
  if (isRgb(c))
    return toCss(c);
    ...
    return toCss4(c);
  throw new Error(`Cannot CSSify unknown colour: ${JSON.stringify(c)}`);
};
var parseCss = (c) => {
