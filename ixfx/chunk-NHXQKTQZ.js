...
var normaliseByRect2 = (rect, normaliseByOrWidth, height4) => {
  let width;
  if (height4 === void 0) {
...
var clampMagnitude = (pt, max2 = 1, min2 = 0) => {
...
var clampMagnitude2 = (v, max2 = 1, min2 = 0) => {
  let mag = v.distance;
...
var isEqual4 = (a, b) => isEqual(a.a, b.a) && isEqual(a.b, b.b);

// src/geometry/line/Multiply.ts
var multiply4 = (line, point3) => Object.freeze({
  ...line,
  a: multiply2(line.a, point3),
  b: multiply2(line.b, point3)
});
...
var toPath = (line) => {
  ...
  return Object.freeze({
    ...
    sum: (point3) => toPath(sum3(line, point3)),
    divide: (point3) => toPath(divide2(line, point3)),
    multiply: (point3) => toPath(multiply4(line, point3)),
    subtract: (point3) => toPath(subtract3(line, point3)),
...
var interpolate3 = (circle, t2) => point(circle, t2 * piPi);
...
var computeDimensions = (paths) => {
  ...
  const lengths2 = paths.map((l) => l.length());
  let totalLength = 0;
  let totalWidth = 0;
  for (const length5 of lengths2) {
    totalLength += length5;
  }
  for (const width of widths) {
    totalWidth += width;
  }
  return { totalLength, totalWidth, widths, lengths: lengths2 };
};
var bbox4 = (paths) => {
...
var QuadTreeNode = class _QuadTreeNode {
  ...
  *children() {
  ...
  #subdivide() {
    ...
    const coords = fromNumbers3(x + w, y, x, y, x, y + h, x + w, y + h);
...
var fromPointPolar = (pt, angleNormalisation = ``, origin = EmptyCartesian2) => {
  ...
  if (angleNormalisation === `unipolar` && direction < 0) direction += piPi3;
    ...
    if (direction > pi3) direction -= piPi3;
    else if (direction <= -pi3) direction += piPi3;
  }
  return Object.freeze({
...
var area3 = (t2) => {
  guard7(t2, `t`);
  const lengths2 = edges2(t2).map((l) => length(l));
  const p = (lengths2[0] + lengths2[1] + lengths2[2]) / 2;
  return Math.sqrt(p * (p - lengths2[0]) * (p - lengths2[1]) * (p - lengths2[2]));
};
...
var fromFlatArray2 = (coords) => {
  ...
  return fromPoints3(fromNumbers3(...coords));
};
...
//# sourceMappingURL=chunk-NHXQKTQZ.js.map