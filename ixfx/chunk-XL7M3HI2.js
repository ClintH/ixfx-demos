...
async function* range(start3, length6) {
...
function* range2(start3, length6) {
  for (let index = 0; index < length6; index++) {
    yield start3++;
  }
}
...
function* unique3(iterable, keyFunction = toStringDefault) {
  const buffer = [];
  let itera = [];
...
var Box = class {
  ...
  hasChild(box) {
    ...
    return byRef !== void 0 || byId2 !== void 0;
  }
  notify(msg, source) {
...
var immutable3 = (startingValueOrBipolar = 0) => {
  ...
  return {
    ...
    interpolate: (amt, b) => {
      return immutable3(clamp2(interpolate3(amt, v, b)));
    },
    asScalar: () => {
      return toScalar(v);
    }
