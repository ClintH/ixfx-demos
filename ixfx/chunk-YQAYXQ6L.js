...
var randomPoint = (within, opts = {}) => {
  ...
  switch (strategy) {
    case `naive`: {
      return point_exports.sum(offset2, Polar_exports.toCartesian(rand() * radius, rand() * piPi));
    }
    case `uniform`: {
      return point_exports.sum(offset2, Polar_exports.toCartesian(Math.sqrt(rand()) * radius, rand() * piPi));
    }
    default: {
...
var fromPointsToPath = (a, b) => toPath2(fromPoints(a, b));
...
var toPath2 = (line) => {
  ...
  return Object.freeze({
    ...
    toPoints: () => [a, b],
    rotate: (amountRadian, origin) => toPath2(rotate3(line, amountRadian, origin)),
    nearest: (point3) => nearest2(line, point3),
    sum: (point3) => toPath2(sum3(line, point3)),
    divide: (point3) => toPath2(divide3(line, point3)),
    multiply: (point3) => toPath2(multiply4(line, point3)),
    subtract: (point3) => toPath2(subtract3(line, point3)),
    ...
    apply: (fn) => toPath2(apply2(line, fn)),
    kind: `line`
  });
};
...
var length4 = (arc) => piPi2 * arc.radius * ((arc.startRadian - arc.endRadian) / piPi2);
var bbox5 = (arc) => {
  if (isPositioned3(arc)) {
...
function* circleRings(circle, opts = {}) {
  ...
  for (let r = ringR; r <= 1; r += ringR) {
    ...
    for (const theta of linearSpace(0, piPi4, n + 1)) {
      yield Object.freeze({
...
//# sourceMappingURL=chunk-YQAYXQ6L.js.map