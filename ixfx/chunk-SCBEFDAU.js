...
import {
  ...
  isLine,
  isQuadraticBezier
} from "./chunk-KSSJG6KL.js";
import {
  array
} from "./chunk-BBPDPC3N.js";
import {
  randomElement,
  stack
} from "./chunk-CBXQ4IJG.js";
import {
  SimpleEventEmitter
} from "./chunk-A7BDXUHD.js";
import {
  getMinMaxAvg,
  isEqualDefault,
  toStringDefault
...
var createPath = (svg2, parent, opts) => {
...
var createCircle = (circle2, parent, opts) => {
  const p = document.createElementNS(`http://www.w3.org/2000/svg`, `circle`);
  p.setAttributeNS(null, `cx`, circle2.x.toString());
  p.setAttributeNS(null, `cy`, circle2.y.toString());
  p.setAttributeNS(null, `r`, circle2.radius.toString());
  parent.appendChild(p);
  if (opts)
    applyOpts2(p, opts);
  return p;
};
var createOrResolve = (parent, type, id) => {
  let existing = null;
  if (id !== void 0)
    existing = parent.querySelector(`#${id}`);
  if (existing === null) {
...
var createLine = (line2, parent, opts, id) => {
  const lineEl = createOrResolve(parent, `line`, id);
...
var createText = (pos, text, parent, opts, id) => {
  const p = createOrResolve(parent, `text`, id);
  ...
  if (opts) {
    applyOpts2(p, opts);
    if (opts.anchor)
      p.setAttributeNS(null, `text-anchor`, opts.anchor);
    if (opts.align)
      p.setAttributeNS(null, `alignment-baseline`, opts.align);
  }
  return p;
};
...
var svg = (parent, opts) => {
  if (opts)
    applyOpts2(parent, opts);
  const o2 = {
    text: (pos, text, opts2, id) => createText(pos, text, parent, opts2, id),
    line: (line2, opts2, id) => createLine(line2, parent, opts2, id),
    circle: (circle2, opts2) => createCircle(circle2, parent, opts2),
    path: (svgStr, opts2) => createPath(svgStr, parent, opts2),
...
var MutableMapOf = class extends SimpleEventEmitter {
  ...
  debugString() {
    ...
    let r2 = `Keys: ${keys.join(`, `)}\r
`;
    keys.forEach((k) => {
      const v2 = __privateGet(this, _map).get(k);
      if (v2 !== void 0) {
        const asArray = this.type.toArray(v2);
        if (asArray !== void 0) {
          r2 += ` - ${k} (${this.type.count(v2)}) = ${JSON.stringify(asArray)}\r
`;
        }
      } else
        r2 += ` - ${k} (undefined)\r
`;
  ...
  addValue(...values) {
    values.forEach((v2) => this.addKeyedValues(this.groupBy(v2), v2));
  }
  hasKeyValue(key, value) {
    const m2 = __privateGet(this, _map).get(key);
    if (m2 === void 0)
      return false;
    return this.type.has(m2, value);
  }
  has(key) {
    return __privateGet(this, _map).has(key);
  }
  deleteKeyValue(key, value) {
    const a2 = __privateGet(this, _map).get(key);
    if (a2 === void 0)
      return;
    const filtered = this.type.without(a2, value);
  ...
  count(key) {
    const e2 = __privateGet(this, _map).get(key);
    if (e2 === void 0)
      return 0;
    return this.type.count(e2);
  }
  get(key) {
    const m2 = __privateGet(this, _map).get(key);
    if (m2 === void 0)
      return void 0;
    return this.type.toArray(m2);
  }
  getSource(key) {
  ...
  keysAndCounts() {
    ...
    const r2 = keys.map((k) => [k, this.count(k)]);
    return r2;
  }
  merge(other) {
    const keys = other.keys();
    keys.forEach((key) => {
      const data2 = other.get(key);
      if (data2 !== void 0)
        this.addKeyedValues(key, ...data2);
    });
  }
};
_map = new WeakMap();
var mutableMapCircular = (opts) => {
  ...
  const t2 = {
    add: (dest, values) => {
      ...
      values.forEach((v2) => dest = dest?.add(v2));
      return dest;
    },
    ...
    has: (source, value) => source.find((v2) => comparer(v2, value)) !== void 0,
    without: (source, value) => source.filter((v2) => !comparer(v2, value))
  };
  return new MutableMapOf(t2, opts);
};
...
var PaletteImpl = class {
  ...
  get(key, fallback) {
    ...
    if (alias !== void 0)
      key = alias;
    const c2 = __privateGet(this, _store).get(key);
    if (c2 !== void 0)
      return c2;
    const varName = `--` + key;
...
var draw = (buffer, drawing) => {
  ...
  if (showYAxis) {
    series.forEach((s2) => {
      if (drawing.yAxes !== void 0) {
        if (typeof drawing.yAxes === `string` && s2.name !== drawing.yAxes)
          return;
        if (!drawing.yAxes.includes(s2.name))
          return;
      }
      drawSeriesAxis(s2, drawing);
  ...
  plotDrawing.dataXScale = plotDrawing.width / drawing.capacity, ctx.translate(0, margin);
  series.forEach((s2) => {
    const data2 = buffer.getSource(s2.name);
    if (data2 === void 0)
      return;
    drawSeries(s2, data2, plotDrawing);
  });
  ctx.resetTransform();
};
...
var drawSeries = (series, values, drawing) => {
  ...
  for (let i2 = 0; i2 < values.length; i2 += incrementBy) {
    let y = (1 - (values[i2] - series.min) / series.range) * height;
    if (i2 == 0)
      ctx.moveTo(x, y);
    ctx.lineTo(x, y);
    x += dataXScale;
    if (i2 + 1 == values.pointer) {
      leadingEdge = { x, y };
    }
  }
  ctx.stroke();
...
var plot2 = (parentElOrQuery, opts) => {
  const parentEl = resolveEl(parentElOrQuery);
  if (parentEl.nodeName === `CANVAS`)
    throw new Error(`Parent element should be a container, not a CANVAS`);
  ...
  const buffer = mutableMapCircular({ capacity });
...
var randomPalette = (minColours = 3) => {
  const d = data;
  const c2 = randomElement(d);
  if (c2.combinations.length < minColours)
    return randomPalette(minColours);
  const combo = randomElement(c2.combinations);
  const palette = [];
  for (const v2 of d) {
    if (v2.name === c2.name)
      continue;
    if (v2.combinations.includes(combo)) {
      palette.push(v2);
    }
  }
...
//# sourceMappingURL=chunk-SCBEFDAU.js.map